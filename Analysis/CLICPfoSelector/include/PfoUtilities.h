...
namespace PfoUtil {
  ...
  bool PfoSortFunction(EVENT::ReconstructedParticle* lhs, EVENT::ReconstructedParticle* rhs) {
    ...
    if (lhs_clusters.size() == 0)
      lhs_type = 1;
    if (rhs_clusters.size() == 0)
      rhs_type = 1;
    if (lhs_particleType == 22)
      lhs_type = 10;
    if (rhs_particleType == 22)
      rhs_type = 10;
    if (lhs_particleType == 2112)
      lhs_type = 20;
    if (rhs_particleType == 2112)
      rhs_type = 20;
  ...
  void GetClusterTimes(const Cluster* cluster, float& meanTime, int& nCaloHitsUsed, float& meanTimeEcal, int& nEcal,
