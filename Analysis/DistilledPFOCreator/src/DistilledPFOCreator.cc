...
DistilledPFOCreator::DistilledPFOCreator() : marlin::Processor("DistilledPFOCreator") {
  ...
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "InputParticleCollectionName1",
                          "Input Particle Collection Name1 ", _inputParticleCollectionName1,
                          inputParticleCollectionName1);

  std::string inputParticleCollectionName2 = "GammaGammaParticles";
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "InputParticleCollectionName2",
                          "Input Particle Collection Name2 ", _inputParticleCollectionName2,
...
void DistilledPFOCreator::processRunHeader(LCRunHeader* /*run*/) { return; }
...
void DistilledPFOCreator::processEvent(LCEvent* evt) {
  ...
  if (found) {
    if (_printing > 1)
      std::cout << "Create DistilledPFOs : " << std::endl;
...
void DistilledPFOCreator::end() { return; }
...
// GammaGammaParticles
  ...
  if (_printing > 1)
    std::cout << "FindPFOs : (nPFOs (GammaGammaParticles) = " << _ggpfovec.size() << " )" << std::endl;
...
void DistilledPFOCreator::CreateDistilledPFOs(LCCollectionVec* recparcol) {
  ...
  if (_printing > 1)
    std::cout << "CreateDistilledPFOs : (nPFOs initially = " << _pfovec.size() << " " << _ggpfovec.size() << " )"
              << std::endl;

  typedef std::set<const ReconstructedParticle*> PfoSet;
  PfoSet particles_used; // Set with pointers to the daughter particles (photons for now) that are already used in the
                         // solution
  ...
  for (unsigned int i = 0; i < _ggpfovec.size(); i++) {
    ...
    if (_printing > 3)
      std::cout << "CreateDistilledPFOs: (nparticles = " << particles.size() << " )" << std::endl;
  ...
  if (_printing > 3)
    std::cout << "CreateDistilledPFOs: (nparticles_used = " << particles_used.size() << " )" << std::endl;
  ...
  for (unsigned int i = 0; i < _pfovec.size(); i++) {
    ...
    if (particles_used.find(_pfovec[i]) ==
        particles_used.end()) { // Condition is true if the i'th PandoraPFO is not in the particles_used set
  ...
  if (_printing > 3)
    std::cout << "CreateDistilledPFOs : (nPFOs finally = " << recparcol->getNumberOfElements() << " )" << std::endl;

  if (_printing > 3)
    std::cout << "CreateDistilledPFOs SUMMARY " << ntot << " " << nppfo << " " << ncharged << " " << chargesum << " "
              << esumch << " " << nphotons << " " << esumph << " " << nNH << " " << esumNH << " " << nothers << " "
              << esumothers << " " << esump << " " << esumd << " " << esumgg << " " << V << " " << npi0 << " " << neta
              << " " << netap << " " << epi0 << " " << eeta << " " << eetap << " " << Vpi0 << " " << Veta << " "
              << Vetap << " " << std::endl;
  return;
}
