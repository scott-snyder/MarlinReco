...
ErrorFlow::ErrorFlow() : Processor("ErrorFlow") {
  ...
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "InputPFOCollection", "Input collection of jets",
                          p_inputJetCollection, std::string("InputJets"));

  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "OutputPFOCollection",
                          "Out collection of jets with calculated covariance matrix", p_outputJetCollection,
                          std::string("OutputJets"));

  registerInputCollection(LCIO::LCRELATION, "InputMCTruthLinkCollection", "Input collection of MC Truth Links",
                          p_inputMCTruth, std::string("RecoMCTruthLink"));

  // Reference of default value: doi:10.1016/j.nima.2009.07.026
  registerProcessorParameter("ECALStochastic", "Stochastic term coefficient in ECAL resolution", p_aECAL, (double)0.16);

  // Reference of default value: doi:10.1016/j.nima.2009.07.026
  registerProcessorParameter("ECALConstant", "Constant term coefficient in ECAL resolution", p_cECAL, (double)0.02);

  // Reference of defautl value: 2012 JINST 7 P09017
  registerProcessorParameter("HCALStochastic", "Stochastic term coefficient in HCAL resolution", p_aHCAL, (double)0.50);

  // Reference of defautl value: 2012 JINST 7 P09017
  registerProcessorParameter("HCALConstant", "Constant term coefficient in HCAL resolution", p_cHCAL, (double)0.01);

  registerProcessorParameter("ConfusionScaleFactor", "Scale factor for confusion resolution term", p_scaleConf,
                             (double)1.0);
  ...
  registerProcessorParameter("EnableSemiLepCorrection",
                             ...
                             p_semiLepCorrection, (bool)false);

  registerProcessorParameter("EnableConfusionTerm", "Enable/disable confusion term to be added to covariance matrix",
                             p_confusionterm, (bool)true);
  ...
  registerProcessorParameter("PropagateConfusion2Mom",
                             ...
                             p_propagateConfusiontoMomentumComp, (bool)true);
  ...
  registerProcessorParameter("CovMatFactorNeutralHadrons",
                             ...
                             p_CovMatFactorNeutralHadrons, (double)1.0);

  registerProcessorParameter("SotreInTree", "Enable/disable storing computed qunatities in a ROOT tree", p_storeTree,
                             (bool)false);
    ...
    for (size_t iJet = 0; iJet < nJets; ++iJet) {
      ...
      for (size_t iPFO = 0; iPFO < nPFOs; ++iPFO) {
        ...
        if (p_useFullCovMatNeut) {
          ...
          for (size_t iElement = 0; iElement < 9;
               ++iElement) {    // for all CovMat elements except energy error ( CovMat(9) := sigma_E^2 )
            if (iElement < 6) { // for elements not involving E ( sigma_px^2 , sigma_pxpy , sigma_py^2 , sigma_pxpz ,
                                // sigma_pypz , sigma_pz^2 )
              jetCovMatrix[iElement] += scaleFactor * scaleFactor * particleCovMatrix[iElement];
            } else { // for elements involving E ( sigma_pxE , sigma_pyE , sigma_pzE )
      ...
      relConfSquared = pow(relConfCharged, 2.0) + pow(relConfPhotons, 2.0) + pow(relConfNeutral, 2.0);
      absConfSquared = relConfSquared * eJetTotal * eJetTotal;

      // Compute absolute semi-leptonic resolution squared
      absSemiLepResSquared = p_semiLepSigmaCorrFactor * p_semiLepSigmaCorrFactor * eLeptonsTotal * eLeptonsTotal;
      ...
      if (p_confusionterm && p_propagateConfusiontoMomentumComp) {
        for (int iElement = 0; iElement < 10; ++iElement) {
          jetCovMatrix[iElement] = jetCovMatrix[iElement] +
                                   (pow(jetEnergy, 2) * absConfSquared * p_scaleConf * p_scaleConf / pow(jetMom, 2)) *
                                       jetCovMatrixConf[iElement];
        }
      } else {
        jetCovMatrix[9] = jetCovMatrix[9] + (p_confusionterm ? absConfSquared * p_scaleConf * p_scaleConf : 0.0) +
 ...
 * =====================================================================================
 */
double* ErrorFlow::getRelativeConfusion(double t_jetEnergy, double t_chargedHadronsEnergy, double t_photonsEnergy,
                                        double t_neutralHadronsEnergy, double t_confusion[]) {
  ...
  if (45.0 >= t_jetEnergy) {
    ...
    t_confusion[0] =
        (((t_jetEnergy - 45.0) * (conf100[0] - conf45[0]) / (100.0 - 45.0)) + conf45[0]) * gammaChargedHadrons / 100.0;
    t_confusion[1] =
        (((t_jetEnergy - 45.0) * (conf100[1] - conf45[1]) / (100.0 - 45.0)) + conf45[1]) * gammaPhotons / 100.0;
    t_confusion[2] =
        (((t_jetEnergy - 45.0) * (conf100[2] - conf45[2]) / (100.0 - 45.0)) + conf45[2]) * gammaNeutalHadrons / 100.0;
  } else if (100 < t_jetEnergy && 180.0 >= t_jetEnergy) {
    t_confusion[0] = (((t_jetEnergy - 100.0) * (conf180[0] - conf100[0]) / (180.0 - 100.0)) + conf100[0]) *
                     gammaChargedHadrons / 100.0;
    t_confusion[1] =
        (((t_jetEnergy - 100.0) * (conf180[1] - conf100[1]) / (180.0 - 100.0)) + conf100[1]) * gammaPhotons / 100.0;
    t_confusion[2] = (((t_jetEnergy - 100.0) * (conf180[2] - conf100[2]) / (180.0 - 100.0)) + conf100[2]) *
                     gammaNeutalHadrons / 100.0;
  } else {
    t_confusion[0] = (((t_jetEnergy - 180.0) * (conf250[0] - conf180[0]) / (250.0 - 180.0)) + conf180[0]) *
                     gammaChargedHadrons / 100.0;
    t_confusion[1] =
        (((t_jetEnergy - 180.0) * (conf250[1] - conf180[1]) / (250.0 - 180.0)) + conf180[1]) * gammaPhotons / 100.0;
    t_confusion[2] = (((t_jetEnergy - 180.0) * (conf250[2] - conf180[2]) / (250.0 - 180.0)) + conf180[2]) *
                     gammaNeutalHadrons / 100.0;
  }
...
double ErrorFlow::getPhotonSigmaESqr(double t_phEnergy) {
  double sigmaEnSquared = (p_aECAL * p_aECAL) * t_phEnergy + (p_cECAL * p_cECAL) * (t_phEnergy * t_phEnergy);
...
double ErrorFlow::getNeuHadSigmaESqr(double t_neuHadEnergy) {
  double sigmaEnSquared =
      (p_aHCAL * p_aHCAL) * t_neuHadEnergy + (p_cHCAL * p_cHCAL) * (t_neuHadEnergy * t_neuHadEnergy);
