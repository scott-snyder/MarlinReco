...
Add4MomCovMatrixCharged::Add4MomCovMatrixCharged() : Processor("Add4MomCovMatrixCharged") {
  ...
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "InputPandoraPFOsCollection",
                          "Name of the PandoraPFOs collection", _colPFOs, std::string("PandoraPFOs"));
}
...
void Add4MomCovMatrixCharged::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }
...
void Add4MomCovMatrixCharged::processEvent(LCEvent* evt) {
  ...
  for (int i = 0; i < colPFO->getNumberOfElements(); ++i) {
    ...
    ReconstructedParticleImpl* recPar = dynamic_cast<ReconstructedParticleImpl*>(colPFO->getElementAt(i));
      ...
      if (tracks.size() == 2 && (itype == 22 || itype == 310 || itype == 3122)) {
        for (int itrk = 0; itrk < 2; itrk++) {
          ...
          for (size_t j = 0; j < 10; j++)
            covarianceP[j] += covariancePtrk[j];
          delete trackPFO;
        }
    ...
    if (newcov) {
      try {
        ...
        streamlog_out(DEBUG5) << "Set covariance matrix for charged pfo with charge " << (int)recPar->getCharge()
                              << " and (px,py,pz,e) = (" << recPar->getMomentum()[0] << "," << recPar->getMomentum()[1]
                              << "," << recPar->getMomentum()[2] << "," << recPar->getEnergy() << "):" << std::endl
                              << std::setprecision(6) << " cov1[px px] " << recPar->getCovMatrix()[0] << std::endl
...
void Add4MomCovMatrixCharged::end() {
  ...
  streamlog_out(MESSAGE4) << " processed " << _nEvt << " events in " << _nRun << " runs " << std::endl;
}
