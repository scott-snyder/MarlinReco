...
MomentumCov::MomentumCov() : Processor("MomentumCov") {
  ...
  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "OutputNewPFOsCollection",
                           std::string("Name of the new PFOs collection") += " with covariance matrix", _colNewPFOs,
                           std::string("Newpfos"));
}
...
void MomentumCov::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }
...
void MomentumCov::processEvent(LCEvent* evt) {
  ...
  for (int i = 0; i < colPFO->getNumberOfElements(); ++i) {
    ...
    ReconstructedParticle* recPar = dynamic_cast<ReconstructedParticle*>(colPFO->getElementAt(i));
