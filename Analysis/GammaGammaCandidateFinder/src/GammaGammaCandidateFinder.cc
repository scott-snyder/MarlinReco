...
GammaGammaCandidateFinder::GammaGammaCandidateFinder() : marlin::Processor("GammaGammaCandidateFinder") {
  ...
  registerProcessorParameter("GammaGammaResonanceName", "Particle decaying to Gamma Gamma", _ggResonanceName,
                             ggResonanceName);
  ...
  registerProcessorParameter("GammaMomentumCut", "Minimum Momentum Cut for Photon (GeV)", _gammaMomentumCut,
                             (float)0.5);

  registerProcessorParameter("GammaGammaResonanceMass", "Nominal Mass of Particle Decaying to Gamma Gamma (GeV)",
                             _ggResonanceMass, (float)0.135);

  registerProcessorParameter("MaxDeltaMgg",
                             "Maximum difference between candidate mass and GammaGama Resonance mass (GeV)", _dmggcut,
                             (float)0.040);

  registerProcessorParameter("FitProbabilityCut", "Minimum fit probability", _fitProbabilityCut, (double)0.001);

  registerProcessorParameter("fitter", "0 = OPALFitter, 1 = NewFitter, 2 = NewtonFitter", _ifitter, (int)0);
...
void GammaGammaCandidateFinder::processRunHeader(LCRunHeader* /*run*/) { return; }
...
void GammaGammaCandidateFinder::processEvent(LCEvent* evt) {
  ...
  if (found) {
    if (_printing > 1)
      std::cout << "Analysis : " << _ggResonanceName << std::endl;
...
void GammaGammaCandidateFinder::end() { return; }
...
void GammaGammaCandidateFinder::FindGammaGammaCandidates(LCCollectionVec* recparcol) {
  ...
  if (_printing > 1)
    std::cout << "FindGammaGammaCandidates : (nPFOs = " << _pfovec.size() << " )" << std::endl;
  ...
  for (unsigned int i = 0; i < _pfovec.size(); i++) {
    ...
    if (fabs(_pfovec[i]->getCharge() - 0.0) < 0.5) {
      ...
      if (_pfovec[i]->getEnergy() < _gammaMomentumCut)
        photon = false;
      if (photon) {
        if (_printing > 1)
          std::cout << "FindGammaGammaCandidates : Photon " << _pfovec[i]->getType() << std::endl;
        LorentzVector pgam(_pfovec[i]->getMomentum()[0], _pfovec[i]->getMomentum()[1], _pfovec[i]->getMomentum()[2],
                           _pfovec[i]->getEnergy());
  ...
  if (_printing > 1)
    std::cout << "FindGammaGammaCandidates : (nphotons = " << pGammaPfoVec.size() << " " << pgamma.size() << " )"
              << std::endl;
  ...
  if (pgamma.size() >= 2) {
    for (unsigned int i = 0; i < pgamma.size() - 1; i++) {
      for (unsigned int j = i + 1; j < pgamma.size(); j++) {
        ...
        float mgg = pgg.m();
        if (_printing > 2)
          std::cout << "Testing for " << _ggResonanceName << " " << i << " " << j << "  M = " << mgg << std::endl;
        ...
        if (fabs(mgg - _ggResonanceMass) < _dmggcut) {
          ...
          JetFitObject j1(pgi.e(), pgi.theta(), pgi.phi(), 0.16 * std::sqrt(pgi.e()), 0.001 / std::sqrt(pgi.e()),
                          0.001 / std::sqrt(pgi.e()), 0.0);
          //          j1->setName("Photon1");
          JetFitObject j2(pgj.e(), pgj.theta(), pgj.phi(), 0.16 * std::sqrt(pgj.e()), 0.001 / std::sqrt(pgj.e()),
                          0.001 / std::sqrt(pgj.e()), 0.0);
          //          j2->setName("Photon2");
          ...
          // Get the individual fitted photon 4-vectors - needed to calculate the four-momentum covariance matrix of the
          // gamma-gamma system
          ...
          if (cov_dim == 6) {
            ...
            double jacobian_by_columns[nrows * ncolumns] = {Mom1[0] / E1,
                                                            Mom1[0] * Mom1[2] / pT1,
                                                            -Mom1[1],
                                                            Mom2[0] / E2,
                                                            Mom2[0] * Mom2[2] / pT2,
            ...
            for (int ii = 0; ii < 4; ii++) {
              for (int jj = 0; jj < 4; jj++) {
                if (_printing > 3)
                  std::cout << "4-vector cov. " << ii << " " << jj << " " << Covmatrix(ii, jj) << std::endl;
              }
            }
            ...
            if (_printing > 1)
              std::cout << " Fit has no valid covariance information (cov_dim = " << cov_dim << " )" << std::endl;
          }
          ...
          if (_ggResonanceName == "Eta")
            recoPart->setType(221);
          if (_ggResonanceName == "EtaPrime")
            recoPart->setType(331);

          if (_printing > 1)
            std::cout << "Fitting " << _ggResonanceName << " gg candidate  M= " << mgg << " " << i << " " << j << " "
                      << pgi.e() << " " << pgj.e() << " Fitted energies: " << j1.getE() << " " << j2.getE()
          ...
          if (fit_probability > _fitProbabilityCut) {
            if (_printing > 1)
              std::cout << "GG candidate passes fit probability cut - KEEP" << std::endl;
