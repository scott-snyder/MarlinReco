...
GammaGammaCandidateTruthFilter::GammaGammaCandidateTruthFilter() : Processor("GammaGammaCandidateTruthFilter") {
  ...
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "GammaGammaCandidateCollection",
  ...
  /*
    ...
    while( MCParticle* mcp = mcpIt.next()  ) {
      ...
      if (keep) {
        ...
        streamlog_out(DEBUG) << " Found GammaGamma decaying parent MCParticle " << "Index = " << index_mcp << " pdgID =
    " << mcp->getPDG()
                             << ", genstat = " << mcp->getGeneratorStatus()
                             << ", E = " << mcp->getEnergy() << std::endl;
        streamlog_out(DEBUG) << " Momentum = " << mcp->getMomentum()[0] << " " << mcp->getMomentum()[1] << " " <<
    mcp->getMomentum()[2] << std::endl; streamlog_out(DEBUG) << " Vertex =   " << mcp->getVertex()[0] << " " <<
    mcp->getVertex()[1] << " " << mcp->getVertex()[2] << std::endl; streamlog_out(DEBUG) << " Endpoint = " <<
    mcp->getEndpoint()[0] << " " << mcp->getEndpoint()[1] << " " << mcp->getEndpoint()[2] << std::endl;
    ...
    mcd->getMomentum()[2] << std::endl; streamlog_out(DEBUG) << " Vertex =   " << mcd->getVertex()[0] << " " <<
    mcd->getVertex()[1] << " " << mcd->getVertex()[2] << std::endl; streamlog_out(DEBUG) << " Endpoint = " <<
    mcd->getEndpoint()[0] << " " << mcd->getEndpoint()[1] << " " << mcd->getEndpoint()[2] << std::endl;
          ...
          if (maxcaloweight > 0.1) {
            streamlog_out(MESSAGE) << " found reco photon for mcd at imaxcaloweight = " << imaxcaloweight << " with
    weight = " << maxcaloweight << std::endl ; nseenphoton++; weightsum += maxcaloweight;
          }
    ...
    streamlog_out(DEBUG) << " nMCGammaGammaParticles Found = " << nMCPi0
                         << " ( " << nMCPi0-nMCEta-nMCEtaPrime << " " << nMCEta << " " << nMCEtaPrime << " )" << " Esum
    = " << ESum_MCP << std::endl;
  */
  ...
  while (ReconstructedParticle* ggp = ggpIt.next()) {
    ...
    for (unsigned int igamma = 0; igamma < gammas.size(); igamma++) {
      ...
      //      ESum_Reco_Meas += gammas[igamma]->getEnergy();    // Measured Energy Sum of the Photons that are fitted to
      //      GammaGammaParticles
      for (int i = 0; i < 3; i++)
      ...
      if (!isConversion) {
        for (unsigned int irel = 0; irel < truevec.size(); irel++) {
          ...
          streamlog_out(DEBUG) << " irel " << irel << " E = " << dummy->getEnergy()
                               << ", truew = " << int(truthweightvec.at(irel)) << ", true PDG = " << dummy->getPDG()
                               << " cos(RP - MC) " << cosg << ", true par. PDG = " << dummy->getParents()[0]->getPDG()
  ...
  if (typeRequired == 111)
    streamlog_out(MESSAGE) << "                SUMMARY 111 " << nCorrectPi0 << " " << ESum_Reco_Correct << " "
                           << ESum_Reco_MC_Correct << " " << ESum_Reco_Meas << endl;
  if (typeRequired == 221)
    streamlog_out(MESSAGE) << "                SUMMARY 221 " << nCorrectEta << " " << ESum_Reco_Correct << " "
                           << ESum_Reco_MC_Correct << " " << ESum_Reco_Meas << endl;
  if (typeRequired == 331)
    streamlog_out(MESSAGE) << "           SUMMARY 331 " << nCorrectEtaPrime << " " << ESum_Reco_Correct << " "
                           << ESum_Reco_MC_Correct << " " << ESum_Reco_Meas << endl;
  ...
  /*
    ...
    " "
                                           << std::setw(10) << ESum_Reco_Correct << " " << std::setw(10) <<
    ESum_Reco_Wrong << " "
