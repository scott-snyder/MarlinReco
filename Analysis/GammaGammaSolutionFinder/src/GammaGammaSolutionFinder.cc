...
GammaGammaSolutionFinder::GammaGammaSolutionFinder() : marlin::Processor("GammaGammaSolutionFinder") {
  ...
  registerProcessorParameter("Printing", "Print certain messages", _printing, (int)1);
  ...
  registerInputCollections(LCIO::RECONSTRUCTEDPARTICLE, "GammaGammaCandidateCollections",
                           "Gamma-Gamma Candidate Collection Names", _gammagammaCandidateCollections,
                           gammagammaCandidateCollections);

  std::string outputParticleCollectionName = "GammaGammaSolutions";
  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "OutputParticleCollectionName",
                           "Output Particle Collection Name ", _outputParticleCollectionName,
                           outputParticleCollectionName);

  registerProcessorParameter("MaxCombinationsCut", "Maximum Number of Explorable Combinations", _maxCombinationsCut,
                             (double)100000000.0);

  registerProcessorParameter("LessThanMaximalCut", "Maximum Number of Fewer GammaGammaParticles", _nToRemove,
                             (int)0); // (set to huge number - like 999 if want to explore all)

  registerProcessorParameter("SolutionFindingAlgorithm", "Solution Finding Algorithm", _algorithm, (int)2);
...
void GammaGammaSolutionFinder::processRunHeader(LCRunHeader* /*run*/) { return; }
...
void GammaGammaSolutionFinder::processEvent(LCEvent* evt) {
  ...
  if (found) {
    if (_printing > 1)
      std::cout << "Find GammaGamma Solution: " << std::endl;
    unsigned int nphotons = this->CountIndependentPhotons();
    unsigned int nedges = _pfovec.size();

    if (_printing > 1)
      std::cout << "nphotons = " << nphotons << " nedges = " << nedges << std::endl;
    unsigned long int nCr = this->CombinatorialFactor(nedges, nphotons / 2);
    if (_printing > 1)
      std::cout << "Naive combinatorial factor of " << std::setw(22) << nCr << std::endl;
    ...
    if (_algorithm == 1) {
      ...
      if (double(nCr) < _maxCombinationsCut) {
        if (_printing > 1)
          std::cout << " Pursuing full combinatoric exploration " << std::endl;
        this->FindGammaGammaSolutions(recparcol);
      } else {
        if (_printing > 1)
          std::cout << " Too many combinations - assigning quick greedy solution " << std::endl;
        this->FindGammaGammaSolutionZero(recparcol);
      }
    } else {
...
void GammaGammaSolutionFinder::end() { return; }
...
void GammaGammaSolutionFinder::FindCombinations(std::vector<std::vector<int>> array, unsigned int i,
...
// All GammaGammaCandidates
  ...
  if (_printing > 1)
    std::cout << "FindPFOs : (nPFOs = " << _pfovec.size() << " )" << std::endl;

  if (_printing > 1)
    std::cout << "Find PFOs : " << tf << std::endl;
...
void GammaGammaSolutionFinder::FindGammaGammaSolutionZero(LCCollectionVec* recparcol) {
  ...
  if (_printing > 1)
    std::cout << "FindGammaGammaSolutionZero : (nPFOs = " << _pfovec.size() << " )" << std::endl;

  typedef std::set<const ReconstructedParticle*> PfoSet;
  PfoSet particles_assigned; // Set with pointers to the daughter particles (photons for now) that are already used in
                             // the solution
  ...
  // Algorithm 1. Based on fit probability ordering, add GammaGammaCandidates to the output GammaGammaParticles
  // collection if they do not contain already assigned constituent particles. For now, the constituents are always two
  // photons
  ...
  for (unsigned int i = 0; i < _pfovec.size(); i++) {
    ...
    if (_printing > 3)
      std::cout << "FindGammaGammaSolutionZero: (nparticles = " << particles.size() << " )" << std::endl;
    ...
    for (unsigned int j = 0; j < particles.size(); j++) { // TODO - maybe double check these are indeed photons ..
      ...
      if (particles_assigned.find(particle) != particles_assigned.end())
        assignable = false; // If particle is already assigned then it is not still assignable
    }
    if (assignable) {
      recparcol->addElement(
          _pfovec[i]); // Add meson to the output collection if all constituent particles are still assignable
...
unsigned int GammaGammaSolutionFinder::CountIndependentPhotons() {
  ...
  if (_printing > 1)
    std::cout << "Count Independent Photons : (nPFOs = " << _pfovec.size() << " )" << std::endl;
  ...
  if (_printing > 1)
    std::cout << "Number of independent photons available = " << daughter_particles.size() << std::endl;
...
void GammaGammaSolutionFinder::FindGammaGammaSolutions(LCCollectionVec* recparcol) {
  ...
  if (_printing > 1)
    std::cout << "FindGammaGammaSolutions : (nPFOs = " << _pfovec.size() << " )" << std::endl;
  ...
  for (unsigned int i = 0; i < _pfovec.size(); i++) {
    const ReconstructedParticleVec particles = _pfovec[i]->getParticles();
    if (_printing > 3)
      std::cout << "FindGammaGammaSolutions: (nparticles = " << particles.size() << " )" << std::endl;
    for (unsigned int j = 0; j < particles.size(); j++) { // TODO - maybe double check these are indeed photons ..
      const ReconstructedParticle* particle = particles[j];
      // GWW DEBUG
      if (_printing > 7)
      ...
      if (daughter_particles.find(particle) == daughter_particles.end()) {
        ...
        if (_printing > 3)
          std::cout << "New photon " << particle << " " << k << std::endl;
      }
    }
  }
  ...
  if (_printing > 3)
    std::cout << "Set size " << daughter_particles.size() << std::endl;
  for (std::set<const ReconstructedParticle*>::iterator it = daughter_particles.begin(); it != daughter_particles.end();
       ++it) {
    if (_printing > 3)
  ...
  if (_printing > 5)
    std::cout << "Graph definition file information Dry-Run" << std::endl;
  if (_printing > 5)
    std::cout << std::setw(4) << daughter_particles.size() << std::setw(4) << _pfovec.size() << std::endl;
  ...
  for (unsigned int i = 0; i < _pfovec.size(); i++) {
    const ReconstructedParticleVec particles = _pfovec[i]->getParticles();
    if (_printing > 5)
      std::cout << "GammaGammaCandidate " << std::setw(4) << i << std::endl;
    for (unsigned int j = 0; j < particles.size(); j++) { // TODO - maybe double check these are indeed photons ..
      const ReconstructedParticle* particle = particles[j];
      ...
      if (retcode.second) {
        if (_printing > 5)
          std::cout << particle << " inserted as element ";
      } else {
        if (_printing > 5)
          std::cout << particle << " already exists as element ";
      }
      if (_printing > 5)
        std::cout << std::distance(daughter_particles.begin(), retcode.first) << std::endl;
    }
  }
  ...
  if (_printing > 6)
    std::cout << "GRAPH definition file information" << std::endl;
  ...
  for (unsigned int i = 0; i < _pfovec.size(); i++) {
    ...
    for (unsigned int j = 0; j < particles.size(); j++) { // TODO - maybe double check these are indeed photons ..
      ...
      retcode = daughter_particles.insert(particle); // insert should fail as all photons should already be in the set
      if (_printing > 6)
        std::cout << "  " << std::setw(4) << std::distance(daughter_particles.begin(), retcode.first);
      if (j == 0)
        u = std::distance(daughter_particles.begin(), retcode.first);
      if (j != 0)
        v = std::distance(daughter_particles.begin(), retcode.first);
    }
    ...
    if (_printing > 6)
      std::cout << "  " << std::setw(4) << particleType << "  " << std::fixed << std::setw(10) << std::setprecision(6)
                << GoodnessOfPID << std::endl;
  ...
  if (_printing > 6)
    std::cout << " " << std::endl;
  if (_printing > 6)
    std::cout << "Edge summary " << std::endl;
  for (int i = 0; i < nedges; ++i) {
    if (_printing > 6)
      std::cout << std::setw(3) << i << " " << evec[i].vbits << " " << std::setw(2) << evec[i].u << " " << std::setw(2)
                << evec[i].v << std::endl;
  }
  if (_printing > 6)
    std::cout << " " << std::endl;
  ...
  for (int i = 0; i < nvertices; ++i) {
    if (_printing > 6)
      std::cout << std::setw(3) << i << " " << std::setw(3) << vertex_degree[i] << std::endl;
    vdsum += vertex_degree[i];
  }
  if (_printing > 6)
    std::cout << "vdsum = " << vdsum << std::endl;

  if (_printing > 6)
    std::cout << "Vertex - Vertex partnerships" << std::endl;
  for (int iv = 0; iv < nvertices; ++iv) {
    if (_printing > 6)
      std::cout << "Vertex " << std::setw(2) << iv << ": ";
    for (int je = 0; je < nedges; ++je) {
      if (evec[je].u == iv) {
        if (_printing > 6)
          std::cout << std::setw(2) << evec[je].v << " ";
      }
    }
    if (_printing > 6)
      std::cout << std::endl;
  ...
  if (_printing > 6)
    std::cout << "Vertex - Edge partnerships" << std::endl;
  for (int iv = 0; iv < nvertices; ++iv) {
    if (_printing > 6)
      std::cout << "Vertex " << std::setw(2) << iv << ": ";
    febits = 0;
  ...
  if (_printing > 6)
    std::cout << " " << std::endl;
  if (_printing > 6)
    std::cout << "Check FirstVertex structure" << std::endl;
  if (_printing > 6)
    std::cout << std::setw(2) << fvec.size() << " FirstVertices out of " << std::setw(2) << nvertices
              << " vertices in total" << std::endl;
  if (_printing > 6)
    std::cout << " i iv ne           febits                 Edges" << std::endl;

  int nfirst_vertices = 0;
  for (unsigned int i = 0; i < fvec.size(); ++i) {
    nfirst_vertices++;
    if (_printing > 6)
      std::cout << std::setw(2) << i << " " << std::setw(2) << fvec[i].ivertex << " " << std::setw(2) << fvec[i].nedges
                << " " << fvec[i].febits << "  { ";
  ...
  if (_printing > 6)
    std::cout << "nfirst_vertices = " << nfirst_vertices << std::endl;
  ...
  if (_printing > 2)
    std::cout << "Setting rmax to " << rmax << std::endl;
  ...
  if (_printing > 2)
    std::cout << "Setting rmin to " << rmin << std::endl;

  gettimeofday(&tv, NULL);
  double t1 = tv.tv_sec + (tv.tv_usec / 1000000.0);
  double t2 = 0.0;
  if (_printing > 3)
    std::cout << "Initialization took " << std::fixed << std::setw(12) << std::setprecision(6) << t1 - t0 << " (s) "
              << std::endl;
  ...
  for (unsigned int r = rmax; r >= rmin; --r) { // Try various size matchings
    ...
    if (_printing > 2)
      std::cout << "subsidiary matching problem: find " << r << " edges from the set of size " << n
    ...
    if (_printing > 2)
      std::cout << "combination counting ... nfv = " << n << " r = " << std::setw(2) << r << "  nCr = " << std::setw(22)
                << nCr << "  nCr (fv) = " << std::setw(22) << nCr2 << std::endl;
    if (_printing > 2)
      std::cout << std::endl;
    int nmatch_withr = 0;
    ...
    std::fill(vb.begin() + r, vb.end(),
              true); // fill elements vb[r] .... vb[n-1] with true -- other elements should be false
    do {
      for (int i = 0; i < n; ++i) {
        if (!vb[i]) {
      ...
      // Probably best to create a vector of vectors with all the combinations of edges associated with each first
      // vertex, and loop through those.
      ...
      if (!comb.empty())
        comb.clear(); // Need to also clear the output array - otherwise it just keeps accumulating.
        ...
        if (nused == 2 * r) { // Number of vertices used equals twice the number of requested edges ( = MATCH )
          ...
          for (unsigned int j = 0; j < r; ++j) {
            /*              std::cout << "Assigned edge details" << std::endl;
                          std::cout << std::setw(2) << c[i] << " " << evec[c[i]].vbits << " " << std::setw(2) <<
               evec[c[i]].u << " "
                                    << std::setw(2) << evec[c[i]].v << " " << evec[c[i]].pdgid
                                    << std::fixed << std::setw(10) << std::setprecision(5) << evec[c[i]].w << " "
                                    << std::fixed << std::setw(10) << std::setprecision(6) << evec[c[i]].edge_pvalue <<
               std::endl;   */
            ebits.flip(c[j]); // Set appropriate bit in the bitset for edge c[i]
            wsum += evec[c[j]].w;
            if (evec[c[j]].pdgid == 111)
              npi0++;
            if (evec[c[j]].pdgid == 221)
              neta++;
            if (evec[c[j]].pdgid == 331)
              netap++;
          }
          ...
          // Not really needed to calculate the pvalue - but this was measured to be fairly fast (0.1s degradation for
          // 47000 matches).
          double dof = double(r);
      ...
      if (!d.empty())
        d.clear(); // Clear vector d
      if (!array.empty())
        array.clear(); // Clear vector of vectors array

      icombi++; // Increment combination index
    } while (std::next_permutation(vb.begin(), vb.end()));

    if (_printing > 2)
      std::cout << "Found " << std::setw(7) << nmatch_withr << " matches with " << std::setw(3) << r << " edges from "
                << std::setw(22) << nCr << " combinations " << std::endl;

    gettimeofday(&tv, NULL);
    t2 = tv.tv_sec + (tv.tv_usec / 1000000.0);
    if (_printing > 3)
      std::cout << "Match search for n = " << std::setw(2) << n << " r = " << std::setw(2) << r << " " << std::fixed
                << std::setw(12) << std::setprecision(6) << t2 - t1 << " (s) " << std::fixed << std::setw(12)
                << std::setprecision(6) << 1.0e6 * (t2 - t1) / double(nCr) << " micro-seconds per combination "
                << std::endl;
    t1 = t2;
  ...
  if (_printing > 2)
    std::cout << "Found " << std::setw(7) << nmatch << " matches total " << std::endl;

  if (_printing > 5)
    std::cout << "Two particle hypothesis results (pi0, eta) with rmax = " << rmax << std::endl;
  ...
  for (unsigned int r = rmax; r > 0; --r) {
    ...
    for (unsigned int i = 0; i < svec.size(); ++i) {
      if (svec[i].ne == int(r) && svec[i].netap == 0) {
        ...
        //              std::cout << "Sums etc " << imin << " " << min_wsum << " " << imax << " " << max_wsum <<
        //              std::endl;
      }
    }
    //       std::cout << " Checking solutions " << r << " " << imin << " " << imax << std::endl;
    if (r == rmax && imin >= 0)
      ibest = imin; // Flag this as the "best solution".
    if (imin >= 0 && imax >= 0)
      if (_printing > 5)
        std::cout << "wsum extrema for r = " << std::setw(2) << r << " " << std::setw(10) << min_wsum << " "
                  << std::setw(10) << max_wsum << " pvalues " << std::setw(12) << svec[imin].pvalue << "  "
                  << std::setw(12) << svec[imax].pvalue << "  ( " << imin << " " << imax << " ) " << " Combinations "
                  << svec[imin].icomb << " " << svec[imax].icomb << std::endl;
  }
  // Examine best solution
  if (ibest >= 0) {
    double pvalue = gsl_cdf_chisq_Q(svec[ibest].wsum, svec[ibest].ne);
    if (_printing > 4)
      std::cout << "Best candidate solution " << ibest << std::endl;
    if (_printing > 4)
      std::cout << "npi0, neta, netap " << svec[ibest].npi0 << " " << svec[ibest].neta << " " << svec[ibest].netap
                << std::endl;
    if (_printing > 4)
      std::cout << std::setw(4) << ibest << " " << std::setw(2) << svec[ibest].ne << " " << std::setw(2)
                << svec[ibest].nv << "   " << svec[ibest].ebits << " " << std::fixed << std::setw(10)
                << std::setprecision(5) << svec[ibest].wsum << " " << std::fixed << std::setw(10)
                << std::setprecision(6) << pvalue << std::endl;
    // Now loop over the edges
    for (unsigned int i = 0; i < NEMAX; ++i) {
      if ((svec[ibest].ebits).test(i)) {
        if (_printing > 4)
          std::cout << "Edge " << std::setw(2) << i << " " << std::setw(2) << evec[i].u << " " << std::setw(2)
                    << evec[i].v << " " << evec[i].pdgid << " " << std::fixed << std::setw(10) << std::setprecision(6)
                    << evec[i].edge_pvalue << std::endl;
      }
    }
  }
  ...
  if (_printing > 2)
    std::cout << "Solution analysis time = " << std::fixed << std::setw(12) << std::setprecision(6) << t3 - t2
              << " (s) " << std::endl;
  if (_printing > 2)
    std::cout << "Total elapsed time     = " << std::fixed << std::setw(12) << std::setprecision(6) << t3 - t0
              << " (s) " << std::endl;
  ...
  if (_printing > 3)
    std::cout << "ALGORITHM2 RESULTS" << std::endl;
  for (unsigned int i = 0; i < _pfovec.size(); ++i) {
    if ((svec[ibest].ebits).test(i)) {
      const ReconstructedParticleVec particles = _pfovec[i]->getParticles();
      bool assignable = true;
      ...
      if (assignable) {
        ...
        for (unsigned int j = 0; j < particles.size(); ++j) {
          ...
          retcode =
              daughter_particles.insert(particle); // insert should fail as all photons should already be in the set
          if (_printing > 3)
            std::cout << "  " << std::setw(4) << std::distance(daughter_particles.begin(), retcode.first);
        }
        const int particleType = _pfovec[i]->getType();
        // GoodnessOfPID currently filled with fit probability - may at some point be more convenient to fill with the
        // fit chi-squared ...
        const float GoodnessOfPID = _pfovec[i]->getGoodnessOfPID();
        if (_printing > 3)
          std::cout << "  " << std::setw(4) << particleType << "  " << std::fixed << std::setw(10)
                    << std::setprecision(6) << GoodnessOfPID << std::endl;
      }
    }
  }
  return;
...
bool GammaGammaSolutionFinder::PfoProbabilitySortFunction(ReconstructedParticle* lhs, ReconstructedParticle* rhs) {
  ...
  */
  ...
  // GoodnessOfPID currently filled with fit probability - may at some point be more convenient to fill with the fit
  // chi-squared ...
  const float lhs_GoodnessOfPID = lhs->getGoodnessOfPID();
  const float rhs_GoodnessOfPID = rhs->getGoodnessOfPID();

  if (lhs_particleType == rhs_particleType)
    return (lhs_GoodnessOfPID > rhs_GoodnessOfPID); // This rank makes sense when GoodnessOfPID has the fit probability
