...
CheatedMCOverlayRemoval::CheatedMCOverlayRemoval()
    :
      ...
      Processor("CheatedMCOverlayRemoval"), m_nRun(0), m_nEvt(0), m_nAllPFOs(0), m_nMCPs(0) {

  _description = "CheatedMCOverlayRemoval identifies MC particles that are overlay and removes the corresponding PFOs "
                 "from the collection";
  ...
  registerInputCollection(LCIO::MCPARTICLE, "MCParticleCollection", "Name of the MCParticle collection",
                          _MCParticleCollectionName, std::string("MCParticlesSkimmed"));
  ...
  registerInputCollection(LCIO::LCRELATION, "MCTruthRecoLink", "Name of the MCTruthRecoLink input collection",
                          _mcTruthRecoLink, std::string("MCTruthRecoLink"));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "OutputPfoCollection",
                           "Name of output PFO collection with Overlay particles removed", _OutputPfoCollection,
                           std::string("PFOsWithoutOverlay"));
  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "OutputOverlayCollection",
                           "Name of output PFO collection of the Overlay particles", _OutputOverlayCollection,
...
void CheatedMCOverlayRemoval::processEvent(LCEvent* pLCEvent) {
  ...
  try {
    ...
    for (int i = 0; i < m_nMCPs; i++) {
      ...
      if (mcp->isOverlay()) {
        ...
        ReconstructedParticle* linkedPFO =
            getLinkedPFO(mcp, RecoMCParticleNav, MCParticleRecoNav, weightPFOtoMCP, weightMCPtoPFO);
        if (linkedPFO == NULL) {
          continue;
        }
    ...
    for (int i = 0; i < m_nAllPFOs; i++) {
      ...
      if (linkedMCP != NULL) {
        if (linkedMCP->isOverlay()) {
          streamlog_out(DEBUG5) << "i = " << i << ": energy = " << pfo->getEnergy()
                                << ", gen status = " << linkedMCP->getGeneratorStatus() << std::endl;
          continue;
        }
      }
    ...
    streamlog_out(DEBUG5) << "In event " << m_nEvt << ": Kept PFOs = " << nKeptPFOs
                          << " VS removed number of PFOs = " << nRemovedPFOs
                          << " VS total number of PFOs = " << m_nAllPFOs << std::endl;
...
EVENT::MCParticle* CheatedMCOverlayRemoval::getLinkedMCP(EVENT::ReconstructedParticle* recoParticle,
  ...
  if (iPFOtoMCPmax != -1) {
    ...
    for (unsigned int i_pfo = 0; i_pfo < PFOvec.size(); i_pfo++) {
      ...
      streamlog_out(DEBUG0) << "Test Visible MCParticle linkWeight to PFO: " << PFOweightvec.at(i_pfo)
