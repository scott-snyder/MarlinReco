 ...
 *
 * The CPIDProcessor gathers PID-related observables, trains a model to optimise PID determination and infers from the
 * model to data. The processors uses modules to achieve this: InputAlgorithms to collect PID information,
 * TrainingModels to train and infer. The modules are made available via dynamic loading, using a base class and a Mgr.
 ...
 * The processor then checks if float and/or string vectors are specified as processor parameters called [name].F and
 * [name].S, respectively. The function of these parameters is specified in each individual module description. The
 * modules are created and initialised in init.
 *
 ...
 * If specified, training is run in end. Training can be run with the previously generated root file only, i.e. without
 * extraction. If specified, inference is run in processEvent. It needs extraction. Training and inference cannot be
 * done simultaneously.
 *
 * The training creates a reference file in which the used observables and the used signal and background PDGs are
 * stored, as well as the location of the model's weight files together with the corresponding momentum bin. The
 * inference reads the reference file and loads the TrainingModel based on the information therein.
 *
 * For each observable in each InputAlgorithm one branch is created in the TTree called
 * [algorithm_name]_[observable_name]. In addition, the processor also collects for each PFO: momentum, lambda angle
 * (polar angle relative to z=0), the PDG of the likeliest connected MCParticle, the number of associated tracks as well
 * as d0 and z0 of the first track (if any). The training observables can be specified. If not done so, the defaults are
 * all observables associated with the specified InputAlgorithms + momentum and lambda.
 *
 ...
 * The PFOs are divided into momentum bins, for each of which a separate TrainingModel is created and trained/inferred
 * from. If inference is specified, confusion matrix plots of all signal PDGs can be created in the specified folder
 * during end.
 *
 ...
 *    bool, default: false.
 * @param _modeTrain - Set true to train via the specified TrainingModels. Cannot be true at the same time as
 * _modeInfer. bool, default: false.
 * @param _modeInfer - Set true to infer PID from the specified TrainingModels. Cannot be true at the same time as
 * _modeTrain. bool, default: false.
 *
 * @param _TTreeFileName - Name of the root file in which the TTree with all observables is stored; in case of
 * extraction it is an optional output with no output if left empty, otherwise it is a necessary input. string, default:
 * "TTreeFile.root".
 ...
 *    string vector, default: {}.
 * @param _reffile - Reference file(s). If only one file but several training models are specified the reference files
 * are auto-numbered. string vector, default: {"Ref.txt"}.
 * @param _trainingObservables - List of observables that should be used for traning. If empty, all observables from the
 * specified algorithms + momabs + lambda are used. string vector, default: {}.
 *
 ...
 *    int vector, default: {}.
 *
 * @param _plotFolder - Folder in which the automatic confusion matrix plots of inference will be put, is created if not
 * already existing; if empty, no plots are created. string, default: "CPID_Plots".
 ...
 *    int, default: 12.
 *
 * @param _cutD0 - PFOs whose first track have a d0 larger than the given value will be ignored; set to 0 to accept all
 * particles. float, default: 0.
 * @param _cutZ0 - PFOs whose first track have a z0 larger than the given value will be ignored; set to 0 to accept all
 * particles. float, default: 0.
 * @param _cutLamMin - PFOs whose first track have an angle lambda (relative to the cathode) smaller than the given
 * value will be ignored; set to 0 to accept all particles. float, default: 0.
 * @param _cutLamMax - PFOs whose first track have an angle lambda (relative to the cathode) larger than the given value
 * will be ignored; set to 0 to accept all particles. float, default: 0.
...
public:
  ...
  // standard creator and reader of training reference file; created after TrainingModel::initTrain, read before
  // TrainingModel::initInfer
