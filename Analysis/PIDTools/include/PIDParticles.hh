...
public:
  ...
  PIDParticle_base(const PIDParticle_base& base) : pdg(base.pdg), mass(base.mass), _name(base.Name()) {
    for (int i = 0; i < 5; i++)
      _BBpars[i] = base.GetBBpars()[i];
  }
...
public:
  LLPIDHypothesis(const char* name, int _pdg, double _mass, float _prior, const double* BBpars)
      : PIDParticle_base(name, _pdg, _mass, BBpars), prior(_prior), _posterior(0), _logL(0), _threshold(0) {}

  LLPIDHypothesis(const PIDParticle_base& base, float _prior)
      : PIDParticle_base(base), prior(_prior), _posterior(0), _logL(0), _threshold(0) {}
...
public:
  ...
  MVAPIDHypothesis(const char* name, int _pdg, double _mass, const double* BBpars, const float mvaCut = 0.)
      : PIDParticle_base(name, _pdg, _mass, BBpars), _mva(0), _q(0), _sigAbove(0), _mvaCut(mvaCut),
        _reader(new TMVA::Reader("Silent")), _histoQ(NULL), _histoSig(NULL), _histoBkg(NULL) {}

  MVAPIDHypothesis(const PIDParticle_base& base, const float mvaCut = 0.)
      : PIDParticle_base(base), _mva(0), _q(0), _sigAbove(0), _mvaCut(mvaCut), _reader(new TMVA::Reader("Silent")),
  ...
  void AddMVAVariable(const TString& name, Float_t* ptr) { _reader->AddVariable(name, ptr); };
  TMVA::IMethod* BookMVA(const TString& method, const TString& wfile) { return _reader->BookMVA(method, wfile); };

  void Evaluate(const TString& method);

  void SetHistoQ(const TH1F* histoQ) {
    _histoQ = (TH1F*)(histoQ->Clone("clonedQ"));
    _histoQ->SetDirectory(0);
  };
  void SetHistoSig(const TH1F* histoSig) {
    _histoSig = (TH1F*)(histoSig->Clone("clonedSig"));
    _histoSig->SetDirectory(0);
  };
  void SetHistoBkg(const TH1F* histoBkg) {
    _histoBkg = (TH1F*)(histoBkg->Clone("clonedBkg"));
    _histoBkg->SetDirectory(0);
  };
