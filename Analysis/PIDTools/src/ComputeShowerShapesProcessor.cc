...
ComputeShowerShapesProcessor::ComputeShowerShapesProcessor() : Processor("ComputeShowerShapesProcessor") {
  ...
  registerProcessorParameter("RadiationLength_Ecal", "RadiationLength of Absorbers", _X01, float(3.50));

  registerProcessorParameter("RadiationLength_Hcal", "RadiationLength of Absorbers", _X02, float(17.57));

  registerProcessorParameter("MoliereRadius_Ecal", "Moliere radius of Absorbers", _Rm1, float(9.00));
    ...
    for (unsigned int jClu = 0; jClu < part->getClusters().size(); jClu++) {
      ...
      for (unsigned int iHit = 0; iHit < nHitsInCluster; ++iHit) {
        ...
        case CHT::lcal:
          ...
          break;
        case CHT::lhcal:
          typ[iHit] = lhcal_Index;
          break;
        case CHT::bcal:
          typ[iHit] = bcal_Index;
          break;
        default:
          streamlog_out(DEBUG) << " no subdetector found for hit with type: " << pCalorimeterHit->getType()
                               << std::endl;
        }
      }
      ...
      const dd4hep::rec::LayeredCalorimeterData* eCalBarrelExtension = MarlinUtil::getLayeredCalorimeterData(
      ...
      const dd4hep::rec::LayeredCalorimeterData* eCalEndcapExtension = MarlinUtil::getLayeredCalorimeterData(
      ...
      float maxed = a * c * gsl_sf_gammainv(b) * pow(b - 1, b - 1) *
                    exp(-b + 1); // for advanced multiply with fabs(d) to avoid NaN
      float maxlength_pho =
          (1.0 * std::log(clusterEnergy / (X0[0] * 0.021 / Rm[0])) - 0.5); // this definition, +0.5 if gamma
      ...
      shapes.push_back(
          pClusterShapes->getRhitMean(xStart, index_xStart, X0, Rm)); // mean of the radius of the hits wrt cog
      shapes.push_back(
          pClusterShapes->getRhitRMS(xStart, index_xStart, X0, Rm)); // RMS of the radius of the hits wrt cog
