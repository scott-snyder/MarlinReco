...
std::vector<std::string> InputAlgorithm_dNdx::init(const std::vector<float>& inparF,
  ...
  sloM << "init parameters used: gas = " << _gas << ", scaling = " << _scaling << ", B field = " << _bField
       << " T, track inner radius = " << _innRad << " mm, track outer radius = " << _outRad
       << " mm, tracks max z = " << _zLen << " mm" << std::endl;

  std::vector<std::string> obsNames{"elDis", "muDis", "piDis", "kaDis", "prDis"};
  return obsNames;
}

std::vector<std::pair<float, float>> InputAlgorithm_dNdx::extractObservables(ReconstructedParticleImpl* pfo,
                                                                             LCCollection*, int mcpdg) {
  std::vector<std::pair<float, float>> obsValues{};
  std::vector<std::pair<float, float>> failDefault{};
  for (int i = 0; i < 5; ++i)
    failDefault.push_back(std::pair<float, float>{-1e+10, -1});
  ...
  if (tracks.size() > 0 && mass) {
    ...
    sloD << mass << " " << mom << " " << bg << " | " << phi0 << " " << phi1 << " " << len << " | " << mCl << " " << nCl
         << " " << dNdx << std::endl;
    ...
    for (int i = 0; i < 5; ++i) {
      ...
      if (nCl == 0 || dNdxRef == 0)
        dNdxValue.first = -1e+10;
      sloD << " " << dNdxValue.first;
      obsValues.push_back(dNdxValue);
    }
    sloD << std::endl;
  } else {
    sloD << " dN/dx fail: tracks.size() = " << tracks.size() << ", mass = " << mass << ", MCPDG = " << mcpdg
         << std::endl;
    return failDefault;
  }
...
float InputAlgorithm_dNdx::nClusters(float bg, int gas) {
  ...
  double nCl_He_Iso[n] = {42.94, 23.6,  18.97, 12.98, 12.2,  12.13, 12.24, 12.73, 13.03,
                          13.29, 13.63, 14.08, 15.56, 16.43, 16.8,  16.95, 16.98, 16.98};
  double nCl_He[n] = {11.79, 6.5, 5.23, 3.59, 3.38, 3.37, 3.4,  3.54, 3.63,
                      3.7,   3.8, 3.92, 4.33, 4.61, 4.78, 4.87, 4.89, 4.89};
  double nCl_Ar_Eth[n] = {130.04, 71.55, 57.56, 39.44, 37.08, 36.9,  37.25, 38.76, 39.68,
                          40.49,  41.53, 42.91, 46.8,  48.09, 48.59, 48.85, 48.93, 48.93};
  double nCl_Ar[n] = {88.69, 48.93, 39.41, 27.09, 25.51, 25.43, 25.69, 26.78, 27.44,
                      28.02, 28.77, 29.78, 32.67, 33.75, 34.24, 34.57, 34.68, 34.68};

  double nCl[n];
  switch (gas) {
  case 0:
    std::copy(nCl_He_Iso, nCl_He_Iso + n, nCl);
    break;
  case 1:
    std::copy(nCl_He, nCl_He + n, nCl);
    break;
  case 2:
    std::copy(nCl_Ar_Eth, nCl_Ar_Eth + n, nCl);
    break;
  case 3:
    std::copy(nCl_Ar, nCl_Ar + n, nCl);
    break;
  }
  ...
  if (success > -1e20) {
    if (abs(points[2]) > zmax && abs(points[5]) > zmax) {
      ...
      if (points[1] == 0)
        ...
        phi = atan(-points[0] / points[1]);
    } else if (abs(points[2]) < abs(points[5]))
      if (points[1] == 0)
        phi = points[0] < 0 ? M_PI * 0.5 : M_PI * 1.5;
      else
        phi = atan(-points[0] / points[1]);
    else if (points[4] == 0)
      phi = points[3] < 0 ? M_PI * 0.5 : M_PI * 1.5;
    else
      phi = atan(-points[3] / points[4]);
  }
  return phi;
}
