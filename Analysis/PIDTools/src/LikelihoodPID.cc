...
LikelihoodPID::LikelihoodPID(string fname, double* pars, std::vector<float> cost) {
  ...
  for (int j = 0; j < 17; j++) {
    if (pdf[2][j] != NULL && pdf[3][j] != NULL && pdf[4][j] != NULL) {
      ...
      for (int i = 2; i < 5; i++)
        _weights[i][j] = _weights[i][j] / denom;
    } else
      for (int i = 2; i < 5; i++)
        _weights[i][j] = 1.0 / 3.0; // same weights
  }
...
int LikelihoodPID::Classification(TLorentzVector pp, EVENT::Track* trk, EVENT::ClusterVec& cluvec) {
  ...
  // get dedx distance
  for (int i = 0; i < 5; i++)
    _dEdxDist[i] = get_dEdxChi2(i, pp.Vect(), trk->getdEdxError(), trk->getdEdx());
  ...
  } else if (_likelihood[0] == 0.0 && _likelihood[1] == 0.0 && _likelihood[2] == 0.0 && _likelihood[3] == 0.0 &&
             _likelihood[4] == 0.0) {
    for (int i = 0; i < 6; i++) {
      _likelihood[i] = 999.0;
      _posterior[i] = 0.0;
...
double* LikelihoodPID::GetPosterior() { return _posterior; }

double* LikelihoodPID::GetLikelihood() { return _likelihood; }
...
double LikelihoodPID::get_dEdxChi2(int parttype, TVector3 p, float hit, double dEdx) {
  ...
  if (dEdx_Norm - ExpdEdx < 0.0)
    chi2 = -chi2; // get signed chi2
...
double LikelihoodPID::get_dEdxDist(int parttype) {
  ...
  if (_dEdxDist[parttype] != 0.0)
    dedxdist = sqrt(fabs(_dEdxDist[parttype])) * _dEdxDist[parttype] / fabs(_dEdxDist[parttype]);
  if (dedxdist != dedxdist)
    dedxdist = 0.0;
  return dedxdist;
}

double LikelihoodPID::get_Norm(double dedx) { return dedx / _dEdxnorm; }
...
double LikelihoodPID::BetheBloch(double x, double mass, double* pars) {
  ...
  return (0.5 * pars[0] * TMath::Log(pars[1] * TMath::Power(bg, 2.0) * tmax) - pars[3] * b * b - pars[4] * bg / 2.0) /
         (b * b);
}
...
int LikelihoodPID::Class_electron(TLorentzVector pp, EVENT::Track* trk, EVENT::ClusterVec& cluvec) {
  ...
  for (int j = 0; j < 7; j++) { // variables
    ...
    for (int i = 0; i < 5; i++) { // particle type
      ...
      if (j < 6) {
        ...
        if (mucal == 0.0 && i == 1)
          okval[i] = getValue(5, valtype, var[j]); // likelihood
      } else if (j == 6)
    ...
    if (!_showerShapesFlg && (j >= 2 && j <= 5))
      continue;
    // for dEdx flg
    if (!_dEdxFlg && j == 6)
      continue;
    // don't use some dEdxin the case of LikelihoodPID
    if (_basicFlg && _showerShapesFlg && _dEdxFlg && !(j <= 6))
      continue;
    ...
    for (int i = 0; i < 5; i++) // particle type
      priorprob[i] = prior[i] / (prior[0] + prior[1] + prior[2] + prior[3] + prior[4]);

    // cal total probability for each particle type
    total = 0.0;
    for (int i = 0; i < 5; i++)
      total += priorprob[i] * okval[i];
    ...
    _likelihood[5] +=
        TMath::Log(_weights[2][valtype] * okval[2] + _weights[3][valtype] * okval[3] + _weights[4][valtype] * okval[4]);

    // bayesian updating
    for (int i = 0; i < 5; i++)
      prior[i] = TMath::Exp(posterior[i]);
  }
  ...
  for (int i = 0; i < 5; i++) { // particle type
    risk[i] = penalty[i][0] * TMath::Exp(posterior[0]) + penalty[i][1] * TMath::Exp(posterior[1]) +
              penalty[i][2] * TMath::Exp(posterior[2]) + penalty[i][3] * TMath::Exp(posterior[3]) +
              penalty[i][4] * TMath::Exp(posterior[4]);
  }
  ...
  if (_usebayes == 0) {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      if (fabs(tmppp - _likelihood[i]) < 1.0e-6)
        okflg = i;
    }
    //------------------------------------------------------------------
  } else {
    ...
    for (int i = 0; i < 5; i++) {
      if (fabs(tmppp - risk[i]) < 1.0e-8)
        okflg = i;
    }

    // threshold is set.
    if (okflg == 0 && posterior[okflg] < TMath::Log(threshold[okflg]))
      okflg = -1;
    //------------------------------------------------------------------
  }
...
int LikelihoodPID::Class_muon(TLorentzVector pp, EVENT::Track* trk, EVENT::ClusterVec& cluvec) {
  ...
  for (int j = 0; j < 13; j++) { // variables
    ...
    for (int i = 0; i < 5; i++) { // particle type(electron isn't be checked)
      ...
      if (j < 7) {
        ...
        if (var[2] == 0.0 && i == 1)
          okval[i] = getValue(5, valtype, var[j + 4]);      // likelihood
    ...
    if (!_basicFlg && j <= 2)
      continue;
    // for cluster shape flg
    if (!_showerShapesFlg && ((j > 2 && j <= 6) || j >= 8))
      continue;
    // for dEdx flg
    if (!_dEdxFlg && j == 7)
      continue;
    // don't use some dEdxin the case of LikelihoodPID
    if (_basicFlg && _showerShapesFlg && _dEdxFlg && !(j <= 9 || (j >= 10 && pp.P() > 0.3 && pp.P() < 6.0)))
      continue;
    if (mucal != 0.0 && j == 10)
      continue;
  ...
  if (_usebayes == 0) {
    ...
    if (okflg == 1 && posterior[okflg] < TMath::Log(threshold[okflg]))
      okflg = -1; // threshold is set.
    //------------------------------------------------------------------
  }
...
int LikelihoodPID::Class_hadron(TLorentzVector pp, EVENT::Track* trk, EVENT::ClusterVec& cluvec) {
  ...
  for (int j = 0; j < 6; j++) { // variables
    ...
    if (!_basicFlg && j < 2)
      continue;
    // for cluster shape flg
    if (!_showerShapesFlg && j >= 2 && j <= 4)
      continue;
    // for dEdx flg
    if (!_dEdxFlg && j >= 5)
      continue;
    // don't use some dEdxin the case of LikelihoodPID
    if (_basicFlg && _showerShapesFlg && _dEdxFlg && !(j <= 5))
      continue;
    ...
    for (int i = 0; i < 5; i++) { // particle type
      total[i] = priorprob[0] * okval[0] + priorprob[1] * okval[1] + priorprob[2] * okval[2] + priorprob[3] * okval[3] +
                 priorprob[4] * okval[4];
      if (total[i] == 0.0)
        total[i] = 1.0e-100;
    }
  ...
  if (_usebayes == 2) {
    ...
    if (fabs(pp.CosTheta()) < 1.0) {
      penalty[4][3] =
          max(1.0, penalty[4][3] * (0.439 * pp.CosTheta() * pp.CosTheta() - 0.940 * fabs(pp.CosTheta()) + 0.495));
  ...
  if (_usebayes == 0) {
    ...
    for (int i = 0; i < 5; i++) {
      if (fabs(tmppp - risk[i]) < 1.0e-6)
        okflg = i;
    }
    if (okflg < 2)
      okflg = 2;
    if (posterior[okflg] <= TMath::Log(0.21))
      okflg = TMath::Max(2, okflg); // do not go to leptons

    // check penalty -1 means undefined
    if (okflg >= 2 && posterior[okflg] < TMath::Log(threshold[okflg]))
      okflg = -1; // threshold is set.
    //------------------------------------------------------------------
  }
