...
RecoMCTruthLinker::RecoMCTruthLinker() : Processor("RecoMCTruthLinker") {
  ...
  registerInputCollection(LCIO::TRACK, "TrackCollection", "Name of the Tracks input collection", _trackCollectionName,
                          std::string("MarlinTrkTracks"));

  registerInputCollection(LCIO::CLUSTER, "ClusterCollection", "Name of the Clusters input collection",
                          _clusterCollectionName, std::string("PandoraClusters"));
  ...
  registerInputCollections(LCIO::SIMTRACKERHIT, "SimTrackerHitCollections",
                           "Names of the SimTrackerHits input collection", _simTrkHitCollectionNames,
                           trackerHitsInputColNamesDefault);

  registerProcessorParameter("UseTrackerHitRelations", "true: use relations for TrackerHits, false : use getRawHits ",
  ...
  registerInputCollections(LCIO::SIMCALORIMETERHIT, "SimCaloHitCollections",
                           "Names of the SimCaloHits input collections", _simCaloHitCollectionNames,
  ...
  registerInputCollections("LCRelation", "SimCalorimeterHitRelationNames",
                           ...
                           _caloHitRelationNames, caloHitsRelInputColNamesDefault);

  // <<<<<<

  //  >>>> Output true<->seen realations

  registerOutputCollection(LCIO::LCRELATION, "MCTruthTrackLinkName", "Name of the trackMCTruthLink output collection",
                           _mCTruthTrackLinkName, std::string(""));

  registerOutputCollection(LCIO::LCRELATION, "TrackMCTruthLinkName",
  ...
  registerOutputCollection(LCIO::LCRELATION, "MCTruthClusterLinkName",
                           "Name of the MCTruthClusterLink output collection", _mCTruthClusterLinkName,
                           std::string(""));
  ...
  registerOutputCollection(LCIO::LCRELATION, "RecoMCTruthLinkName",
                           "Name of the RecoMCTruthLink output collection - not created if empty()",
                           _recoMCTruthLinkName, std::string("RecoMCTruthLink"));

  registerOutputCollection(LCIO::LCRELATION, "MCTruthRecoLinkName", "Name of the MCTruthRecoLink output collection",
  ...
  registerProcessorParameter("daughtersECutMeV", "energy cut for daughters that are kept from KeepDaughtersPDG",
                             _eCutMeV, float(10.));

  registerProcessorParameter("SaveBremsstrahlungPhotons", "save photons from Brems", _saveBremsstrahlungPhotons,
                             bool(false));

  registerProcessorParameter("UsingParticleGun", "If Using Particle Gun Ignore Gen Stat", _using_particle_gun,
                             bool(false));

  registerProcessorParameter("BremsstrahlungEnergyCut", "energy cut for Brems that are kept", _bremsstrahlungEnergyCut,
                             float(1.));
...
void RecoMCTruthLinker::init() {
  ...
  if (!_use_tracker_hit_relations) {
    ...
    streamlog_out(WARNING)
        << "  ====== UseTrackerHitRelations=false => not using TrackerHit-SimTrackerHit-Relations  but getRawHits() "
           "instead - \n"
        << "         this is probably not what you want (only for backward compatibility with very old files...)"
        << std::endl;
  }
...
void RecoMCTruthLinker::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }

void RecoMCTruthLinker::processEvent(LCEvent* evt) {

  _nEvt++;

  streamlog_out(MESSAGE0) << " processEvent " << evt->getEventNumber() << "  - " << evt->getRunNumber() << std::endl;
  ...
  if (!haveTracks) {
    streamlog_out(DEBUG9) << " Track collection : " << _trackCollectionName << " not found - cannot create relation "
                          << std::endl;
...
void RecoMCTruthLinker::trackLinker(LCEvent* evt, LCCollection* /*mcpCol*/, LCCollection* trackCol,
  ...
  for (int i = 0; i < nTrack; ++i) {
    ...
    if (nSimHit == 0) {
      ...
      streamlog_out(WARNING) << " No simulated tracker hits found. Set UseTrackerHitRelations to true in steering file "
                                "to enable using TrackerHit relations if they are available."
                             << std::endl;
      ...
      for (TrackerHitVec::const_iterator hitIt = trkHits.begin(); hitIt != trkHits.end(); ++hitIt) {
        ...
        TrackerHit* hit = *hitIt; // ... and a seen hit ...
        streamlog_out(WARNING) << hit->getPosition()[0] << " " << hit->getPosition()[1] << " " << hit->getPosition()[2]
                               << " " << std::endl;
    ...
    for (MCPMap::iterator it = mcpMap.begin(); // iterate trough the map, map->first is the
      ...
      mother = (it->first->getParents().size() != 0 ? dynamic_cast<MCParticle*>(it->first->getParents()[0])
      ...
      if (_using_particle_gun || it->first->getGeneratorStatus() == 1) { // genstat 1 particle, ie. it is a bona fide
                                                                         // creating true particle: enter it into the
                                                                         // list, and note how many hits it produced.
    ...
    for (int iii = 0; iii < ifound; iii++) {
      ...
      streamlog_out(DEBUG4) << "    track " << trk->id() << " has " << MCPhits[iii] << " hits of " << nSimHit
                            << " SimHits (" << nHit << " TrackerHits) "
                            << " weight = " << weight << " , "
                            << " inv rel weight = " << inv_weight << "  "
                            << " of MCPart with pdg : " << theMCPs[iii]->getPDG() << " , SimHits "
                            << Total_SimHits_forMCP << " and genstat : " << theMCPs[iii]->getGeneratorStatus()
...
void RecoMCTruthLinker::clusterLinker(LCEvent* evt, LCCollection* mcpCol, LCCollection* clusterCol,
  ...
  streamlog_out(DEBUG6) << " *** Sorting out simHit<->MCParticle connections, and find corresponding calo hits "
                        << std::endl;
  ...
  for (unsigned i = 0, iN = _simCaloHitCollectionNames.size(); i < iN; ++i) {
    ...
    if (col) {
      ...
      for (int j = 0, jN = col->getNumberOfElements(); j < jN; ++j) {
        ...
        if (caloHits.size() != 1) {
          streamlog_out(DEBUG9) << " Sim hit with nore than one calo hit ? " << std::endl;
        }
        ...
        streamlog_out(DEBUG3) << "      ncalo hits : " << caloHits.size() << " calib factor " << calib_factor
                              << " position " << simHit->getPosition()[0] << " " << simHit->getPosition()[1] << " "
        ...
        for (int k = 0; k < simHit->getNMCContributions(); k++) {
          ...
          if (mcp == 0) {
            ...
            streamlog_out(DEBUG6) << " collection nb " << i << " nhits " << jN << " hit " << j << " contr " << k
                                  << std::endl;
            streamlog_out(DEBUG6) << " Sim hit with no mcp. N contrib is " << simHit->getNMCContributions()
                                  << std::endl;
            streamlog_out(DEBUG6) << "     Hit Position: " << simHit->getPosition()[0] << " "
                                  << simHit->getPosition()[1] << " " << simHit->getPosition()[2] << " " << std::endl;
          ...
          streamlog_out(DEBUG3) << "      initial true contributor " << k << " id " << mcp->id()
                                << " (with E = " << mcp->getEnergy() << " and pdg " << mcp->getPDG()
                                << " ) e hit: " << e << std::endl;
          ...
          if (remap_as_you_go.find(mcp) != remap_as_you_go.end()) {
            ...
            streamlog_out(DEBUG3) << "     Hit " << simHit->id() << " / " << caloHit->id() << " attributed to "
                                  << mcp->id()
                                  << " because it's origin mcp has already been treated : originator case 0 "
                                  << std::endl;
          } else {
            MCParticle* mother = 0;
            ...
            if (mcp->getGeneratorStatus() == 0 && (mcp->getParents().empty() == false)) {
              ...
              streamlog_out(DEBUG2) << "        simHit " << simHit->id() << "," << j
                                    << " not created by generator particle. backtracking ..." << std::endl;
              streamlog_out(DEBUG2) << "          " << mcp->id() << " gs " << mcp->getGeneratorStatus() << " dint "
                                    << mcp->isDecayedInTracker() << " bs " << mcp->isBackscatter() << " ndi "
                                    << mcp->vertexIsNotEndpointOfParent() << " npar " << mcp->getParents().size()
                                    << " pdg " << mcp->getPDG() << " " << mcp->getVertex()[0] << " "
                                    << mcp->getVertex()[1] << " " << mcp->getVertex()[2] << " " << mcp->getEndpoint()[0]
                                    << " " << mcp->getEndpoint()[1] << " " << mcp->getEndpoint()[2] << std::endl;

              mother = dynamic_cast<MCParticle*>(mcp->getParents()[0]);

              if (!this_Kid->isBackscatter() && mother != 0 && mother->getParents().size() > 0 &&
                  mother->getGeneratorStatus() == 0 && !mother->isDecayedInTracker()) {
              ...
              while (!this_Kid->isBackscatter() && mother != 0 && mother->getParents().size() > 0 &&
                ...
                if (this_Kid->vertexIsNotEndpointOfParent() != _invertedNonDestructiveInteractionLogic) {
                  ...
                  if (oma->isDecayedInTracker()) {
                    streamlog_out(DEBUG1)
                        << "          break out : gandmother " << oma->id() << " gs " << oma->getGeneratorStatus()
                        << " dint " << oma->isDecayedInTracker() << " bs " << oma->isBackscatter() << " ndi "
                        << oma->vertexIsNotEndpointOfParent() << " npar " << oma->getParents().size() << " pdg "
                        << oma->getPDG() << std::endl;
                    break;
                  }
                }
                ...
                streamlog_out(DEBUG1) << "          shower-part mother " << mother->id() << " gs "
                                      << mother->getGeneratorStatus() << " dint " << mother->isDecayedInTracker()
                                      << " bs " << mother->isBackscatter() << " ndi "
                                      << mother->vertexIsNotEndpointOfParent() << " npar "
                                      << mother->getParents().size() << " pdg " << mother->getPDG() << std::endl;
              }
              ...
              if (this_Kid->isBackscatter()) {
                ...
                mcp = this_Kid;
                streamlog_out(DEBUG3) << "     Hit " << simHit->id() << " / " << caloHit->id() << " attributed to kid "
                                      << mcp->id() << " because it's origin is a back-scatter : originator case 2 "
                                      << std::endl;
                streamlog_out(DEBUG2) << "          " << this_Kid->id() << " gs " << this_Kid->getGeneratorStatus()
                                      << " dint " << this_Kid->isDecayedInTracker() << " bs "
                                      << this_Kid->isBackscatter() << " npar " << this_Kid->getParents().size()
                ...
                streamlog_out(DEBUG3) << "     Hit " << simHit->id() << " / " << caloHit->id() << " attributed to kid "
                                      << mcp->id() << " because it's origin is in tracker : originator case 3 "
                                      << std::endl;
                ...
                if (mother == 0) { // ... which of course implies no grand-mother, and no gen stat
                  ...
                  streamlog_out(WARNING) << "  MCparticle " << this_Kid->id()
                                         << " is a simulation particle, created in the calorimeter by nothing . "
                                         << std::endl;
                  ...
                  mcp = this_Kid; // can't do better than that.
                         ...
                         // still possible that this_Kid came from a "non-destructive interaction" with the
                         // tracke-detector material. This we now try to figure out.
                  ...
                  if (this_Kid->vertexIsNotEndpointOfParent() == _invertedNonDestructiveInteractionLogic) {
                    ...
                    // it isn't: actually it is "false", but is "true" for the for particles that *do* originate at the
                    ...
                    streamlog_out(DEBUG3)
                        << "     Hit " << simHit->id() << " / " << caloHit->id() << " attributed to mother "
                        << mcp->id() << " because it is a generator particle or started in tracker : originator case 4 "
                        << std::endl;

                  } else { // here we DO have a "non-destructive interaction". Unluckily, we cant directly know if this
                           // took place in the tracker (in which case we should keep this_Kid as the mcp to save and
                           // assign hits to), or not. We will play a few clean tricks to find the cases where it either
                           // certain that the "non-destructive interation" was in the tracker, or that it was in the
                           // calo. This reduces the number of uncertain cases to play dirty tricks with.
                    ...
                    for (unsigned kkk = 0; kkk < mother->getDaughters().size(); kkk++) {
                      ...
                      if (abs(sister->getVertex()[0] - this_Kid->getVertex()[0]) > 0.1 ||
                          ...
                          abs(sister->getVertex()[2] - this_Kid->getVertex()[2]) > 0.1)
                    ...
                    streamlog_out(DEBUG1) << "          " << starts_in_tracker << " " << has_pi0 << " " << has_bs << " "
                    ...
                    if (starts_in_tracker == 1) { // this_Kid is a clear-cut hit-originator
                      ...
                      streamlog_out(DEBUG3)
                          << "     Hit " << simHit->id() << " / " << caloHit->id() << " attributed to kid " << mcp->id()
                          << " because it's origin could be deduced to be in tracker : originator case 5 " << std::endl;

                      streamlog_out(DEBUG2) << "        Details of case 5: " << this_Kid->getVertex()[0] << " "
                                            << this_Kid->getVertex()[1] << " " << this_Kid->getVertex()[2] << " "
                                            << mother->getEndpoint()[0] << " " << mother->getEndpoint()[1] << " "
                                            << mother->getEndpoint()[2] << " " << mother->getGeneratorStatus() << " "
                                            << this_Kid->vertexIsNotEndpointOfParent() << std::endl;
                      streamlog_out(DEBUG1) << " starts in tracker " << std::endl;

                    } else if (has_pi0 != 0 || has_bs != 0 ||
                               oma_in_calo != 0) { // clear-cut case of this_Kid starting in the calo.
                      ...
                      streamlog_out(DEBUG3)
                          << "     Hit " << simHit->id() << " / " << caloHit->id() << " attributed to mother "
                          << mcp->id() << " because it's origin could be deduced to be in tracker : originator case 6 "
                          << std::endl;
                      streamlog_out(DEBUG2)
                          << "        Case 6 details: kid starts in calo " << " " << this_Kid->getVertex()[1] << " "
                          ...
                          << oma_in_calo << std::endl;
                             ...
                             // also in the calo. Calos are dense, so typically in the case the "non-destructive
                             // interaction" is in the calo, one would guess  that the distance is small, ie. large
                             // distance -> unlikely that it was in the calo.
                      ...
                      if (rdist > 200.) { // guess "non-destructive interaction" not in calo -> this_Kid is originator
                        ...
                        mcp = this_Kid; // this_Kid started before the calo, and is the one
                                        // the hit should be attributed to
                        streamlog_out(DEBUG3)
                            << "     Hit " << simHit->id() << " / " << caloHit->id() << " attributed to kid "
                            << mcp->id() << " because it's origin is guessed to be in tracker : originator case 7 "
                            << std::endl;
                        streamlog_out(DEBUG2)
                            << "        Case 7 details: guess kid starts in tracker " << " " << this_Kid->getVertex()[1]
                            << " " << this_Kid->getVertex()[2] << " " << rdist << " " << has_pi0 << " " << std::endl;
                        ...
                        streamlog_out(DEBUG3)
                            ...
                            << std::endl;
                        streamlog_out(DEBUG2)
                            << "        Case 8 details:  guess kid starts in calo " << " " << this_Kid->getVertex()[1]
                            << " " << this_Kid->getVertex()[2] << " " << rdist << " " << has_pi0 << " " << std::endl;
                      }
                    }
                  }
              ...
              streamlog_out(DEBUG3) << "     Hit " << simHit->id() << " / " << caloHit->id() << " attributed to "
                                    << mcp->id() << " because it's origin is a generator particle : originator case 1 "
                                    << std::endl;
          ...
          streamlog_out(DEBUG4) << "      gs " << mcp->getGeneratorStatus() << " dint " << mcp->isDecayedInTracker()
                                << " bs " << mcp->isBackscatter() << " npar " << mcp->getParents().size() << " pdg "
                                << mcp->getPDG() << " " << mcp->getEndpoint()[0] << " " << mcp->getEndpoint()[1] << " "
          ...
          for (size_t jk = 0; jk < caloHits.size(); jk++) {
            chitTruthRelNav.addRelation(caloHits[jk], mcp,
  ...
  streamlog_out(DEBUG6)
      << " *** Sorting out Cluster<->MCParticle using simHit<->MCParticle, re-assigning the latter in some rare cases."
      << std::endl;
  ...
  for (int i = 0; i < nCluster; i++) {
    ...
    streamlog_out(DEBUG5) << "  ================= " << std::endl;
    streamlog_out(DEBUG5) << "Cluster clu = " << clu->id() << " (i = " << i << " ) with " << cluHits.size() << " hits "
                          << std::endl;

    double ecalohitsum = 0.;
    double ecalohitsum_unknown = 0.;
    int no_sim_hit = 0;
    for (CalorimeterHitVec::const_iterator hitIt = cluHits.begin(); hitIt != cluHits.end(); ++hitIt) {
      ...
      streamlog_out(DEBUG4) << std::endl;
      streamlog_out(DEBUG4) << "     Treating hit = " << hit->id() << " e " << hit->getEnergy()
      ...
      for (LCObjectVec::const_iterator objIt = simHits.begin(); objIt != simHits.end(); ++objIt) {
        ...
        double calib_factor =
            hit->getEnergy() /
            simHit->getEnergy(); // In this place, this defn of calib_factor is still appropriate for split hits (DJ)
        ...
        for (int j = 0; j < simHit->getNMCContributions(); j++) {
          ...
          if (it_mcp == remap_as_you_go.end()) {
            streamlog_out(DEBUG7) << "      simhit = " << simHit << "; its true contributor " << mcp->id()
                                  << " is not found in the map" << std::endl;
            continue;
          }

          double e = simHit->getEnergyCont(j) * calib_factor;
          streamlog_out(DEBUG3) << "     true contributor = " << mcp->id() << " e: " << e << " mapped to "
                                << it_mcp->second->id() << std::endl;
          mcp = it_mcp->second;
      ...
      if (nsimhit == 0) {
        ...
        streamlog_out(DEBUG5) << " Warning: no simhits for calohit " << hit << ". Will have to guess true particle ... "
                              << std::endl;
        no_sim_hit = 1;
        ecalohitsum_unknown += hit->getEnergy();
        streamlog_out(DEBUG5) << " sim-less calohit E and position " << hit->getEnergy() << " " << hit->getPosition()[0]
                              << " " << hit->getPosition()[1] << " " << hit->getPosition()[2] << " " << std::endl;
        streamlog_out(DEBUG5) << " sim-less calohit clust E and position " << clu->getEnergy() << " "
                              << clu->getPosition()[0] << " " << clu->getPosition()[1] << " " << clu->getPosition()[2]
                              << " " << std::endl;
      }
    } // hit loop
    if (no_sim_hit == 1) {
    ...
    streamlog_out(DEBUG5) << std::endl;
    streamlog_out(DEBUG5) << "    Sum of calohit E: " << ecalohitsum << " cluster E " << clu->getEnergy()
                          << " Sum of Simcalohit E: " << eTot << "  no_sim_hit: " << no_sim_hit
                          << ". Energy from unknow source : " << ecalohitsum_unknown << std::endl;
    ...
    for (MCPMapDouble::iterator it = mcpEnergy.begin(); // iterate trough the map.
         it != mcpEnergy.end(); ++it) {
      if (it->first ==
          0) { // ( if == 0, this cluster contains some (but not all) sim-hits with unknown origin.
               //   If *all* sim-hits would have had unknown origin, we would already have "continue":ed above)
        streamlog_out(MESSAGE) << " SimHit with unknown origin in cluster " << clu << " ( " << clu->id() << " ) "
                               << std::endl;
        continue;
      }
      ...
      if (it->first->getGeneratorStatus() == 1) { // genstat 1 particle, ie. it is a bona fide
        ...
        if (mother != 0) {
          if (mother->isDecayedInTracker() && // ... so the partic apeared in the
            ...
            streamlog_out(DEBUG2) << " case 1 for " << it->first->id() << "(morefound=" << morefound << ")"
                                  << " mother: " << mother->id() << " gs " << mother->getGeneratorStatus() << " dint "
                                  << mother->isDecayedInTracker() << " bs " << mother->isBackscatter() << " pdg "
          ...
          streamlog_out(DEBUG6) << " case 2 for " << it->first->id() << "(morefound=" << morefound << ")" << std::endl;
          moreMCPs.push_back(it->first);
          moreMCPes.push_back(it->second);
          morefound++;
        }
    ...
    if (morefound > 0) {
      for (int iii = 0; iii < morefound; iii++) {
        streamlog_out(DEBUG2) << " iii, moreMCPes[iii], moreMCPs[iii], gs " << iii << " " << moreMCPes[iii] << " "
                              << moreMCPs[iii]->id() << " " << moreMCPs[iii]->getGeneratorStatus() << std::endl;
      }
      for (int iii = 0; iii < ifound; iii++) {
        streamlog_out(DEBUG2) << " iii, MCPes[iii], theMCPs[iii] " << iii << " " << MCPes[iii] << " "
    ...
    for (int iii = 0; iii < morefound; iii++) {
      ...
      streamlog_out(DEBUG2) << "      partic vert: " << moreMCPs[iii]->getVertex()[0] << " "
                            << moreMCPs[iii]->getVertex()[1] << " " << moreMCPs[iii]->getVertex()[2] << std::endl;
      streamlog_out(DEBUG2) << "      partic endpoint: " << moreMCPs[iii]->getEndpoint()[0] << " "
                            << moreMCPs[iii]->getEndpoint()[1] << " " << moreMCPs[iii]->getEndpoint()[2] << std::endl;
      streamlog_out(DEBUG2) << "      partic status : gs " << moreMCPs[iii]->getGeneratorStatus() << " dint "
                            << moreMCPs[iii]->isDecayedInTracker() << " bs " << moreMCPs[iii]->isBackscatter()
                            << " npar " << moreMCPs[iii]->getParents().size() << " pdg " << moreMCPs[iii]->getPDG()
                            << " dinc " << moreMCPs[iii]->isDecayedInCalorimeter() << " bye "
                            << moreMCPs[iii]->hasLeftDetector() << " stop " << moreMCPs[iii]->isStopped() << std::endl;

      while (mother != 0 && mother->getGeneratorStatus() != 2) { // back-track to the
                                                                 // beginning of the chain

        streamlog_out(DEBUG2) << "       mother " << mother->id() << " gs " << mother->getGeneratorStatus() << " dint "
                              << mother->isDecayedInTracker() << " " << " bs " << mother->isBackscatter() << " pdg "
                              << mother->getPDG() << " dinc " << mother->isDecayedInCalorimeter() << std::endl;
        streamlog_out(DEBUG2) << "       mother vert: " << mother->getVertex()[0] << " " << mother->getVertex()[1]
                              << " " << mother->getVertex()[2] << std::endl;
        streamlog_out(DEBUG2) << "       mother endpoint: " << mother->getEndpoint()[0] << " "
                              << mother->getEndpoint()[1] << " " << mother->getEndpoint()[2] << std::endl;
        ...
        for (int kkk = 0; kkk < ifound; kkk++) {
          ...
          if (tmcp == mother) {
            ...
            streamlog_out(DEBUG3) << "        found " << moreMCPs[iii] << "(iii= " << iii << ")" << kkk
                                  << " to be related to " << mother->id() << " add e : " << moreMCPes[iii] << std::endl;
    ...
    streamlog_out(DEBUG5) << " cluster " << clu->id() << " , E = " << clu->getEnergy() << " , ifound = " << ifound
                          << std::endl;
    ...
    for (int iii = 0; iii < ifound; iii++) {
      ...
      streamlog_out(DEBUG5) << " cluster " << clu->id() << " has " << MCPes[iii] << " of " << eTot << "  [ " << weight
                            << " ] "
                            << " of MCParticle " << theMCPs[iii]->id() << " with pdg : " << theMCPs[iii]->getPDG()
                            << " and genstat : " << theMCPs[iii]->getGeneratorStatus() << std::endl;
      ...
      if (theMCPs[iii]->getGeneratorStatus() == 0 && (theMCPs[iii]->getParents().empty() == false)) {
        streamlog_out(DEBUG5) << " mother id " << theMCPs[iii]->getParents()[0]->id() << " and genstat "
  ...
  streamlog_out(DEBUG6) << " *** Guessing Cluster<->MCParticle in cases where MCParticle<->simHit is broken (LCAL/DBD)"
                        << std::endl;
  ...
  for (unsigned i = 0; i < missingMC.size(); ++i) {
    ...
    gear::Vector3D recP(clu->getPosition()[0], clu->getPosition()[1], clu->getPosition()[2]);
    ...
    if (maxProd > 0.) {
      ...
      streamlog_out(DEBUG5) << "  neutral cluster particle recovered" << clu->getEnergy() << " maxProd: " << maxProd
                            << " px: " << closestMCP->getMomentum()[0] << " py: " << closestMCP->getMomentum()[1]
...
void RecoMCTruthLinker::particleLinker(LCCollection* mcpCol, LCCollection* particleCol, LCCollection* ttrlcol,
  ...
  for (int i = 0; i < nPart; ++i) {
    ...
    streamlog_out(DEBUG3) << "      Treating particle " << part->id() << " with index " << i << " it has " << ntrk
                          << " tracks, and " << nclu << " clusters " << std::endl;
    ...
    if (ntrk > 1) {
      ...
      for (int j = 0; j < ntrk; j++) {
        ...
        streamlog_out(DEBUG2) << "         Track " << tracks[j]->id() << " with index " << j << " has " << nhit[j]
                              << " hits " << std::endl;
      }
    ...
    for (int j = 0; j < ntrk; j++) {
      ...
      if (tracks[j] != 0) {
        ...
        int ntp = mcvec.size();
        streamlog_out(DEBUG3) << "      Track " << tracks[j]->id() << " with index " << j << " has " << ntp
        ...
        if (ntp > 0) {
          if (mcvec[0] != 0) {
            ...
            for (int kkk = 0; kkk < ntp; kkk++) {
              ...
              streamlog_out(DEBUG2) << "        Individual track weight to " << mcvec[kkk] << " is " << www[kkk]
                                    << ", scaled one is " << www[kkk] * (float(nhit[j]) / float(nhitT))
    ...
    if (_FullRecoRelation || ntrk == 0) {
      ...
      if (nclu > 1) {
        ...
        for (int j = 0; j < nclu; j++) {
          ...
          streamlog_out(DEBUG2) << "        Cluster " << clusters[j]->id() << " with index " << j << " has energy "
                                << eclu[j] << std::endl;
        }
      ...
      for (int j = 0; j < nclu; j++) {
        if (clusters[j] != 0) {
          ...
          int ntp = mcvec.size();
          streamlog_out(DEBUG3) << "    Cluster " << clusters[j]->id() << " with index " << j << " has " << ntp
                                << " true particles " << std::endl;
          if (ntp > 0) {
            if (mcvec[0] != 0) {
              for (int kkk = 0; kkk < ntp; kkk++) {
                ...
                mcmap[dynamic_cast<MCParticle*>(mcvec[kkk])] += int(www[kkk] * 1000. * (eclu[j] / ecluT) + 0.5) * 10000;
                streamlog_out(DEBUG2) << "        Individual cluster Weight to " << mcvec[kkk] << " is " << www[kkk]
                                      << ", scaled one is " << www[kkk] * (eclu[j] / ecluT)
                                      << " ( loop -index : " << kkk << ")" << std::endl;
              }
            }
          }
    ...
    if (_FullRecoRelation) {
      for (std::map<MCParticle*, int>::iterator mcit = mcmap.begin(); mcit != mcmap.end(); mcit++) {
        ...
        streamlog_out(DEBUG4) << "     particle " << part->id() << " has weight " << mcit->second
                              << " (Track: " << int(mcit->second) % 10000 << " , Cluster: " << int(mcit->second) / 10000
                              << " ) "
                              << " of MCParticle with pdg : " << mcit->first->getPDG()
                              << " and genstat : " << mcit->first->getGeneratorStatus() << " id: " << mcit->first->id()
                              << std::endl;
      ...
      if (mcmax != NULL) {
        ...
        streamlog_out(DEBUG3) << "      particle " << part->id() << " has weight " << maxwgt
                              ...
                              << " and genstat : " << mcmax->getGeneratorStatus() << " id: " << mcmax->id()
                              << ". Particle charge and ntracks : " << part->getCharge() << " " << ntrk << std::endl;
      } else {
        streamlog_out(WARNING) << " particle has weight " << maxwgt
                               << ". Particle charge and ntracks : " << part->getCharge() << " " << ntrk
                               << " but no mcparticle found " << std::endl;
      }
    }
  }
  // The reverse map. Differs in what the weight means: here it means:
  ...
  for (int i = 0; i < nMCP; i++) {
    ...
    for (unsigned j = 0; j < partvec.size(); j++) {
      ...
      streamlog_out(DEBUG4) << "    True Particle " << mcp->id() << " ( pdg " << mcp->getPDG() << " ) has weight "
                            << c_wgt << " / " << t_wgt << "( " << int(wgt) << " ) to particle " << msp->id()
                            << "  with " << cluvec_p.size() << " clusters, and " << trkvec_p.size() << " tracks "
                            << std::endl;
    }
  }
...
void RecoMCTruthLinker::makeSkim(LCCollection* mcpCol, LCCollection* ttrlcol, LCCollection* ctrlcol,
  ...
  for (int i = 0; i < nMCP; i++) {
    ...
    if (!mcp->isCreatedInSimulation() || mcp->getGeneratorStatus() != 0) {
      ...
      if (trackObjects.size() > 0 || clusterObjects.size() > 0) {
        ...
        streamlog_out(DEBUG5) << " keep MCParticle - e :" << mcp->getEnergy() << " charge: " << mcp->getCharge()
                              << " px: " << mcp->getMomentum()[0] << " py: " << mcp->getMomentum()[1]
                              << " pz: " << mcp->getMomentum()[2] << " mass: " << mcp->getMass() << " pdg "
                              << mcp->getPDG() << std::endl;
  ...
  for (int i = 0; i < nMCP; i++) {
    ...
    if (mcp->ext<MCPKeep>() == true && mcp->isDecayedInTracker()) { //&& !mcp->isStopped()  ){
      ...
      if (_pdgSet.find(thePDG) != _pdgSet.end()) {
        ...
        for (MCParticleVec::const_iterator dIt = daughters.begin(); dIt != daughters.end(); ++dIt) {
...
void RecoMCTruthLinker::keepMCParticle(MCParticle* mcp) {
  ...
  streamlog_out(DEBUG3) << " keepMCParticle keep particle with pdg : " << mcp->getPDG() << std::endl;
...
void RecoMCTruthLinker::check(LCEvent* evt) {
  ...
  if (isFirstEvent()) {
    ...
    hmcp_e = AIDAProcessor::histogramFactory(this)->createCloud1D("hmcp_e", " energy/GeV - all ", 100);
    //       createHistogram1D( "hmcp_e", " energy/GeV ", 100, 0. , 10. ) ;

    hmcp_etot = AIDAProcessor::histogramFactory(this)->createCloud1D("hmcp_etot", " total energy/GeV ", 100);
    //       createHistogram1D( "hmcp_etot_e", " energy/GeV ", 1000, 0. , 1000. ) ;

    hmcp_n = AIDAProcessor::histogramFactory(this)->createCloud1D("hmcp_n", " # generated stable particles ", 100);

    hmcp_ntot = AIDAProcessor::histogramFactory(this)->createCloud1D("hmcp_ntot", "  total # particles ", 100);

    hmcpsk_e = AIDAProcessor::histogramFactory(this)->createCloud1D("hmcpsk_e", " energy/GeV - all ", 100);
    //       createHistogram1D( "hmcpsk_e", " energy/GeV ", 100, 0. , 10. ) ;

    hmcpsk_etot = AIDAProcessor::histogramFactory(this)->createCloud1D("hmcpsk_etot", " total energy/GeV ", 100);
    //       createHistogram1D( "hmcpsk_etot_e", " energy/GeV ", 1000, 0. , 1000. ) ;

    hmcpsk_n = AIDAProcessor::histogramFactory(this)->createCloud1D("hmcpsk_n", " # generated stable particles ", 100);

    hmcpsk_ntot = AIDAProcessor::histogramFactory(this)->createCloud1D("hmcpsk_ntot", "  total # particles ", 100);
  }
  ...
  try {
    ...
    streamlog_out(DEBUG9) << "RecoMCTructh::Check(): MCParticle collection \"" << _mcParticleCollectionName
  ...
  try {
    ...
    streamlog_out(DEBUG9) << "RecoMCTructh::Check(): MCParticleSkimmed collection \"" << _mcParticlesSkimmedName
                          << "\" does not exist, skipping" << std::endl;
  }
...
const LCObjectVec* RecoMCTruthLinker::getSimHits(TrackerHit* trkhit, const FloatVec* weights) {
  ...
  if (obj->empty() == false) {
    ...
    streamlog_out(WARNING) << "getSimHits :  TrackerHit : " << trkhit
                           << " has no sim hits related. CellID0 = " << trkhit->getCellID0()
                           << " pos = " << trkhit->getPosition()[0] << " " << trkhit->getPosition()[1] << " "
...
void RecoMCTruthLinker::end() {
  ...
  streamlog_out(DEBUG6) << " processed " << _nEvt << " events in " << _nRun << " runs " << std::endl;
}
...
void RecoMCTruthLinker::mergeTrackerHitRelations(LCEvent* evt) {
  ...
  for (unsigned i = 0; i < nCol; ++i) {
    ...
    if (col != 0) {
      ...
      streamlog_out(DEBUG2) << " mergeTrackerHitRelations: input collection missing : "
...
void RecoMCTruthLinker::mergeCaloHitRelations(LCEvent* evt) {
  ...
  for (unsigned i = 0; i < nCol; ++i) {
    ...
    if (col != 0) {
      ...
      streamlog_out(DEBUG2) << " mergeCaloHitRelations: input collection missing : " << _caloHitRelationNames[i]
                            << std::endl;
    }
  }
...
const LCObjectVec* RecoMCTruthLinker::getCaloHits(CalorimeterHit* calohit, const FloatVec* weights) {
  ...
  if (obj->empty() == false) {
    ...
    streamlog_out(DEBUG5) << "getCaloHits :  CalorimeterHit : " << calohit
                          << " has no sim hits related. CellID0 = " << calohit->getCellID0()
                          << " pos = " << calohit->getPosition()[0] << " " << calohit->getPosition()[1] << " "
                          << calohit->getPosition()[2] << std::endl;
  }
  return obj;
}

void RecoMCTruthLinker::linkPrinter(LCCollection* mcpCol, LCCollection* particleCol, LCCollection* ptrlcol,
  ...
  for (int i = 0; i < nPart; ++i) {
    ...
    int ntp = mcvec.size();
    streamlog_out(DEBUG5) << "    Particle " << part->id() << " (q: " << int(part->getCharge()) << " ) has " << ntp
                          << " true particles. E= " << part->getEnergy() << " " << clu_e;
    ...
    if (ntp > 0) {
      if (mcvec[0] != 0) {
        ...
        for (int kkk = 0; kkk < ntp; kkk++) {
          ...
          streamlog_out(DEBUG3) << "       " << kkk << " " << mcp->id() << " " << mcp->getPDG() << " "
        ...
        if (part->getCharge() != 0) {
          streamlog_out(DEBUG5) << "     True E_ch " << true_charged_E << " total E_calo from true "
                                << total_ecalo_in_this_true << " E_calo from neutral " << total_ecalo_neutral_in_charged
                                << " track weight " << total_trk_weight << " cluster weight " << total_clu_weight
                                << std::endl;
        } else {
  ...
  for (int i = 0; i < nMCPart; ++i) {
    ...
    if (nsp > 0) {
      streamlog_out(DEBUG5) << "    TrueParticle " << mcpart->id() << " (q: " << int(mcpart->getCharge()) << " ) has "
                            << nsp << " seen particles. E= " << mcpart->getEnergy() << " pt= "
      ...
      if (partvec[0] != 0) {
        ...
        for (int kkk = 0; kkk < nsp; kkk++) {
          ...
          streamlog_out(DEBUG3) << "       " << kkk << " " << recopart->id() << " " << recopart->getEnergy()
                                << std::endl;
        }
        streamlog_out(DEBUG5) << "    Total track weight " << total_trk_weight << " , Total cluster weight "
