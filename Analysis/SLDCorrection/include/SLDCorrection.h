...
public:
  ...
  //	hasDownStreamSLDecay checks whether a B-/C-Hadron that decays semi-leptonically, has another semi-leptonic decay
  // in its decay products or not, true: has another semi-leptonic decay, false: does not have any semi-leptonic decay
  // in its decay products
  bool hasDownStreamSLDecay(const MCP& parentHadron);

  //	hasUpStreamSLDecay checks  whether a B-/C-Hadron that decays semi-leptonically, has another semi-leptonic decay
  // in its up-stream, true has a semi-leptonic decay in up-stream, false, does not have a semi-leptonic decay in
  // up-stream
  ...
  //	getNeutrinoFourMomentumModified corrects neutrino energy using rapidity-based approach (checks P_vis_nor,
  // P_vis_par, flightDirection, etc to avoid math divergence)
  TLorentzVector getNeutrinoFourMomentumModified(TVector3& flightDirection, const TLorentzVector& visibleFourMomentum,
                                                 const double& parentHadronMass, const float& solutionSign);

  //	getNeutrinoFourMomentumStandardMethod corrects neutrino energy satndard (p,E) conservation method (highly
  // affected by numeric precision!)
  TLorentzVector getNeutrinoFourMomentumStandardMethod(const TVector3& flightDirection,
  ...
  //	addNeutrinoCovarianceMatrix Calculates Covariance matrix for each solution of neutrinos, based and energy and
  // angular error
  virtual void addNeutrinoCovarianceMatrix(const TLorentzVector& neutrinoFourMomentum, std::vector<float>& NuCovMat);

  virtual void evaluateInputCovMat(const TLorentzVector& trueVisibleFourMomentum, const TVector3& trueFlightDirection,
                                   const TLorentzVector& trueNeutrinoFourMomentum,
                                   const TLorentzVector& visibleFourMomentum, const TVector3& flightDirection,
                                   const TLorentzVector& recoNeutrinoFourMomentum, const FloatVector& CovMatDetector,
                                   const FloatVector& CovMatNeutrino);

  virtual void plotHistograms(const TLorentzVector& trueFourMomentumNeutrino, const TLorentzVector& FourMomentumNuClose,
                              const FloatVector& NeutrinoCovMat);

  virtual void InitializeHistogram(TH1F* histogram, int scale, int color, int lineWidth, int markerSize,
                                   int markerStyle);
...
private:
  ...
  IntVector m_SLDLeptonID{}; //+11/-11: SLDecay to electron/positron, +13/-13: SLDecay to muon/anti-muon, +15/-15:
                             // SLDecay to tau/anti-tau
