...
SLDCorrection::SLDCorrection()
  ...
  registerInputCollection(LCIO::MCPARTICLE, "MCParticleCollection", "Name of the MCParticle collection",
                          m_mcParticleCollection, std::string("MCParticle"));

  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "PfoCollection", "Name of input pfo collection",
                          m_inputPfoCollection, std::string("PandoraPFOs"));

  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "JetCollection", "Name of input jet collection",
                          m_inputJetCollection, std::string("Durham_nJets"));

  registerInputCollection(LCIO::VERTEX, "PrimaryVertex", "Name of Primary Vertex Collection", m_inputPrimaryVertex,
                          std::string("PrimaryVertex"));

  registerInputCollection(LCIO::VERTEX, "BuildUpVertex", "Name of BuildUp Vertex Collection", m_inputBuildUpVertex,
                          std::string("BuildUpVertex"));

  registerInputCollection(LCIO::LCRELATION, "RecoMCTruthLinkCollection", "Name of input RecoMCTruthLink Collection",
                          m_RecoMCTruthLinkCollection, std::string("RecoMCTruthLinkCollection"));

  registerInputCollection(LCIO::LCRELATION, "MCTruthRecoLinkCollection", "Name of input MCTruthRecoLink Collection",
                          m_MCTruthRecoLinkCollection, std::string("MCTruthRecoLinkCollection"));

  registerInputCollection(LCIO::LCRELATION, "ClusterMCTruthLinkCollection",
                          "Name of input m_ClusterMCTruthLink Collection", m_ClusterMCTruthLinkCollection,
                          std::string("ClusterMCTruthLink"));

  registerInputCollection(LCIO::LCRELATION, "MCTruthClusterLinkCollection",
                          "Name of input MCTruthClusterLink Collection", m_MCTruthClusterLinkCollection,
                          std::string("MCTruthClusterLink"));

  registerOutputCollection(LCIO::VERTEX, "SemiLeptonicDecayVertex", "Name of Semi-Leptonic Decay Vertices Collection",
                           m_SLDVertex, std::string("SemiLeptonicDecayVertex"));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "SemiLeptonicDecayVertexRP",
                           "Name of Semi-Leptonic Decay Vertices Reconstructed Particle Collection", m_SLDVertexRP,
                           std::string("SemiLeptonicDecayVertexRP"));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "ReconstructedNeutrino",
                           "Name of Reconstructed Neutrino Collection", m_reconstructedNeutrino,
                           std::string("ReconstructedNeutrino"));

  registerOutputCollection(LCIO::LCRELATION, "JetSLDLinkName",
                           "Name of the JetSemiLeptonicDecayLinkName output collection", m_JetSLDLinkName,
                           std::string("JetSLDLinkName"));

  registerOutputCollection(LCIO::LCRELATION, "SLDJetLinkName",
                           "Name of the SemiLeptonicDecayJetLinkName output collection", m_SLDJetLinkName,
                           std::string("SLDJetLinkName"));

  registerOutputCollection(LCIO::LCRELATION, "mcNurecoNuLinkName",
                           "Name of the trueNeutrino-reconstructedNeutrino output Link collection",
                           m_mcNurecoNuLinkName, std::string("mcNurecoNuLinkName"));

  registerOutputCollection(LCIO::LCRELATION, "recoNumcNuLinkName",
                           "Name of the trueNeutrino-reconstructedNeutrino output Link collection",
                           m_recoNumcNuLinkName, std::string("recoNumcNuLinkName"));

  registerOutputCollection(LCIO::LCRELATION, "SLDNuLinkName",
                           "Name of the NeutrinoSemiLeptonicDecayLinkName output collection", m_SLDNuLinkName,
                           std::string("SLDNuLinkName"));

  registerOutputCollection(LCIO::LCRELATION, "NuSLDLinkName",
                           "Name of the SemiLeptonicDecayNeutrinoLinkName output collection", m_NuSLDLinkName,
                           std::string("NuSLDLinkName"));

  registerProcessorParameter("includeBSLD", "do correction for semi-leptonic decays of B-Hadrons", m_includeBSLD,
                             bool(true));

  registerProcessorParameter("includeCSLD", "do correction for semi-leptonic decays of C-Hadrons", m_includeCSLD,
                             bool(true));

  registerProcessorParameter("includeTSLD", "do correction for semi-leptonic decays of Tau-Leptons", m_includeTSLD,
                             bool(true));

  registerProcessorParameter("cheatSLDLeptons", "Cheat semi-leptonic decays lepton from MCTruth", m_cheatSLDLeptons,
                             bool(true));

  registerProcessorParameter("cheatFlightDirection", "Cheat Flight direction of mother hadron", m_cheatFlightDirection,
                             bool(true));

  registerProcessorParameter("vertexingScenario",
                             "Scenario for finding flight direction of mother hadron: 1 = default , 2 = assign jet "
                             "axis , 3 = assign flight direction of leading particle in the jet",
                             m_vertexingScenario, int(1));

  registerProcessorParameter("cheatLepton4momentum", "Cheat FourMomentum of lepton in semi-leptonic decays",
                             m_cheatLepton4momentum, bool(true));

  registerProcessorParameter("cheatCharged4momentum", "Cheat FourMomentum of charged visibles in semi-leptonic decays",
                             m_cheatCharged4momentum, bool(true));

  registerProcessorParameter("cheatNeutral4momentum", "Cheat FourMomentum of neutral visibles in semi-leptonic decays",
                             m_cheatNeutral4momentum, bool(true));

  registerProcessorParameter("cheatPVAcharged", "Cheat Particle ID for Charged Decay Products", m_cheatPVAcharged,
                             bool(true));

  registerProcessorParameter(
      "chargedCosAcceptanceAngleSLD4",
      "Acceptance angle for charged PFOs to be assigned to the semi-leptonic decay when lepton is in secondary vertex",
      m_chargedCosAcceptanceAngleSLD4, float(0.0));

  registerProcessorParameter(
      "chargedCosAcceptanceAngleSLD5",
      "Acceptance angle for charged PFOs to be assigned to the semi-leptonic decay when lepton is with third vertex",
      m_chargedCosAcceptanceAngleSLD5, float(0.0));

  registerProcessorParameter("cheatPVAneutral", "Cheat Particle ID for Neutral Decay Products", m_cheatPVAneutral,
                             bool(true));

  registerProcessorParameter("cheatSolutionSign", "Cheat the signe of solution for the neutrino correction",
                             m_cheatSolutionSign, bool(false));
  ...
  registerProcessorParameter("neutralCosAcceptanceAngle",
                             ...
                             m_neutralCosAcceptanceAngle, float(0.0));

  registerProcessorParameter("BSLDChargedSLD4InvMassCut",
                             "Cut on Invariant mass of visible charged decay products for semi-leptonic decay of "
                             "B-Hadron when lepton is in secondary vertex",
                             m_BSLDChargedSLD4InvMassCut, float(0.0));

  registerProcessorParameter("BSLDChargedSLD5InvMassCut",
                             "Cut on Invariant mass of visible charged decay products for semi-leptonic decay of "
                             "B-Hadron when lepton is with third vertex",
                             m_BSLDChargedSLD5InvMassCut, float(0.0));

  registerProcessorParameter("BSLDNeutralSLD4InvMassCut",
                             "Cut on Invariant mass of visible neutral decay products for semi-leptonic decay of "
                             "B-Hadron when lepton is in secondary vertex",
                             m_BSLDNeutralSLD4InvMassCut, float(0.0));

  registerProcessorParameter("BSLDNeutralSLD5InvMassCut",
                             "Cut on Invariant mass of visible neutral decay products for semi-leptonic decay of "
                             "B-Hadron when lepton is with third vertex",
                             m_BSLDNeutralSLD5InvMassCut, float(0.0));

  registerProcessorParameter("CSLDChargedSLD4InvMassCut",
                             "Cut on Invariant mass of visible charged decay products for semi-leptonic decay of "
                             "C-Hadron when lepton is in secondary vertex",
                             m_CSLDChargedSLD4InvMassCut, float(0.0));

  registerProcessorParameter("CSLDChargedSLD5InvMassCut",
                             "Cut on Invariant mass of visible charged decay products for semi-leptonic decay of "
                             "C-Hadron when lepton is with third vertex",
                             m_CSLDChargedSLD5InvMassCut, float(0.0));

  registerProcessorParameter("CSLDNeutralSLD4InvMassCut",
                             "Cut on Invariant mass of visible neutral decay products for semi-leptonic decay of "
                             "C-Hadron when lepton is in secondary vertex",
                             m_CSLDNeutralSLD4InvMassCut, float(0.0));

  registerProcessorParameter("CSLDNeutralSLD5InvMassCut",
                             "Cut on Invariant mass of visible neutral decay products for semi-leptonic decay of "
                             "C-Hadron when lepton is with third vertex",
                             m_CSLDNeutralSLD5InvMassCut, float(0.0));
  ...
  registerProcessorParameter("nIterFlightDirCorrection",
                             ...
                             m_nIterFlightDirCorrection, int(1));

  registerProcessorParameter("BSLD4SigmaAlpha",
                             "Angular uncertainty (radian) due to flight direction reconstruction for semi-leptonic "
                             "decay of B-Hadron when leptin is in 2nd vertex",
                             m_BSLD4SigmaAlpha, float(0.004));

  registerProcessorParameter("BSLD5SigmaAlpha",
                             "Angular uncertainty (radian) due to flight direction reconstruction for semi-leptonic "
                             "decay of B-Hadron when leptin is with 3rd vertex",
                             m_BSLD5SigmaAlpha, float(0.010));

  registerProcessorParameter("BSLD4SigmaECPVA",
                             "Energy uncertainty (GeV) due to Charged Particle to Vertex Association (CPVA) for "
                             "semi-leptonic decay of B-Hadron when leptin is in 2nd vertex",
                             m_BSLD4SigmaECPVA, float(0.0));

  registerProcessorParameter("BSLD4SigmaENPVA",
                             "Energy uncertainty (GeV) due to Neutral Particle to Vertex Association (NPVA) for "
                             "semi-leptonic decay of B-Hadron when leptin is in 2nd vertex",
                             m_BSLD4SigmaENPVA, float(0.0));

  registerProcessorParameter("BSLD4SigmaAlphaCPVA",
                             "Angular uncertainty (radian) due to Charged Particle to Vertex Association (NPVA) for "
                             "semi-leptonic decay of B-Hadron when leptin is in 2nd vertex",
                             m_BSLD4SigmaAlphaCPVA, float(0.0));

  registerProcessorParameter("BSLD4SigmaAlphaNPVA",
                             "Angular uncertainty (radian) due to Neutral Particle to Vertex Association (NPVA) for "
                             "semi-leptonic decay of B-Hadron when leptin is in 2nd vertex",
                             m_BSLD4SigmaAlphaNPVA, float(0.0));

  registerProcessorParameter("BSLD5SigmaECPVA",
                             "Energy uncertainty (GeV) due to Charged Particle to Vertex Association (CPVA) for "
                             "semi-leptonic decay of B-Hadron when leptin is with 3rd vertex",
                             m_BSLD5SigmaECPVA, float(0.0));

  registerProcessorParameter("BSLD5SigmaENPVA",
                             "Energy uncertainty (GeV) due to Neutral Particle to Vertex Association (NPVA) for "
                             "semi-leptonic decay of B-Hadron when leptin is with 3rd vertex",
                             m_BSLD5SigmaENPVA, float(0.0));

  registerProcessorParameter("BSLD5SigmaAlphaCPVA",
                             "Angular uncertainty (radian) due to Charged Particle to Vertex Association (NPVA) for "
                             "semi-leptonic decay of B-Hadron when leptin is with 3rd vertex",
                             m_BSLD5SigmaAlphaCPVA, float(0.0));

  registerProcessorParameter("BSLD5SigmaAlphaNPVA",
                             "Angular uncertainty (radian) due to Neutral Particle to Vertex Association (NPVA) for "
                             "semi-leptonic decay of B-Hadron when leptin is with 3rd vertex",
                             m_BSLD5SigmaAlphaNPVA, float(0.0));

  registerProcessorParameter("sigmaENu", "Energy uncertainty (GeV) for reconstructed neutrino", m_sigmaENu, float(4.0));

  registerProcessorParameter("sigmaAlphaNu", "Angular uncertainty (radian) for reconstructed neutrino direction",
                             m_sigmaAlphaNu, float(0.1));

  registerProcessorParameter(
      "displayEvent",
      "Display recoLepton, downstraem vertex and RP and reconstructed secondary vertex in Event Display",
      m_displayEvent, bool(true));

  registerProcessorParameter("fillRootTree", "Fill root tree to check processor performance", m_fillRootTree,
                             bool(true));
  ...
  registerProcessorParameter("CsldMode", "Event selection based on semi-leptonic decays of C-hadrons", m_CSLDMode,
                             int(0));

  registerProcessorParameter("TsldMode", "Event selection based on semi-leptonic decays of tau-leptons", m_TSLDMode,
                             int(0));

  registerProcessorParameter("sldMode", "Event selection based on semi-leptonic decays of All hadrons and tau-leptons",
                             m_SLDMode, int(0));
}
...
void SLDCorrection::init() {
  ...
  if (!m_cheatPVAcharged)
    m_cheatCharged4momentum = false;
  if (!m_cheatPVAneutral)
    m_cheatNeutral4momentum = false;
  printParameters();
  ...
  if (m_fillRootTree) {
    ...
    m_pTTree1->Branch("visibleChargedInvMassCut", &m_visibleChargedInvMassCut);
    m_pTTree1->Branch("visibleNeutralInvMassCut", &m_visibleNeutralInvMassCut);
    m_pTTree1->Branch("nChargedParticlesInPrimVertex", &m_nChargedParticlesInPrimVertex);
    m_pTTree1->Branch("nChargedParticlesNotInPrimVertex", &m_nChargedParticlesNotInPrimVertex);
    m_pTTree1->Branch("trueVisibleFourMomentumAtSLDVertex_Px", &m_trueVisibleFourMomentumAtSLDVertex_Px);
    m_pTTree1->Branch("trueVisibleFourMomentumAtSLDVertex_Py", &m_trueVisibleFourMomentumAtSLDVertex_Py);
    m_pTTree1->Branch("trueVisibleFourMomentumAtSLDVertex_Pz", &m_trueVisibleFourMomentumAtSLDVertex_Pz);
    m_pTTree1->Branch("trueVisibleFourMomentumAtSLDVertex_E", &m_trueVisibleFourMomentumAtSLDVertex_E);
    m_pTTree1->Branch("trueVisibleFourMomentumAtSLDVertex_M", &m_trueVisibleFourMomentumAtSLDVertex_M);
    m_pTTree1->Branch("truePVATrueFourMomentum_Px", &m_truePVATrueFourMomentum_Px);
    m_pTTree1->Branch("truePVATrueFourMomentum_Py", &m_truePVATrueFourMomentum_Py);
    m_pTTree1->Branch("truePVATrueFourMomentum_Pz", &m_truePVATrueFourMomentum_Pz);
    m_pTTree1->Branch("truePVATrueFourMomentum_E", &m_truePVATrueFourMomentum_E);
    m_pTTree1->Branch("truePVATrueFourMomentum_M", &m_truePVATrueFourMomentum_M);
    m_pTTree1->Branch("truePVARecoFourMomentum_Px", &m_truePVARecoFourMomentum_Px);
    m_pTTree1->Branch("truePVARecoFourMomentum_Py", &m_truePVARecoFourMomentum_Py);
    m_pTTree1->Branch("truePVARecoFourMomentum_Pz", &m_truePVARecoFourMomentum_Pz);
    m_pTTree1->Branch("truePVARecoFourMomentum_E", &m_truePVARecoFourMomentum_E);
    m_pTTree1->Branch("truePVARecoFourMomentum_M", &m_truePVARecoFourMomentum_M);
    m_pTTree1->Branch("recoPVARecoFourMomentum_Px", &m_recoPVARecoFourMomentum_Px);
    m_pTTree1->Branch("recoPVARecoFourMomentum_Py", &m_recoPVARecoFourMomentum_Py);
    m_pTTree1->Branch("recoPVARecoFourMomentum_Pz", &m_recoPVARecoFourMomentum_Pz);
    m_pTTree1->Branch("recoPVARecoFourMomentum_E", &m_recoPVARecoFourMomentum_E);
    m_pTTree1->Branch("recoPVARecoFourMomentum_M", &m_recoPVARecoFourMomentum_M);
    m_pTTree1->Branch("usedVisibleFourMomentum_Px", &m_usedVisibleFourMomentum_Px);
    m_pTTree1->Branch("usedVisibleFourMomentum_Py", &m_usedVisibleFourMomentum_Py);
    m_pTTree1->Branch("usedVisibleFourMomentum_Pz", &m_usedVisibleFourMomentum_Pz);
    m_pTTree1->Branch("usedVisibleFourMomentum_E", &m_usedVisibleFourMomentum_E);
    m_pTTree1->Branch("usedVisibleFourMomentum_M", &m_usedVisibleFourMomentum_M);
    m_pTTree1->Branch("trueLeptonFourMomentum_Px", &m_trueLeptonFourMomentum_Px);
    m_pTTree1->Branch("trueLeptonFourMomentum_Py", &m_trueLeptonFourMomentum_Py);
    m_pTTree1->Branch("trueLeptonFourMomentum_Pz", &m_trueLeptonFourMomentum_Pz);
    m_pTTree1->Branch("trueLeptonFourMomentum_E", &m_trueLeptonFourMomentum_E);
    m_pTTree1->Branch("trueLeptonFourMomentum_M", &m_trueLeptonFourMomentum_M);
    m_pTTree1->Branch("recoLeptonFourMomentum_Px", &m_recoLeptonFourMomentum_Px);
    m_pTTree1->Branch("recoLeptonFourMomentum_Py", &m_recoLeptonFourMomentum_Py);
    m_pTTree1->Branch("recoLeptonFourMomentum_Pz", &m_recoLeptonFourMomentum_Pz);
    m_pTTree1->Branch("recoLeptonFourMomentum_E", &m_recoLeptonFourMomentum_E);
    m_pTTree1->Branch("recoLeptonFourMomentum_M", &m_recoLeptonFourMomentum_M);
    m_pTTree1->Branch("usedLeptonFourMomentum_Px", &m_usedLeptonFourMomentum_Px);
    m_pTTree1->Branch("usedLeptonFourMomentum_Py", &m_usedLeptonFourMomentum_Py);
    m_pTTree1->Branch("usedLeptonFourMomentum_Pz", &m_usedLeptonFourMomentum_Pz);
    m_pTTree1->Branch("usedLeptonFourMomentum_E", &m_usedLeptonFourMomentum_E);
    m_pTTree1->Branch("usedLeptonFourMomentum_M", &m_usedLeptonFourMomentum_M);
    m_pTTree1->Branch("truePVATrueChargedFourMomentum_Px", &m_truePVATrueChargedFourMomentum_Px);
    m_pTTree1->Branch("truePVATrueChargedFourMomentum_Py", &m_truePVATrueChargedFourMomentum_Py);
    m_pTTree1->Branch("truePVATrueChargedFourMomentum_Pz", &m_truePVATrueChargedFourMomentum_Pz);
    m_pTTree1->Branch("truePVATrueChargedFourMomentum_E", &m_truePVATrueChargedFourMomentum_E);
    m_pTTree1->Branch("truePVATrueChargedFourMomentum_M", &m_truePVATrueChargedFourMomentum_M);
    m_pTTree1->Branch("truePVARecoChargedFourMomentum_Px", &m_truePVARecoChargedFourMomentum_Px);
    m_pTTree1->Branch("truePVARecoChargedFourMomentum_Py", &m_truePVARecoChargedFourMomentum_Py);
    m_pTTree1->Branch("truePVARecoChargedFourMomentum_Pz", &m_truePVARecoChargedFourMomentum_Pz);
    m_pTTree1->Branch("truePVARecoChargedFourMomentum_E", &m_truePVARecoChargedFourMomentum_E);
    m_pTTree1->Branch("truePVARecoChargedFourMomentum_M", &m_truePVARecoChargedFourMomentum_M);
    m_pTTree1->Branch("recoSLDVertexChargedFourMomentum_Px", &m_recoSLDVertexChargedFourMomentum_Px);
    m_pTTree1->Branch("recoSLDVertexChargedFourMomentum_Py", &m_recoSLDVertexChargedFourMomentum_Py);
    m_pTTree1->Branch("recoSLDVertexChargedFourMomentum_Pz", &m_recoSLDVertexChargedFourMomentum_Pz);
    m_pTTree1->Branch("recoSLDVertexChargedFourMomentum_E", &m_recoSLDVertexChargedFourMomentum_E);
    m_pTTree1->Branch("recoSLDVertexChargedFourMomentum_M", &m_recoSLDVertexChargedFourMomentum_M);
    m_pTTree1->Branch("recoPVARecoChargedFourMomentum_Px", &m_recoPVARecoChargedFourMomentum_Px);
    m_pTTree1->Branch("recoPVARecoChargedFourMomentum_Py", &m_recoPVARecoChargedFourMomentum_Py);
    m_pTTree1->Branch("recoPVARecoChargedFourMomentum_Pz", &m_recoPVARecoChargedFourMomentum_Pz);
    m_pTTree1->Branch("recoPVARecoChargedFourMomentum_E", &m_recoPVARecoChargedFourMomentum_E);
    m_pTTree1->Branch("recoPVARecoChargedFourMomentum_M", &m_recoPVARecoChargedFourMomentum_M);
    m_pTTree1->Branch("usedChargedFourMomentum_Px", &m_usedChargedFourMomentum_Px);
    m_pTTree1->Branch("usedChargedFourMomentum_Py", &m_usedChargedFourMomentum_Py);
    m_pTTree1->Branch("usedChargedFourMomentum_Pz", &m_usedChargedFourMomentum_Pz);
    m_pTTree1->Branch("usedChargedFourMomentum_E", &m_usedChargedFourMomentum_E);
    m_pTTree1->Branch("usedChargedFourMomentum_M", &m_usedChargedFourMomentum_M);
    m_pTTree1->Branch("truePVATrueNeutralFourMomentum_Px", &m_truePVATrueNeutralFourMomentum_Px);
    m_pTTree1->Branch("truePVATrueNeutralFourMomentum_Py", &m_truePVATrueNeutralFourMomentum_Py);
    m_pTTree1->Branch("truePVATrueNeutralFourMomentum_Pz", &m_truePVATrueNeutralFourMomentum_Pz);
    m_pTTree1->Branch("truePVATrueNeutralFourMomentum_E", &m_truePVATrueNeutralFourMomentum_E);
    m_pTTree1->Branch("truePVATrueNeutralFourMomentum_M", &m_truePVATrueNeutralFourMomentum_M);
    m_pTTree1->Branch("truePVARecoNeutralFourMomentum_Px", &m_truePVARecoNeutralFourMomentum_Px);
    m_pTTree1->Branch("truePVARecoNeutralFourMomentum_Py", &m_truePVARecoNeutralFourMomentum_Py);
    m_pTTree1->Branch("truePVARecoNeutralFourMomentum_Pz", &m_truePVARecoNeutralFourMomentum_Pz);
    m_pTTree1->Branch("truePVARecoNeutralFourMomentum_E", &m_truePVARecoNeutralFourMomentum_E);
    m_pTTree1->Branch("truePVARecoNeutralFourMomentum_M", &m_truePVARecoNeutralFourMomentum_M);
    m_pTTree1->Branch("recoPVARecoNeutralFourMomentum_Px", &m_recoPVARecoNeutralFourMomentum_Px);
    m_pTTree1->Branch("recoPVARecoNeutralFourMomentum_Py", &m_recoPVARecoNeutralFourMomentum_Py);
    m_pTTree1->Branch("recoPVARecoNeutralFourMomentum_Pz", &m_recoPVARecoNeutralFourMomentum_Pz);
    m_pTTree1->Branch("recoPVARecoNeutralFourMomentum_E", &m_recoPVARecoNeutralFourMomentum_E);
    m_pTTree1->Branch("recoPVARecoNeutralFourMomentum_M", &m_recoPVARecoNeutralFourMomentum_M);
    m_pTTree1->Branch("usedNeutralFourMomentum_Px", &m_usedNeutralFourMomentum_Px);
    m_pTTree1->Branch("usedNeutralFourMomentum_Py", &m_usedNeutralFourMomentum_Py);
    m_pTTree1->Branch("usedNeutralFourMomentum_Pz", &m_usedNeutralFourMomentum_Pz);
    m_pTTree1->Branch("usedNeutralFourMomentum_E", &m_usedNeutralFourMomentum_E);
    m_pTTree1->Branch("usedNeutralFourMomentum_M", &m_usedNeutralFourMomentum_M);
    m_pTTree1->Branch("recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum_Px",
                      &m_recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum_Px);
    m_pTTree1->Branch("recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum_Py",
                      &m_recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum_Py);
    m_pTTree1->Branch("recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum_Pz",
                      &m_recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum_Pz);
    m_pTTree1->Branch("recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum_E",
                      &m_recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum_E);
    m_pTTree1->Branch("recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum_M",
                      &m_recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum_M);
    m_pTTree1->Branch("recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum_Px",
                      &m_recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum_Px);
    m_pTTree1->Branch("recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum_Py",
                      &m_recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum_Py);
    m_pTTree1->Branch("recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum_Pz",
                      &m_recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum_Pz);
    m_pTTree1->Branch("recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum_E",
                      &m_recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum_E);
    m_pTTree1->Branch("recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum_M",
                      &m_recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum_M);
    m_pTTree1->Branch("trueNeutrinoFourMomentum_Px", &m_trueNeutrinoFourMomentum_Px);
    m_pTTree1->Branch("trueNeutrinoFourMomentum_Py", &m_trueNeutrinoFourMomentum_Py);
    m_pTTree1->Branch("trueNeutrinoFourMomentum_Pz", &m_trueNeutrinoFourMomentum_Pz);
    m_pTTree1->Branch("trueNeutrinoFourMomentum_E", &m_trueNeutrinoFourMomentum_E);
    m_pTTree1->Branch("trueNeutrinoFourMomentum_M", &m_trueNeutrinoFourMomentum_M);
    m_pTTree1->Branch("recoNeutrinoFourMomentumClose_Px", &m_recoNeutrinoFourMomentumClose_Px);
    m_pTTree1->Branch("recoNeutrinoFourMomentumClose_Py", &m_recoNeutrinoFourMomentumClose_Py);
    m_pTTree1->Branch("recoNeutrinoFourMomentumClose_Pz", &m_recoNeutrinoFourMomentumClose_Pz);
    m_pTTree1->Branch("recoNeutrinoFourMomentumClose_E", &m_recoNeutrinoFourMomentumClose_E);
    m_pTTree1->Branch("recoNeutrinoFourMomentumClose_M", &m_recoNeutrinoFourMomentumClose_M);
    m_pTTree1->Branch("trueHadronFourMomentum_Px", &m_trueHadronFourMomentum_Px);
    m_pTTree1->Branch("trueHadronFourMomentum_Py", &m_trueHadronFourMomentum_Py);
    m_pTTree1->Branch("trueHadronFourMomentum_Pz", &m_trueHadronFourMomentum_Pz);
    m_pTTree1->Branch("trueHadronFourMomentum_E", &m_trueHadronFourMomentum_E);
    m_pTTree1->Branch("trueHadronFourMomentum_M", &m_trueHadronFourMomentum_M);
    m_pTTree1->Branch("recoHadronFourMomentum_Px", &m_recoHadronFourMomentum_Px);
    m_pTTree1->Branch("recoHadronFourMomentum_Py", &m_recoHadronFourMomentum_Py);
    m_pTTree1->Branch("recoHadronFourMomentum_Pz", &m_recoHadronFourMomentum_Pz);
    m_pTTree1->Branch("recoHadronFourMomentum_E", &m_recoHadronFourMomentum_E);
    m_pTTree1->Branch("recoHadronFourMomentum_M", &m_recoHadronFourMomentum_M);
    m_pTTree1->Branch("trueFlightDirection_X", &m_trueFlightDirection_X);
    m_pTTree1->Branch("trueFlightDirection_Y", &m_trueFlightDirection_Y);
    m_pTTree1->Branch("trueFlightDirection_Z", &m_trueFlightDirection_Z);
    m_pTTree1->Branch("recoFlightDirection_X", &m_recoFlightDirection_X);
    m_pTTree1->Branch("recoFlightDirection_Y", &m_recoFlightDirection_Y);
    m_pTTree1->Branch("recoFlightDirection_Z", &m_recoFlightDirection_Z);
    m_pTTree1->Branch("usedFlightDirection_X", &m_usedFlightDirection_X);
    m_pTTree1->Branch("usedFlightDirection_Y", &m_usedFlightDirection_Y);
    m_pTTree1->Branch("usedFlightDirection_Z", &m_usedFlightDirection_Z);
    m_pTTree1->Branch("neutralPVAAlpha", &m_NeutralPVAAlpha);
    m_pTTree1->Branch("neutralPVASinAlpha", &m_NeutralPVASinAlpha);
    m_pTTree1->Branch("neutralPVACosAlpha", &m_NeutralPVACosAlpha);
    m_pTTree1->Branch("chargedPVAAlpha", &m_ChargedPVAAlpha);
    m_pTTree1->Branch("chargedPVASinAlpha", &m_ChargedPVASinAlpha);
    m_pTTree1->Branch("chargedPVACosAlpha", &m_ChargedPVACosAlpha);
    m_pTTree1->Branch("PVAAlpha", &m_PVAAlpha);
    m_pTTree1->Branch("PVASinAlpha", &m_PVASinAlpha);
    m_pTTree1->Branch("PVACosAlpha", &m_PVACosAlpha);
    m_pTTree1->Branch("Alpha_RecoPVARecoAll_minus_TruePVARecoCharged_vs_recoPVArecoNeutral",
                      &m_Alpha_RecoPVARecoAll_minus_TruePVARecoCharged_vs_recoPVArecoNeutral);
    m_pTTree1->Branch("SinAlpha_RecoPVARecoAll_minus_TruePVARecoCharged_vs_recoPVArecoNeutral",
                      &m_SinAlpha_RecoPVARecoAll_minus_TruePVARecoCharged_vs_recoPVArecoNeutral);
    m_pTTree1->Branch("CosAlpha_RecoPVARecoAll_minus_TruePVARecoCharged_vs_recoPVArecoNeutral",
                      &m_CosAlpha_RecoPVARecoAll_minus_TruePVARecoCharged_vs_recoPVArecoNeutral);
    m_pTTree1->Branch("Alpha_RecoPVARecoAll_minus_TruePVARecoNeutral_vs_recoPVArecoCharged",
                      &m_Alpha_RecoPVARecoAll_minus_TruePVARecoNeutral_vs_recoPVArecoCharged);
    m_pTTree1->Branch("SinAlpha_RecoPVARecoAll_minus_TruePVARecoNeutral_vs_recoPVArecoCharged",
                      &m_SinAlpha_RecoPVARecoAll_minus_TruePVARecoNeutral_vs_recoPVArecoCharged);
    m_pTTree1->Branch("CosAlpha_RecoPVARecoAll_minus_TruePVARecoNeutral_vs_recoPVArecoCharged",
                      &m_CosAlpha_RecoPVARecoAll_minus_TruePVARecoNeutral_vs_recoPVArecoCharged);
    m_pTTree1->Branch("visiblePxBeforePVA", &m_visiblePxBeforePVA);
    m_pTTree1->Branch("visiblePyBeforePVA", &m_visiblePyBeforePVA);
    m_pTTree1->Branch("visiblePzBeforePVA", &m_visiblePzBeforePVA);
    m_pTTree1->Branch("visibleEnergyBeforePVA", &m_visibleEnergyBeforePVA);
    m_pTTree1->Branch("visibleMassBeforePVA", &m_visibleMassBeforePVA);
    m_pTTree1->Branch("visiblePxAfterVertexPVA", &m_visiblePxAfterVertexPVA);
    m_pTTree1->Branch("visiblePyAfterVertexPVA", &m_visiblePyAfterVertexPVA);
    m_pTTree1->Branch("visiblePzAfterVertexPVA", &m_visiblePzAfterVertexPVA);
    m_pTTree1->Branch("visibleEnergyAfterVertexPVA", &m_visibleEnergyAfterVertexPVA);
    m_pTTree1->Branch("visibleMassAfterVertexPVA", &m_visibleMassAfterVertexPVA);
    m_pTTree1->Branch("visiblePxAfterChargedPVA", &m_visiblePxAfterChargedPVA);
    m_pTTree1->Branch("visiblePyAfterChargedPVA", &m_visiblePyAfterChargedPVA);
    m_pTTree1->Branch("visiblePzAfterChargedPVA", &m_visiblePzAfterChargedPVA);
    m_pTTree1->Branch("visibleEnergyAfterChargedPVA", &m_visibleEnergyAfterChargedPVA);
    m_pTTree1->Branch("visibleMassAfterChargedPVA", &m_visibleMassAfterChargedPVA);
    m_pTTree1->Branch("visiblePxAfterNeutralPVA", &m_visiblePxAfterNeutralPVA);
    m_pTTree1->Branch("visiblePyAfterNeutralPVA", &m_visiblePyAfterNeutralPVA);
    m_pTTree1->Branch("visiblePzAfterNeutralPVA", &m_visiblePzAfterNeutralPVA);
    m_pTTree1->Branch("visibleEnergyAfterNeutralPVA", &m_visibleEnergyAfterNeutralPVA);
    m_pTTree1->Branch("visibleMassAfterNeutralPVA", &m_visibleMassAfterNeutralPVA);
    m_pTTree1->Branch("trueVisibleFourMomentumPx", &m_trueVisibleFourMomentumPx);
    m_pTTree1->Branch("trueVisibleFourMomentumPy", &m_trueVisibleFourMomentumPy);
    m_pTTree1->Branch("trueVisibleFourMomentumPz", &m_trueVisibleFourMomentumPz);
    m_pTTree1->Branch("trueVisibleFourMomentumE", &m_trueVisibleFourMomentumE);
    m_pTTree1->Branch("recoVisibleFourMomentumPx", &m_recoVisibleFourMomentumPx);
    m_pTTree1->Branch("recoVisibleFourMomentumPy", &m_recoVisibleFourMomentumPy);
    m_pTTree1->Branch("recoVisibleFourMomentumPz", &m_recoVisibleFourMomentumPz);
    m_pTTree1->Branch("recoVisibleFourMomentumE", &m_recoVisibleFourMomentumE);
    m_pTTree1->Branch("residualVisibleFourMomentumPx", &m_residualVisibleFourMomentumPx);
    m_pTTree1->Branch("residualVisibleFourMomentumPy", &m_residualVisibleFourMomentumPy);
    m_pTTree1->Branch("residualVisibleFourMomentumPz", &m_residualVisibleFourMomentumPz);
    m_pTTree1->Branch("residualVisibleFourMomentumE", &m_residualVisibleFourMomentumE);
    m_pTTree1->Branch("sigmaPxPx_Det", &m_sigmaPxPx_Det);
    m_pTTree1->Branch("sigmaPxPy_Det", &m_sigmaPxPy_Det);
    m_pTTree1->Branch("sigmaPyPy_Det", &m_sigmaPyPy_Det);
    m_pTTree1->Branch("sigmaPxPz_Det", &m_sigmaPxPz_Det);
    m_pTTree1->Branch("sigmaPyPz_Det", &m_sigmaPyPz_Det);
    m_pTTree1->Branch("sigmaPzPz_Det", &m_sigmaPzPz_Det);
    m_pTTree1->Branch("sigmaPxE_Det", &m_sigmaPxE_Det);
    m_pTTree1->Branch("sigmaPyE_Det", &m_sigmaPyE_Det);
    m_pTTree1->Branch("sigmaPzE_Det", &m_sigmaPzE_Det);
    m_pTTree1->Branch("sigmaEE_Det", &m_sigmaEE_Det);
    m_pTTree1->Branch("normalizedResidualVisibleFourMomentumPx", &m_normalizedResidualVisibleFourMomentumPx);
    m_pTTree1->Branch("normalizedResidualVisibleFourMomentumPy", &m_normalizedResidualVisibleFourMomentumPy);
    m_pTTree1->Branch("normalizedResidualVisibleFourMomentumPz", &m_normalizedResidualVisibleFourMomentumPz);
    m_pTTree1->Branch("normalizedResidualVisibleFourMomentumE", &m_normalizedResidualVisibleFourMomentumE);
    m_pTTree1->Branch("trueFlightDirectionUx", &m_trueFlightDirectionUx);
    m_pTTree1->Branch("trueFlightDirectionUy", &m_trueFlightDirectionUy);
    m_pTTree1->Branch("trueFlightDirectionUz", &m_trueFlightDirectionUz);
    m_pTTree1->Branch("recoFlightDirectionUx", &m_recoFlightDirectionUx);
    m_pTTree1->Branch("recoFlightDirectionUy", &m_recoFlightDirectionUy);
    m_pTTree1->Branch("recoFlightDirectionUz", &m_recoFlightDirectionUz);
    m_pTTree1->Branch("residualFlightDirectionUx", &m_residualFlightDirectionUx);
    m_pTTree1->Branch("residualFlightDirectionUy", &m_residualFlightDirectionUy);
    m_pTTree1->Branch("residualFlightDirectionUz", &m_residualFlightDirectionUz);
    m_pTTree1->Branch("trueNeutrinoFourMomentumPx", &m_trueNeutrinoFourMomentumPx);
    m_pTTree1->Branch("trueNeutrinoFourMomentumPy", &m_trueNeutrinoFourMomentumPy);
    m_pTTree1->Branch("trueNeutrinoFourMomentumPz", &m_trueNeutrinoFourMomentumPz);
    m_pTTree1->Branch("trueNeutrinoFourMomentumE", &m_trueNeutrinoFourMomentumE);
    m_pTTree1->Branch("recoNeutrinoFourMomentumClosePx", &m_recoNeutrinoFourMomentumClosePx);
    m_pTTree1->Branch("recoNeutrinoFourMomentumClosePy", &m_recoNeutrinoFourMomentumClosePy);
    m_pTTree1->Branch("recoNeutrinoFourMomentumClosePz", &m_recoNeutrinoFourMomentumClosePz);
    m_pTTree1->Branch("recoNeutrinoFourMomentumCloseE", &m_recoNeutrinoFourMomentumCloseE);
    m_pTTree1->Branch("sigmaNeutrinoPxPx", &m_sigmaNeutrinoPxPx);
    m_pTTree1->Branch("sigmaNeutrinoPxPy", &m_sigmaNeutrinoPxPy);
    m_pTTree1->Branch("sigmaNeutrinoPyPy", &m_sigmaNeutrinoPyPy);
    m_pTTree1->Branch("sigmaNeutrinoPxPz", &m_sigmaNeutrinoPxPz);
    m_pTTree1->Branch("sigmaNeutrinoPyPz", &m_sigmaNeutrinoPyPz);
    m_pTTree1->Branch("sigmaNeutrinoPzPz", &m_sigmaNeutrinoPzPz);
    m_pTTree1->Branch("sigmaNeutrinoPxE", &m_sigmaNeutrinoPxE);
    m_pTTree1->Branch("sigmaNeutrinoPyE", &m_sigmaNeutrinoPyE);
    m_pTTree1->Branch("sigmaNeutrinoPzE", &m_sigmaNeutrinoPzE);
    m_pTTree1->Branch("sigmaNeutrinoEE", &m_sigmaNeutrinoEE);
    m_pTTree1->Branch("residualNeutrinoFourMomentumPx", &m_residualNeutrinoFourMomentumPx);
    m_pTTree1->Branch("residualNeutrinoFourMomentumPy", &m_residualNeutrinoFourMomentumPy);
    m_pTTree1->Branch("residualNeutrinoFourMomentumPz", &m_residualNeutrinoFourMomentumPz);
    m_pTTree1->Branch("residualNeutrinoFourMomentumE", &m_residualNeutrinoFourMomentumE);
    m_pTTree1->Branch("normalizedResidualNeutrinoFourMomentumPx", &m_normalizedResidualNeutrinoFourMomentumPx);
    m_pTTree1->Branch("normalizedResidualNeutrinoFourMomentumPy", &m_normalizedResidualNeutrinoFourMomentumPy);
    m_pTTree1->Branch("normalizedResidualNeutrinoFourMomentumPz", &m_normalizedResidualNeutrinoFourMomentumPz);
    m_pTTree1->Branch("normalizedResidualNeutrinoFourMomentumE", &m_normalizedResidualNeutrinoFourMomentumE);
    m_pTTree1->Branch("recoNeutrinoDirectionError", &m_recoNeutrinoDirectionError);
    m_pTTree1->Branch("PCAatLeptonX", &m_PCAatLeptonX);
    m_pTTree1->Branch("PCAatLeptonY", &m_PCAatLeptonY);
    m_pTTree1->Branch("PCAatLeptonZ", &m_PCAatLeptonZ);
    m_pTTree1->Branch("PCAatOtherParticleX", &m_PCAatOtherParticleX);
    m_pTTree1->Branch("PCAatOtherParticleY", &m_PCAatOtherParticleY);
    m_pTTree1->Branch("PCAatOtherParticleZ", &m_PCAatOtherParticleZ);
    m_pTTree1->Branch("JetAxisX", &m_JetAxisX);
    m_pTTree1->Branch("JetAxisY", &m_JetAxisY);
    m_pTTree1->Branch("JetAxisZ", &m_JetAxisZ);
    m_pTTree1->Branch("PrimaryVertexX", &m_PrimaryVertexX);
    m_pTTree1->Branch("PrimaryVertexY", &m_PrimaryVertexY);
    m_pTTree1->Branch("PrimaryVertexZ", &m_PrimaryVertexZ);
    m_pTTree1->Branch("AngleTrueFlightDirectionJet", &m_AngleTrueFlightDirectionJet);
    m_pTTree1->Branch("AngleRecoFlightDirectionJet", &m_AngleRecoFlightDirectionJet);
    m_pTTree1->Branch("AngleLeptonJet", &m_AngleLeptonJet);
    m_pTTree1->Branch("AngleDSVertexJet", &m_AngleDSVertexJet);
    m_pTTree1->Branch("AngleRecoNeutrinoJet", &m_AngleRecoNeutrinoJet);
    m_pTTree1->Branch("AngleTrueNeutrinoJet", &m_AngleTrueNeutrinoJet);
    m_pTTree1->Branch("LeptonDistanceFromPV", &m_LeptonDistanceFromPV);
    ...
    h_NuPxResidual = new TH1F("PxResidual", "; _{}p_{x,#nu}^{REC} - p_{x,#nu}^{MC} [GeV]; Normalized #SLDs / 0.1 GeV",
                              200, -10.0, 10.0);
    h_NuPyResidual = new TH1F("PyResidual", "; _{}p_{y,#nu}^{REC} - p_{y,#nu}^{MC} [GeV]; Normalized #SLDs / 0.1 GeV",
                              200, -10.0, 10.0);
    h_NuPzResidual = new TH1F("PzResidual", "; _{}p_{z,#nu}^{REC} - p_{z,#nu}^{MC}  [GeV]; Normalized #SLDs / 0.1 GeV",
                              200, -10.0, 10.0);
    h_NuEResidual =
        new TH1F("EResidual", "; _{}E_{#nu}^{REC} - E_{#nu}^{MC} [GeV]; Normalized #SLDs / 0.1 GeV", 200, -10.0, 10.0);
    h_NuPxNormalizedResidual = new TH1F(
        "PxNormalizedResidual",
        "; ( _{}p_{x,#nu}^{REC} - p_{x,#nu}^{MC} ) / #sigma_{p_{x,#nu}}; Normalized #SLDs / 0.1", 200, -10.0, 10.0);
    h_NuPyNormalizedResidual = new TH1F(
        "PyNormalizedResidual",
        "; ( _{}p_{y,#nu}^{REC} - p_{y,#nu}^{MC} ) / #sigma_{p_{y,#nu}}; Normalized #SLDs / 0.1", 200, -10.0, 10.0);
    h_NuPzNormalizedResidual = new TH1F(
        "PzNormalizedResidual",
        "; ( _{}p_{z,#nu}^{REC} - p_{z,#nu}^{MC} ) / #sigma_{p_{z,#nu}}; Normalized #SLDs / 0.1", 200, -10.0, 10.0);
    h_NuENormalizedResidual =
        new TH1F("ENormalizedResidual",
                 "; ( _{}E_{#nu}^{REC} - E_{#nu}^{MC} ) / #sigma_{E_{#nu}}; Normalized #SLDs / 0.1", 200, -10.0, 10.0);
    h_recoNuPx_mcNuPx = new TH2F("p_{x}^{#nu}", "; _{}p_{x,#nu}^{MC} [GeV] ;  _{}p_{x,#nu}^{REC} [GeV]", 120, -60.0,
                                 60.0, 120, -60.0, 60.0);
    h_recoNuPy_mcNuPy = new TH2F("p_{y}^{#nu}", "; _{}p_{y,#nu}^{MC} [GeV] ;  _{}p_{y,#nu}^{REC} [GeV]", 120, -60.0,
                                 60.0, 120, -60.0, 60.0);
    h_recoNuPz_mcNuPz = new TH2F("p_{z}^{#nu}", "; _{}p_{z,#nu}^{MC} [GeV] ;  _{}p_{z,#nu}^{REC} [GeV]", 120, -60.0,
                                 60.0, 120, -60.0, 60.0);
    h_recoNuE_mcNuE =
        new TH2F("E^{#nu}", "; _{}E_{#nu}^{MC} [GeV] ;  _{}E_{#nu}^{REC} [GeV]", 100, 0.0, 100.0, 100, 0.0, 100.0);
    h_parentPx_daughtersPx =
        new TH2F("p_{x} conservation", "; _{}Px_{parentHadron}^{MC} [GeV] ;  _{}Px_{daughters}^{MC} [GeV]", 200, -100.0,
                 100.0, 200, -100.0, 100.0);
    h_parentPy_daughtersPy =
        new TH2F("p_{y} conservation", "; _{}Py_{parentHadron}^{MC} [GeV] ;  _{}Py_{daughters}^{MC} [GeV]", 200, -100.0,
                 100.0, 200, -100.0, 100.0);
    h_parentPz_daughtersPz =
        new TH2F("p_{z} conservation", "; _{}Pz_{parentHadron}^{MC} [GeV] ;  _{}Pz_{daughters}^{MC} [GeV]", 200, -100.0,
                 100.0, 200, -100.0, 100.0);
    h_parentE_daughtersE = new TH2F("E conservation", "; _{}E_{parentHadron}^{MC} [GeV] ;  _{}E_{daughters}^{MC} [GeV]",
    ...
    h_secondaryVertex = new TH1F("secondary_vertices", ";", 11, 0, 11);
    ...
    h_distRecoLeptonToDownStreamVertex =
        new TH1F("distance of RecoLepton to DownStreamVertex", ";r^{3D} [mm]", 100, 0.0, 10.0);
  }
...
void SLDCorrection::processEvent(EVENT::LCEvent* pLCEvent) {
  ...
  try {
    VertexVector BsemiLeptonicVertices{};
    VertexVector CsemiLeptonicVertices{};
    VertexVector TsemiLeptonicVertices{};
    VertexVector semiLeptonicVertices{};
    ...
    VertexVector tempSemiLeptonicVertices{};
    ...
    for (int i_mcp = 0; i_mcp < nMCP; ++i_mcp) {
      MCP testLepton = dynamic_cast<EVENT::MCParticle*>(MCParticleCollection->getElementAt(i_mcp));
      if (testLepton->isOverlay())
        continue;
      if (abs(testLepton->getPDG()) == 16 && (testLepton->getGeneratorStatus()) == 1)
        ++nTauNeutrino;
      ...
      if ((abs(testLepton->getPDG()) == 11 || abs(testLepton->getPDG()) == 13 || abs(testLepton->getPDG()) == 15)) {
        ...
        if (primarySLDecay) {
          ...
          if (abs(chargedLeptonPDG) == 11) {
            ...
            if (isBHadronSLDecay && m_fillRootTree)
              h_SLDecayModeB->Fill(0.5);
            if (isCHadronSLDecay && m_fillRootTree)
              h_SLDecayModeC->Fill(0.5);
            solveSLD = solveSLD && (chargedLeptonPDG == testLepton->getPDG());
          } else if (abs(chargedLeptonPDG) == 13) {
            ++m_nSLDecayToMuon;
            if (isBHadronSLDecay && m_fillRootTree)
              h_SLDecayModeB->Fill(1.5);
            if (isCHadronSLDecay && m_fillRootTree)
              h_SLDecayModeC->Fill(1.5);
            solveSLD = solveSLD && (chargedLeptonPDG == testLepton->getPDG());
          } else if (abs(chargedLeptonPDG) == 15) {
            ++m_nSLDecayToTau;
            if (isBHadronSLDecay && m_fillRootTree)
              h_SLDecayModeB->Fill(2.5);
            if (isCHadronSLDecay && m_fillRootTree)
          ...
          if (isCHadronSLDecay && !m_includeCSLD)
            solveSLD = false;
          if (isTauLeptonSLDecay && !m_includeTSLD)
            solveSLD = false;
          if (abs(testLepton->getPDG()) == 15)
            solveSLD = false;
          ...
          streamlog_out(DEBUG3)
              ...
              << std::endl;
          streamlog_out(DEBUG3)
              << "	<<<<<<<<<<<<<<<<<<<<<<<<<<< Found a primary semi-leptonic decay >>>>>>>>>>>>>>>>>>>>>>>>>>>"
              << std::endl;
          if (downStreamSLDecay) {
            streamlog_out(DEBUG3)
                << "	There is/are downstream semi-leptonic(s) decay in primary semi-leptonic decay products"
                << std::endl;
          }
          if (upStreamSLDecay) {
            streamlog_out(DEBUG3)
                << "	There is/are upstream semi-leptonic(s) decay in primary semi-leptonic decay products"
                << std::endl;
          }
          if (downStreamSLDecay || upStreamSLDecay) {
            if (m_fillRootTree)
          ...
          if (solveSLD) {
            ...
            streamlog_out(DEBUG3)
                ...
                << std::endl;
            streamlog_out(DEBUG3)
                << "	<<<<<<<<<<<<<<<< There are no upstream and downstream semi-leptonic decay >>>>>>>>>>>>>>>>>"
                << std::endl;
            streamlog_out(DEBUG3)
                << "	<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<   >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
                << std::endl;
            ...
            for (unsigned int i_d = 0; i_d < (testLepton->getParents()[0])->getDaughters().size(); ++i_d) {
              ...
              if (daughterPDG < 11 || daughterPDG > 16) {
                ...
                for (unsigned int i_Ctype = 0; i_Ctype < CHadPDGs.size(); ++i_Ctype) {
                  if (abs(mcDaughter->getPDG()) == CHadPDGs[i_Ctype] && m_fillRootTree)
                    h_CHadronType->Fill(i_Ctype);
                }
              }
            }
          }
    ...
    for (unsigned int bSLD = 0; bSLD < BsemiLeptonicVertices.size(); ++bSLD)
      semiLeptonicVertices.push_back(BsemiLeptonicVertices[bSLD]);
    for (unsigned int cSLD = 0; cSLD < CsemiLeptonicVertices.size(); ++cSLD)
      semiLeptonicVertices.push_back(CsemiLeptonicVertices[cSLD]);
    for (unsigned int tSLD = 0; tSLD < TsemiLeptonicVertices.size(); ++tSLD)
      semiLeptonicVertices.push_back(TsemiLeptonicVertices[tSLD]);
    if (m_BSLDMode == 0 && m_nSLDecayOfBHadron == 0)
      b_BSLDMode = true;
    if (m_BSLDMode == 1)
      b_BSLDMode = true;
    if (m_BSLDMode == 2 && m_nSLDecayOfBHadron > 0)
      b_BSLDMode = true;
    if (m_BSLDMode == 3 && BsemiLeptonicVertices.size() == m_nSLDecayOfBHadron && BsemiLeptonicVertices.size() > 0)
      b_BSLDMode = true;
    if (m_CSLDMode == 0 && m_nSLDecayOfCHadron == 0)
      b_CSLDMode = true;
    if (m_CSLDMode == 1)
      b_CSLDMode = true;
    if (m_CSLDMode == 2 && m_nSLDecayOfCHadron > 0)
      b_CSLDMode = true;
    if (m_CSLDMode == 3 && CsemiLeptonicVertices.size() == m_nSLDecayOfCHadron && CsemiLeptonicVertices.size() > 0)
      b_CSLDMode = true;
    if (m_TSLDMode == 0 && m_nSLDecayOfTauLepton == 0)
      b_TSLDMode = true;
    if (m_TSLDMode == 1)
      b_TSLDMode = true;
    if (m_TSLDMode == 2 && m_nSLDecayOfTauLepton > 0)
      b_TSLDMode = true;
    if (m_TSLDMode == 3 && TsemiLeptonicVertices.size() == m_nSLDecayOfTauLepton && TsemiLeptonicVertices.size() > 0)
      b_TSLDMode = true;
    if (m_SLDMode == 0 && m_nSLDecayTotal == 0)
      b_SLDMode = true;
    if (m_SLDMode == 1)
      b_SLDMode = true;
    if (m_SLDMode == 2 && m_nSLDecayTotal > 0)
      b_SLDMode = true;
    if (m_SLDMode == 3 && semiLeptonicVertices.size() == m_nSLDecayTotal && semiLeptonicVertices.size() > 0)
      b_SLDMode = true;
    streamlog_out(DEBUG9) << "	Number of BSLD: 	Found = " << m_nSLDecayOfBHadron
                          << " , Corrected = " << BsemiLeptonicVertices.size() << std::endl;
    streamlog_out(DEBUG9) << "	Number of CSLD: 	Found = " << m_nSLDecayOfCHadron
                          << " , Corrected = " << CsemiLeptonicVertices.size() << std::endl;
    streamlog_out(DEBUG9) << "	Number of TSLD: 	Found = " << m_nSLDecayOfTauLepton
                          << " , Corrected = " << TsemiLeptonicVertices.size() << std::endl;
    streamlog_out(DEBUG9) << "	Number of SLD: 		Found = " << m_nSLDecayTotal
                          << " , Corrected = " << semiLeptonicVertices.size() << std::endl;
    streamlog_out(DEBUG9) << "	Event selected by number of BSLD:		" << (b_BSLDMode ? "TRUE" : "FALSE")
                          << std::endl;
    streamlog_out(DEBUG9) << "	Event selected by number of CSLD:		" << (b_CSLDMode ? "TRUE" : "FALSE")
                          << std::endl;
    streamlog_out(DEBUG9) << "	Event selected by number of TSLD:		" << (b_TSLDMode ? "TRUE" : "FALSE")
                          << std::endl;
    streamlog_out(DEBUG9) << "	Event selected by number of SLD:		" << (b_SLDMode ? "TRUE" : "FALSE")
                          << std::endl;
    ...
    streamlog_out(MESSAGE0) << "	Found " << semiLeptonicVertices.size() << " semi-leptonic decays with "
                            << semiLeptonicVertexRecoParticles.size() << " associated reconstructed particles in "
                            << jetsOfSemiLeptonicDecays.size() << " jets" << std::endl;
    for (unsigned int i_sld = 0; i_sld < semiLeptonicVertices.size(); ++i_sld) {
      streamlog_out(DEBUG9) << "		SLD[ " << i_sld << " ]:  Status of Charged Lepton: " << SLDStatus[i_sld]
                            << std::endl;
      streamlog_out(DEBUG9) << "		SLD[ " << i_sld
                            << " ]:  Status of Association of Particles to SLD Vertex: " << PVAStatus[i_sld]
                            << std::endl;
    }
...
bool SLDCorrection::hasPrimarySLDecay(const MCP& parentHadron, int& chargedLeptonPDG) {
  bool hasSLDecay = false;
  if (parentHadron->getGeneratorStatus() == 2 &&
      (floor(abs(parentHadron->getPDG()) / 100) == 5 || (floor(abs(parentHadron->getPDG()) / 1000) == 5) ||
       floor(abs(parentHadron->getPDG()) / 100) == 4 || (floor(abs(parentHadron->getPDG()) / 1000) == 4) ||
       (abs(parentHadron->getPDG()) == 15))) {
    for (long unsigned int i_daughter = 0; i_daughter < (parentHadron->getDaughters()).size(); ++i_daughter) {
      MCP daughter = parentHadron->getDaughters()[i_daughter];
      if (daughter->isOverlay())
        continue;
      if (daughter->getGeneratorStatus() == 1) {
        if (abs(daughter->getPDG()) == 11 || abs(daughter->getPDG()) == 13 || abs(daughter->getPDG()) == 15) {
          ...
          for (long unsigned int i_NuCandidate = 0; i_NuCandidate < (parentHadron->getDaughters()).size();
               ++i_NuCandidate) {
            MCP secondDaughter = parentHadron->getDaughters()[i_NuCandidate];
...
bool SLDCorrection::hasDownStreamSLDecay(const MCP& parentHadron) {
  bool hasSLDecay = false;
  bool primarySLDecay = false;
  bool downStreamSLDecay = false;
  for (long unsigned int i_primaryDaughter = 0; i_primaryDaughter < (parentHadron->getDaughters()).size();
       ++i_primaryDaughter) {
    int lepPDG;
    MCP primaryDaughter = parentHadron->getDaughters()[i_primaryDaughter];
...
bool SLDCorrection::hasUpStreamSLDecay(const MCP& parentHadron) {
  bool hasSLDecay = false;
  bool primarySLDecay = false;
  bool upStreamSLDecay = false;
  for (long unsigned int i_upperParent = 0; i_upperParent < (parentHadron->getParents()).size(); ++i_upperParent) {
    int lepPDG;
    MCP upperParent = parentHadron->getParents()[i_upperParent];
...
void SLDCorrection::doSLDCorrection(EVENT::LCEvent* pLCEvent, const MCP& SLDLepton, VertexVector& semiLeptonicVertices,
  ...
  VertexVector buildUpVertexVector{};
  ...
  for (unsigned int i_d = 0; i_d < parentHadron->getDaughters().size(); ++i_d) {
    if (parentHadron->getDaughters()[i_d] != trueNeutrino)
      trueVisibleFourMomentumAtSLDVertex += TLorentzVector(parentHadron->getDaughters()[i_d]->getMomentum(),
                                                           parentHadron->getDaughters()[i_d]->getEnergy());
  }
  ...
  getTruePVADecayProducts(parentHadron, SLDLepton, trueNeutrino, linkedRecoLepton, weightPFOtoMCP, weightMCPtoPFO,
  ...
  streamlog_out(DEBUG5) << "----------------------------------------------------------------------" << std::endl;
  streamlog_out(DEBUG5) << "----------------- " << trueNeutralDecayProducts.size()
                        << " Stable Neutral MCParticles Found------------------" << std::endl;
  streamlog_out(DEBUG5) << "----------------------------------------------------------------------" << std::endl;
  for (unsigned int i_par = 0; i_par < trueNeutralDecayProducts.size(); ++i_par) {
    truePVATrueNeutralFourMomentum += TLorentzVector((trueNeutralDecayProducts[i_par])->getMomentum(),
                                                     (trueNeutralDecayProducts[i_par])->getEnergy());
    streamlog_out(DEBUG5) << "		Found One Stable Neutral Particle in Semi-Leptonic Decay Products:"
                          << std::endl;
    streamlog_out(DEBUG5) << *trueNeutralDecayProducts[i_par] << std::endl;
  }
  streamlog_out(DEBUG5) << "----------------------------------------------------------------------" << std::endl;
  streamlog_out(DEBUG5) << "--------------- " << truePVANeutralDecayProducts.size()
                        << " Neutral Reconstructed Particle Found----------------" << std::endl;
  streamlog_out(DEBUG5) << "----------------------------------------------------------------------" << std::endl;
  for (unsigned int i_par = 0; i_par < truePVANeutralDecayProducts.size(); ++i_par) {
    truePVARecoNeutralFourMomentum += TLorentzVector((truePVANeutralDecayProducts[i_par])->getMomentum(),
                                                     (truePVANeutralDecayProducts[i_par])->getEnergy());
    streamlog_out(DEBUG5) << "		Found One Neutral Reconstructed Particle in Semi-Leptonic Decay Products:"
                          << std::endl;
    streamlog_out(DEBUG5) << *truePVANeutralDecayProducts[i_par] << std::endl;
  }
  streamlog_out(DEBUG5) << "----------------------------------------------------------------------" << std::endl;
  streamlog_out(DEBUG5) << "----------------- " << trueChargedDecayProducts.size()
                        << " Stable Charged MCParticles Found------------------" << std::endl;
  streamlog_out(DEBUG5) << "----------------------------------------------------------------------" << std::endl;
  for (unsigned int i_par = 0; i_par < trueChargedDecayProducts.size(); ++i_par) {
    truePVATrueChargedFourMomentum += TLorentzVector((trueChargedDecayProducts[i_par])->getMomentum(),
                                                     (trueChargedDecayProducts[i_par])->getEnergy());
    streamlog_out(DEBUG5) << "		Found One Stable Charged Particle in Semi-Leptonic Decay Products:"
                          << std::endl;
    streamlog_out(DEBUG5) << *trueChargedDecayProducts[i_par] << std::endl;
  }
  streamlog_out(DEBUG5) << "----------------------------------------------------------------------" << std::endl;
  streamlog_out(DEBUG5) << "--------------- " << truePVAChargedDecayProducts.size()
                        << " Charged Reconstructed Particle Found----------------" << std::endl;
  streamlog_out(DEBUG5) << "----------------------------------------------------------------------" << std::endl;
  for (unsigned int i_par = 0; i_par < truePVAChargedDecayProducts.size(); ++i_par) {
    truePVARecoChargedFourMomentum += TLorentzVector((truePVAChargedDecayProducts[i_par])->getMomentum(),
                                                     (truePVAChargedDecayProducts[i_par])->getEnergy());
    streamlog_out(DEBUG5) << "		Found One Charged Reconstructed Particle in Semi-Leptonic Decay Products:"
                          << std::endl;
  ...
  streamlog_out(DEBUG5) << "" << std::endl;
  streamlog_out(DEBUG5) << "		True Visible Energy At the vertex of Semi-Leptonic Decay, E_vis 	= "
                        << trueVisibleFourMomentumAtSLDVertex.E() << " GeV" << std::endl;
  streamlog_out(DEBUG5) << "		True Visible Energy from decay chain of Semi-Leptonic Decay, E_vis 	= "
                        << truePVATrueFourMomentum.E() << " GeV" << std::endl;
  ...
  m_true_E_vis_prime.push_back((pow(parentHadron->getMass(), 2) + pow(trueVisibleFourMomentumAtSLDVertex.M(), 2)) /
                               (2 * parentHadron->getMass()));
  TVector3 p_vis = TVector3(trueVisibleFourMomentumAtSLDVertex.Px(), trueVisibleFourMomentumAtSLDVertex.Py(),
                            trueVisibleFourMomentumAtSLDVertex.Pz());
  ...
  m_true_P_vis_par_prime.push_back(
      sqrt(pow((pow(parentHadron->getMass(), 2) - pow(trueVisibleFourMomentumAtSLDVertex.M(), 2)) /
                   (2 * parentHadron->getMass()),
               2) -
           pow(sqrt(pow(p_vis.Mag(), 2) - pow(p_vis.Dot(trueFlightDirection), 2)), 2)));
  streamlog_out(DEBUG8) << "		 Flight Dir of Parent Hadron:		( " << trueFlightDirection.X() << " , "
                        << trueFlightDirection.Y() << " , " << trueFlightDirection.Z() << " )" << std::endl;
  ...
  streamlog_out(DEBUG8) << "		 Visible Energy:				" << _visibleEnergy
                        << std::endl;
  double _visibleEnergyPrime = (pow(parentHadron->getMass(), 2) + pow(trueVisibleFourMomentumAtSLDVertex.M(), 2)) /
                               (2 * parentHadron->getMass());
  streamlog_out(DEBUG8) << "		 Visible Energy prime:			" << _visibleEnergyPrime << std::endl;
  double _visibleMomentum = p_vis.Mag();
  streamlog_out(DEBUG8) << "		 Visible Momentum:				" << _visibleMomentum << " ("
                        << p_vis.Px() << " , " << p_vis.Py() << " , " << p_vis.Pz() << ")" << std::endl;
  double _visibleMomentumParallel = p_vis.Dot(trueFlightDirection);
  streamlog_out(DEBUG8) << "		|Visible Momentum (par)|:		" << _visibleMomentumParallel
                        << std::endl;
  double _visibleMomentumPerpendicular = sqrt(pow(p_vis.Mag(), 2) - pow(p_vis.Dot(trueFlightDirection), 2));
  streamlog_out(DEBUG8) << "		|Visible Momentum (nor)|:		" << _visibleMomentumPerpendicular
                        << std::endl;
  double _visibleMomentumParallelPrime =
      sqrt(pow((pow(parentHadron->getMass(), 2) - pow(trueVisibleFourMomentumAtSLDVertex.M(), 2)) /
                   (2 * parentHadron->getMass()),
               2) -
           pow(sqrt(pow(p_vis.Mag(), 2) - pow(p_vis.Dot(trueFlightDirection), 2)), 2));
  streamlog_out(DEBUG8) << "		|Visible Momentum (par-prime)|:	+/-" << _visibleMomentumParallelPrime
                        << std::endl;
  double _cheatedNeutrinoSolutionPos =
      ((_visibleEnergy * _visibleEnergyPrime) - (+1.0 * _visibleMomentumParallel * _visibleMomentumParallelPrime)) *
          _parentHadronMass / (pow(_visibleEnergy, 2) - pow(_visibleMomentumParallel, 2)) -
      _visibleEnergy;
  double _cheatedNeutrinoSolutionNeg =
      ((_visibleEnergy * _visibleEnergyPrime) - (-1.0 * _visibleMomentumParallel * _visibleMomentumParallelPrime)) *
          _parentHadronMass / (pow(_visibleEnergy, 2) - pow(_visibleMomentumParallel, 2)) -
      _visibleEnergy;
  double _trueNeutrinoEnergy = trueNeutrinoFourMomentum.E();
  streamlog_out(DEBUG8) << "		 Neutrino Solution (+):			" << _cheatedNeutrinoSolutionPos
                        << std::endl;
  streamlog_out(DEBUG8) << "		 Neutrino Solution (-):			" << _cheatedNeutrinoSolutionNeg
                        << std::endl;
  ...
  aloneChargedPFOsInJet =
      getParticlesWithAloneTracks(linkedRecoLepton, assignedJet, primaryVertex, buildUpVertexVector);
  ...
  TVector3 leptonDirection = TVector3(SLDLepton->getMomentum());
  leptonDirection.SetMag(1.0);
  TVector3 jetAxis = TVector3(assignedJet->getMomentum());
  jetAxis.SetMag(1.0);
  if (m_displayEvent)
    ced_line(primaryVertex->getPosition()[0] + 10.0 * jetAxis.X(), primaryVertex->getPosition()[1] + 10.0 * jetAxis.Y(),
             primaryVertex->getPosition()[2] + 10.0 * jetAxis.Z(), primaryVertex->getPosition()[0],
             primaryVertex->getPosition()[1], primaryVertex->getPosition()[2], 2, 1, 0x000000);

  VertexVector verticesInJet = getVerticesInJet(assignedJet, buildUpVertexVector);
  ...
  if (SLDStatus < 0)
    SLDStatus = getRecoFlightDirection(
        linkedRecoLepton, recoFlightDirection, hadronFlightLength, primaryVertex, startVertex, SLDVertex, SLDVertexRP,
        assignedJet, verticesInJet, sortedChargedPFOs, helicesDistance, vertexingScenario,
        daughterHadronFlightDirection, daughterHadronFlightDistance, sldVertexPosition, PCAatLepton, PCAatOtherParticle,
        lepton3DImpactParameter, OtherParticle3DImpactParameter);
  if (flightDirection.Dot(jetAxis) < 0.0)
    flightDirection = -1.0 * flightDirection;
  ...
  TVector3 LepFromPV(PCAatLepton.X() - primaryVertex->getPosition()[0],
                     PCAatLepton.Y() - primaryVertex->getPosition()[1],
                     PCAatLepton.Z() - primaryVertex->getPosition()[2]);
  m_LeptonDistanceFromPV.push_back(LepFromPV.Mag());
  LepFromPV.SetMag(1.0);
  TVector3 DSVFromPV(PCAatOtherParticle.X() - primaryVertex->getPosition()[0],
                     PCAatOtherParticle.Y() - primaryVertex->getPosition()[1],
                     PCAatOtherParticle.Z() - primaryVertex->getPosition()[2]);
  ...
  for (unsigned int i_par = 0; i_par < assignedJet->getParticles().size(); ++i_par) {
    ...
    if (jetParticle == linkedRecoLepton)
      continue;
    ...
    if (jetParticle->getTracks().size() == 0 && jetParticleMomentum.Dot(PVAConeAxis) >= m_neutralCosAcceptanceAngle) {
      ...
      for (unsigned int i_vtx = 0; i_vtx < buildUpVertexVector.size(); ++i_vtx) {
        if (!particleIsInAVertex) {
          ...
          streamlog_out(DEBUG0) << "	Looking for particle in build up vertex[ " << i_vtx << " ]: " << testVertex
                                << std::endl;
  ...
  if (SLDStatus == 4) {
    ...
    for (unsigned int i_par = 0; i_par < SLDVertexRP->getParticles().size(); ++i_par) {
      ...
      if (chargedDecayProduct != linkedRecoLepton)
        recoSLDVertexDecayProducts.push_back(chargedDecayProduct);
      if (m_displayEvent)
        drawReconstructedParticle(chargedDecayProduct, primaryVertex, 0x0075df, 0x2e8e04);
    }
  } else if (SLDStatus == 5) {
  ...
  for (unsigned int i_par = 0; i_par < recoSLDVertexDecayProducts.size(); ++i_par) {
    visibleTLV += TLorentzVector(recoSLDVertexDecayProducts[i_par]->getMomentum(),
                                 recoSLDVertexDecayProducts[i_par]->getEnergy());
  }

  VertexVector availableVerticesInJet{};
  for (unsigned int i_vtx = 0; i_vtx < verticesInJet.size(); ++i_vtx) {
    if (verticesInJet[i_vtx] != SLDVertex)
      availableVerticesInJet.push_back(verticesInJet[i_vtx]);
  }
  if (m_cheatPVAneutral) {
    for (unsigned int i_par = 0; i_par < truePVANeutralDecayProducts.size(); ++i_par) {
      recoPVANeutralDecayProducts.push_back(truePVANeutralDecayProducts[i_par]);
      if (m_displayEvent)
        drawReconstructedParticle(truePVANeutralDecayProducts[i_par], primaryVertex, 0x0075df, 0x2e8e04);
    }
    if (m_cheatPVAcharged) {
      for (unsigned int i_par = 0; i_par < truePVAChargedDecayProducts.size(); ++i_par) {
        recoPVAChargedDecayProducts.push_back(truePVAChargedDecayProducts[i_par]);
        if (m_displayEvent)
          drawReconstructedParticle(truePVAChargedDecayProducts[i_par], primaryVertex, 0x0075df, 0x2e8e04);
      }
    } else {
      ...
      assignVerticesToSemiLeptonicDecay(recoPVAChargedDecayProducts, availableVerticesInJet, InvMassCutCharged,
                                        PVAConeAxis, startVertex, visibleTLV);
      ...
      assignParticlesToSemiLeptonicDecay(recoPVAChargedDecayProducts, chargedPFOsInJet, InvMassCutCharged, PVAConeAxis,
                                         visibleTLV);
    ...
    if (m_cheatPVAcharged) {
      for (unsigned int i_par = 0; i_par < truePVAChargedDecayProducts.size(); ++i_par) {
        recoPVAChargedDecayProducts.push_back(truePVAChargedDecayProducts[i_par]);
        visibleTLV += TLorentzVector(truePVAChargedDecayProducts[i_par]->getMomentum(),
                                     truePVAChargedDecayProducts[i_par]->getEnergy());
        if (m_displayEvent)
          drawReconstructedParticle(truePVAChargedDecayProducts[i_par], primaryVertex, 0x0075df, 0x2e8e04);
      }
    ...
    assignParticlesToSemiLeptonicDecay(recoPVANeutralDecayProducts, neutralPFOsInJet, InvMassCutNeutral, PVAConeAxis,
                                       visibleTLV);
  ...
  if (m_displayEvent)
    drawReconstructedParticle(linkedRecoLepton, primaryVertex, 0xfc0000, 0x2e8e04);
  ...
  for (unsigned int i_par = 0; i_par < recoSLDVertexDecayProducts.size(); ++i_par) {
    recoSLDVertexChargedFourMomentum += TLorentzVector(recoSLDVertexDecayProducts[i_par]->getMomentum(),
                                                       recoSLDVertexDecayProducts[i_par]->getEnergy());
  ...
  for (unsigned int i_par = 0; i_par < recoPVAVertexDecayProducts.size(); ++i_par) {
    recoPVARecoChargedFourMomentum += TLorentzVector(recoPVAVertexDecayProducts[i_par]->getMomentum(),
                                                     recoPVAVertexDecayProducts[i_par]->getEnergy());
  ...
  for (unsigned int i_par = 0; i_par < recoPVAChargedDecayProducts.size(); ++i_par) {
    recoPVARecoChargedFourMomentum += TLorentzVector(recoPVAChargedDecayProducts[i_par]->getMomentum(),
                                                     recoPVAChargedDecayProducts[i_par]->getEnergy());
  ...
  for (unsigned int i_par = 0; i_par < recoPVANeutralDecayProducts.size(); ++i_par) {
    recoPVARecoNeutralFourMomentum += TLorentzVector(recoPVANeutralDecayProducts[i_par]->getMomentum(),
                                                     recoPVANeutralDecayProducts[i_par]->getEnergy());
    recoPVADecayProducts.push_back(recoPVANeutralDecayProducts[i_par]);
  }
  recoPVARecoFourMomentum = recoLeptonFourMomentum + recoSLDVertexChargedFourMomentum + recoPVARecoChargedFourMomentum +
                            recoPVARecoNeutralFourMomentum;
  ...
  streamlog_out(DEBUG8) << "			     (  			PDG 	, Mass		, Px	"
                           "	, Py		, Pz		, E		, Charge	)"
                        << std::endl;
  ...
  streamlog_out(DEBUG8) << "			True:(				" << SLDLepton->getPDG() << "	, "
                        << SLDLepton->getMass() << "	, " << SLDLepton->getMomentum()[0] << "	, "
                        << SLDLepton->getMomentum()[1] << "	, " << SLDLepton->getMomentum()[2] << "	, "
                        << SLDLepton->getEnergy() << "	, " << SLDLepton->getCharge() << "	)" << std::endl;
  ...
  streamlog_out(DEBUG8) << "			Used:(				" << "xxx" << "	, "
                        ...
                        << leptonFourMomentum.E() << "	, " << (int)SLDLepton->getCharge() << "		)" << std::endl;
  streamlog_out(DEBUG8) << "" << std::endl;
  streamlog_out(DEBUG8) << "		Charged" << std::endl;
  streamlog_out(DEBUG8) << "			TruePVATrue4Mom:(		" << "qqq" << "	, "
                        << truePVATrueChargedFourMomentum.M() << "	, " << truePVATrueChargedFourMomentum.Px()
                        << "	, " << truePVATrueChargedFourMomentum.Py() << "	, "
                        << truePVATrueChargedFourMomentum.Pz() << "	, " << truePVATrueChargedFourMomentum.E()
                        << "	, " << "qqq" << "		)" << std::endl;
  streamlog_out(DEBUG8) << "			TruePVAReco4Mom:(		" << "qqq" << "	, "
                        << truePVARecoChargedFourMomentum.M() << "	, " << truePVARecoChargedFourMomentum.Px()
                        << "	, " << truePVARecoChargedFourMomentum.Py() << "	, "
                        << truePVARecoChargedFourMomentum.Pz() << "	, " << truePVARecoChargedFourMomentum.E()
                        << "	, " << "qqq" << "		)" << std::endl;
  streamlog_out(DEBUG8) << "			RecoPVAReco4Mom:(		" << "qqq" << "	, "
                        << (recoPVARecoChargedFourMomentum + recoSLDVertexChargedFourMomentum).M() << "	, "
                        << (recoPVARecoChargedFourMomentum + recoSLDVertexChargedFourMomentum).Px() << "	, "
                        << (recoPVARecoChargedFourMomentum + recoSLDVertexChargedFourMomentum).Py() << "	, "
                        << (recoPVARecoChargedFourMomentum + recoSLDVertexChargedFourMomentum).Pz() << "	, "
                        << (recoPVARecoChargedFourMomentum + recoSLDVertexChargedFourMomentum).E() << "	, " << "qqq"
                        << "		)" << std::endl;
  streamlog_out(DEBUG8) << "			Used:(				" << "qqq" << "	, "
                        << chargedFourMomentum.M() << "	, " << chargedFourMomentum.Px() << "	, "
                        << chargedFourMomentum.Py() << "	, " << chargedFourMomentum.Pz() << "	, "
                        << chargedFourMomentum.E() << "	, " << "nnn" << "		)" << std::endl;
  streamlog_out(DEBUG8) << "" << std::endl;
  streamlog_out(DEBUG8) << "		Neutral" << std::endl;
  streamlog_out(DEBUG8) << "			TruePVATrue4Mom:(		" << "nnn" << "	, "
                        << truePVATrueNeutralFourMomentum.M() << "	, " << truePVATrueNeutralFourMomentum.Px()
                        << "	, " << truePVATrueNeutralFourMomentum.Py() << "	, "
                        << truePVATrueNeutralFourMomentum.Pz() << "	, " << truePVATrueNeutralFourMomentum.E()
                        << "	, " << "qqq" << "		)" << std::endl;
  streamlog_out(DEBUG8) << "			TruePVAReco4Mom:(		" << "nnn" << "	, "
                        << truePVARecoNeutralFourMomentum.M() << "	, " << truePVARecoNeutralFourMomentum.Px()
                        << "	, " << truePVARecoNeutralFourMomentum.Py() << "	, "
                        << truePVARecoNeutralFourMomentum.Pz() << "	, " << truePVARecoNeutralFourMomentum.E()
                        << "	, " << "qqq" << "		)" << std::endl;
  streamlog_out(DEBUG8) << "			RecoPVAReco4Mom:(		" << "nnn" << "	, "
                        << recoPVARecoNeutralFourMomentum.M() << "	, " << recoPVARecoNeutralFourMomentum.Px()
                        << "	, " << recoPVARecoNeutralFourMomentum.Py() << "	, "
                        << recoPVARecoNeutralFourMomentum.Pz() << "	, " << recoPVARecoNeutralFourMomentum.E()
                        << "	, " << "qqq" << "		)" << std::endl;
  streamlog_out(DEBUG8) << "			Used:(				" << "nnn" << "	, "
                        << neutralFourMomentum.M() << "	, " << neutralFourMomentum.Px() << "	, "
                        << neutralFourMomentum.Py() << "	, " << neutralFourMomentum.Pz() << "	, "
                        << neutralFourMomentum.E() << "	, " << "nnn" << "		)" << std::endl;
  streamlog_out(DEBUG8) << "" << std::endl;
  streamlog_out(DEBUG8) << "		Visible" << std::endl;
  streamlog_out(DEBUG8) << "			True:(				" << "nnn" << "	, "
                        << truePVATrueFourMomentum.M() << "	, " << truePVATrueFourMomentum.Px() << "	, "
                        << truePVATrueFourMomentum.Py() << "	, " << truePVATrueFourMomentum.Pz() << "	, "
                        << truePVATrueFourMomentum.E() << "	, " << "nnn" << "		)" << std::endl;
  streamlog_out(DEBUG8) << "			Used:(				" << "nnn" << "	, "
                        << visibleFourMomentum.M() << "	, " << visibleFourMomentum.Px() << "	, "
                        << visibleFourMomentum.Py() << "	, " << visibleFourMomentum.Pz() << "	, "
                        << visibleFourMomentum.E() << "	, " << "nnn" << "		)" << std::endl;
  streamlog_out(DEBUG8) << "" << std::endl;
  streamlog_out(DEBUG8) << "" << std::endl;
  streamlog_out(DEBUG8) << "		Flight Direction	(						"
                           "X			, Y			, Z		)"
                        << std::endl;
  streamlog_out(DEBUG8) << "			True:		(						"
                        << trueFlightDirection.X() << "		, " << trueFlightDirection.Y() << "		, "
                        << trueFlightDirection.Z() << "	)" << std::endl;
  streamlog_out(DEBUG8) << "			Reco:		(						"
                        << recoFlightDirection.X() << "		, " << recoFlightDirection.Y() << "		, "
                        << recoFlightDirection.Z() << "	)" << std::endl;
  streamlog_out(DEBUG8) << "			Used:		(						"
                        << flightDirection.X() << "		, " << flightDirection.Y() << "		, "
                        << flightDirection.Z() << "	)" << std::endl;
  ...
  if (m_fillRootTree)
    h_SLDStatus->Fill(SLDStatus - 0.5);

  recoNeutrinoFourMomentumPos =
      getNeutrinoFourMomentumModified(flightDirection, visibleFourMomentum, parentHadronMass, +1.0);
  recoNeutrinoFourMomentumNeg =
      getNeutrinoFourMomentumModified(flightDirection, visibleFourMomentum, parentHadronMass, -1.0);
  // recoNeutrinoFourMomentumPos = getNeutrinoFourMomentum( flightDirection , visibleFourMomentum , parentHadronMass ,
  // +1.0 ); recoNeutrinoFourMomentumNeg = getNeutrinoFourMomentum( flightDirection , visibleFourMomentum ,
  // parentHadronMass , -1.0 ); recoNeutrinoFourMomentumPos = getNeutrinoFourMomentumStandardMethod( flightDirection ,
  // visibleFourMomentum , parentHadronMass , +1.0 ); recoNeutrinoFourMomentumNeg =
  // getNeutrinoFourMomentumStandardMethod( flightDirection , visibleFourMomentum , parentHadronMass , -1.0 );
  ...
  if (m_cheatPVAneutral || m_cheatPVAcharged)
    PVAStatus = 0;
  ...
  if (fabs(_cheatedNeutrinoSolutionPos - _trueNeutrinoEnergy) <=
      fabs(_cheatedNeutrinoSolutionNeg - _trueNeutrinoEnergy)) {
  ...
  streamlog_out(DEBUG4) << "	Closest Neutrino 4-Momentum:			( "
                        << recoNeutrinoFourMomentumClose.Px() << "	, " << recoNeutrinoFourMomentumClose.Py()
                        << "	, " << recoNeutrinoFourMomentumClose.Pz() << "	, " << recoNeutrinoFourMomentumClose.E()
                        << " )" << std::endl;
  streamlog_out(DEBUG4) << "	True Neutrino 4-Momentum:			( " << trueNeutrinoFourMomentum.Px()
                        << "	, " << trueNeutrinoFourMomentum.Py() << "	, " << trueNeutrinoFourMomentum.Pz()
                        << "	, " << trueNeutrinoFourMomentum.E() << " )" << std::endl;
  if (fabs(recoNeutrinoFourMomentumClose.E() - trueNeutrinoFourMomentum.E()) > 0.1 * trueNeutrinoFourMomentum.E()) {
    if (m_traceEvent) {
      streamlog_out(MESSAGE) << "	Closest Neutrino 4-Momentum:			( "
                             << recoNeutrinoFourMomentumClose.Px() << "	, " << recoNeutrinoFourMomentumClose.Py()
                             << "	, " << recoNeutrinoFourMomentumClose.Pz() << "	, "
                             << recoNeutrinoFourMomentumClose.E() << " )" << std::endl;
      streamlog_out(MESSAGE) << "	True Neutrino 4-Momentum:			( "
                             << trueNeutrinoFourMomentum.Px() << "	, " << trueNeutrinoFourMomentum.Py()
                             << "	, " << trueNeutrinoFourMomentum.Pz() << "	, "
                             << trueNeutrinoFourMomentum.E() << " )" << std::endl;
      streamlog_out(MESSAGE) << "	!!! Big Difference between true and reco neutrino Energy : "
                             << recoNeutrinoFourMomentumClose.E() - trueNeutrinoFourMomentum.E()
                             << "  GeV in event: " << pLCEvent->getEventNumber() << std::endl;
  ...
  streamlog_out(DEBUG8) << "				" << NeutrinoCovMatPos[1] << "	,	"
                        << NeutrinoCovMatPos[2] << std::endl;
  streamlog_out(DEBUG8) << "				" << NeutrinoCovMatPos[3] << "	,	"
                        << NeutrinoCovMatPos[4] << "	,	" << NeutrinoCovMatPos[5] << std::endl;
  streamlog_out(DEBUG8) << "				" << NeutrinoCovMatPos[6] << "	,	"
                        << NeutrinoCovMatPos[7] << "	,	" << NeutrinoCovMatPos[8] << "	,	"
                        << NeutrinoCovMatPos[9] << std::endl;
  streamlog_out(DEBUG8) << "" << std::endl;
  streamlog_out(DEBUG8) << "	CovMatNeutrino(-) :	" << NeutrinoCovMatNeg[0] << std::endl;
  streamlog_out(DEBUG8) << "				" << NeutrinoCovMatNeg[1] << "	,	"
                        << NeutrinoCovMatNeg[2] << std::endl;
  streamlog_out(DEBUG8) << "				" << NeutrinoCovMatNeg[3] << "	,	"
                        << NeutrinoCovMatNeg[4] << "	,	" << NeutrinoCovMatNeg[5] << std::endl;
  streamlog_out(DEBUG8) << "				" << NeutrinoCovMatNeg[6] << "	,	"
                        << NeutrinoCovMatNeg[7] << "	,	" << NeutrinoCovMatNeg[8] << "	,	"
                        << NeutrinoCovMatNeg[9] << std::endl;
  streamlog_out(DEBUG8) << "" << std::endl;

  evaluateInputCovMat(truePVATrueFourMomentum, trueFlightDirection, trueNeutrinoFourMomentum, visibleFourMomentum,
                      flightDirection, recoNeutrinoFourMomentumClose, CovMatrixDetector, NeutrinoCovMat);
  TVector3 recoNeutrinoDirection(recoNeutrinoFourMomentumClose.Px(), recoNeutrinoFourMomentumClose.Py(),
                                 recoNeutrinoFourMomentumClose.Pz());
  recoNeutrinoDirection.SetMag(1.0);
  m_AngleRecoNeutrinoJet.push_back(acos(recoNeutrinoDirection.Dot(jetAxis)));
  TVector3 trueNeutrinoDirection(trueNeutrinoFourMomentum.Px(), trueNeutrinoFourMomentum.Py(),
                                 trueNeutrinoFourMomentum.Pz());
  trueNeutrinoDirection.SetMag(1.0);
  ...
  fillTrueRecoFourMomentum(trueVisibleFourMomentumAtSLDVertex, truePVATrueFourMomentum, truePVARecoFourMomentum,
                           recoPVARecoFourMomentum, visibleFourMomentum, trueLeptonFourMomentum, recoLeptonFourMomentum,
                           leptonFourMomentum, truePVATrueChargedFourMomentum, truePVARecoChargedFourMomentum,
                           recoSLDVertexChargedFourMomentum, recoPVARecoChargedFourMomentum, chargedFourMomentum,
                           truePVATrueNeutralFourMomentum, truePVARecoNeutralFourMomentum,
                           recoPVARecoNeutralFourMomentum, neutralFourMomentum, trueNeutrinoFourMomentum,
                           recoNeutrinoFourMomentumClose, trueHadronFourMomentum, recoHadronFourMomentum,
                           trueFlightDirection, recoFlightDirection, flightDirection);

  streamlog_out(DEBUG8) << "		Semi-leptonic decay type: " << SLDStatus << std::endl;

  double MomentumPos[3]{recoNeutrinoFourMomentumPos.Px(), recoNeutrinoFourMomentumPos.Py(),
                        recoNeutrinoFourMomentumPos.Pz()};
  ...
  for (unsigned int j = 0; j < linkedRecoLepton->getParticleIDs().size(); ++j) {
    ...
    for (unsigned int k = 0; k < inPID->getParameters().size(); ++k)
      outPID->addParameter(inPID->getParameters()[k]);
  ...
  double MomentumNeg[3]{recoNeutrinoFourMomentumNeg.Px(), recoNeutrinoFourMomentumNeg.Py(),
                        recoNeutrinoFourMomentumNeg.Pz()};
  ...
  streamlog_out(DEBUG7) << "		Position: ( " << vertexPosition[0] << " , " << vertexPosition[1] << " , "
  ...
  streamlog_out(DEBUG7) << "		Position : " << semiLeptonicVertex->getPosition()[0] << " , "
                        << semiLeptonicVertex->getPosition()[1] << " , " << semiLeptonicVertex->getPosition()[2]
                        << std::endl;
  semiLeptonicVertex->setAssociatedParticle(semiLeptonicVertexRecoParticle);
  streamlog_out(DEBUG7) << "		AssociatedParticle : " << semiLeptonicVertex->getAssociatedParticle()
                        << std::endl;
  ...
  for ( unsigned int j = 0 ; j < linkedRecoLepton->getParticleIDs().size() ; ++j )
  {
          ...
          outPID->setLikelihood( inPID->getLikelihood() );
          outPID->setAlgorithmType( inPID->getAlgorithmType() ) ;
          for ( unsigned int k = 0 ; k < inPID->getParameters().size() ; ++k ) outPID->addParameter(
  inPID->getParameters()[ k ] ); semiLeptonicVertexRecoParticle->addParticleID( outPID );
  }
  */
...
void SLDCorrection::checkSLDInput(const MCP& SLDHadron) {
  if (SLDHadron->getDaughters().size() == 0) {
    ...
    streamlog_out(MESSAGE)
        << "		--------------------------------------------------------------------------------------------"
        << std::endl;
    streamlog_out(MESSAGE) << "		semi-leptonic decay has a decay product with "
                           << SLDHadron->getDaughters().size() << " daughters:" << std::endl;
...
void SLDCorrection::addNeutrinoCovarianceMatrix(const TLorentzVector& neutrinoFourMomentum,
                                                std::vector<float>& NuCovMat) {
  ...
  //
  //
  //			E . ( cos( Theta) . cos( Phi ) - sin( Phi ) )		E . ( cos( Theta) . sin( Phi ) + cos(
  // Phi ) )		-E . cos( Theta)		0 	 J = 			sin( Theta ) . cos( Phi )
  ...
  double jacobian_by_rows[rows * columns] = {energy * (cos(theta) * cos(phi) - sin(phi)),
                                             energy * (cos(theta) * sin(phi) - cos(phi)),
                                             -1.0 * energy * sin(theta),
                                             0.0,
                                             sin(theta) * cos(phi),
                                             sin(theta) * sin(phi),
                                             cos(theta),
  ...
  streamlog_out(DEBUG6) << "	Jacobian :	" << jacobian(0, 0) << "	,	" << jacobian(0, 1)
                        << "	,	" << jacobian(0, 2) << "	,	" << jacobian(0, 3) << std::endl;
  streamlog_out(DEBUG6) << "			" << jacobian(1, 0) << "	,	" << jacobian(1, 1)
                        << "	,	" << jacobian(1, 2) << "	,	" << jacobian(1, 3) << std::endl;
  double cov_matrix_by_rows[rows * rows] = {pow(sigma_alpha, 2), 0.0, 0.0, pow(sigma_E, 2)};
  ...
  streamlog_out(DEBUG6) << "				" << covMatrix(1, 0) << "	,	" << covMatrix(1, 1)
                        << std::endl;
  streamlog_out(DEBUG6) << "				" << covMatrix(2, 0) << "	,	" << covMatrix(2, 1)
                        << "	,	" << covMatrix(2, 2) << std::endl;
  streamlog_out(DEBUG6) << "				" << covMatrix(3, 0) << "	,	" << covMatrix(3, 1)
                        << "	,	" << covMatrix(3, 2) << "	,	" << covMatrix(3, 3) << std::endl;
  ...
  streamlog_out(DEBUG6) << "				" << NuCovMat[3] << "	,	" << NuCovMat[4]
                        << "	,	" << NuCovMat[5] << std::endl;
  streamlog_out(DEBUG6) << "				" << NuCovMat[6] << "	,	" << NuCovMat[7]
                        << "	,	" << NuCovMat[8] << "	,	" << NuCovMat[9] << std::endl;
  streamlog_out(DEBUG6) << "" << std::endl;
}

void SLDCorrection::evaluateInputCovMat(const TLorentzVector& trueVisibleFourMomentum,
                                        const TVector3& trueFlightDirection,
                                        const TLorentzVector& trueNeutrinoFourMomentum,
                                        const TLorentzVector& visibleFourMomentum, const TVector3& flightDirection,
                                        const TLorentzVector& recoNeutrinoFourMomentum,
                                        const FloatVector& CovMatDetector, const FloatVector& CovMatNeutrino) {
  ...
  m_normalizedResidualVisibleFourMomentumPx.push_back((visibleFourMomentum.Px() - trueVisibleFourMomentum.Px()) /
                                                      sqrt(CovMatDetector[0]));
  m_normalizedResidualVisibleFourMomentumPy.push_back((visibleFourMomentum.Py() - trueVisibleFourMomentum.Py()) /
                                                      sqrt(CovMatDetector[2]));
  m_normalizedResidualVisibleFourMomentumPz.push_back((visibleFourMomentum.Pz() - trueVisibleFourMomentum.Pz()) /
                                                      sqrt(CovMatDetector[5]));
  m_normalizedResidualVisibleFourMomentumE.push_back((visibleFourMomentum.E() - trueVisibleFourMomentum.E()) /
                                                     sqrt(CovMatDetector[9]));
  ...
  m_normalizedResidualNeutrinoFourMomentumPx.push_back((recoNeutrinoFourMomentum.Px() - trueNeutrinoFourMomentum.Px()) /
                                                       sqrt(CovMatNeutrino[0]));
  m_normalizedResidualNeutrinoFourMomentumPy.push_back((recoNeutrinoFourMomentum.Py() - trueNeutrinoFourMomentum.Py()) /
                                                       sqrt(CovMatNeutrino[2]));
  m_normalizedResidualNeutrinoFourMomentumPz.push_back((recoNeutrinoFourMomentum.Pz() - trueNeutrinoFourMomentum.Pz()) /
                                                       sqrt(CovMatNeutrino[5]));
  m_normalizedResidualNeutrinoFourMomentumE.push_back((recoNeutrinoFourMomentum.E() - trueNeutrinoFourMomentum.E()) /
                                                      sqrt(CovMatNeutrino[9]));
  TVector3 trueNeutrinoDirection(trueNeutrinoFourMomentum.Px(), trueNeutrinoFourMomentum.Py(),
                                 trueNeutrinoFourMomentum.Pz());
  trueNeutrinoDirection.SetMag(1.0);
  TVector3 recoNeutrinoDirection(recoNeutrinoFourMomentum.Px(), recoNeutrinoFourMomentum.Py(),
                                 recoNeutrinoFourMomentum.Pz());
  recoNeutrinoDirection.SetMag(1.0);
...
void SLDCorrection::showTrueParameters(const MCP& SLDLepton) {
  ...
  for (unsigned int i_mcp = 0; i_mcp < parentHadron->getDaughters().size(); ++i_mcp) {
    MCP daughter = parentHadron->getDaughters()[i_mcp];
    streamlog_out(DEBUG4) << *daughter << std::endl;
    if (std::fabs(daughter->getPDG()) != 12 && std::fabs(daughter->getPDG()) != 14 &&
        std::fabs(daughter->getPDG()) != 16) {
  ...
  streamlog_out(DEBUG4) << "	TRUE FLIHT DIRECTION (x,y,z): 			" << trueFliDir.X()
                        << "	,	" << trueFliDir.Y() << "	,	" << trueFliDir.Z() << std::endl;
  streamlog_out(DEBUG4) << "	TRUE VISIBLE FOUR-MOMENTUM (Px,Py,Pz,E):	" << true4mom.Px() << "	,	"
                        << true4mom.Py() << "	,	" << true4mom.Pz() << "	,	" << true4mom.E() << std::endl;
  TVector3 truePvisPar = trueFliDir.Dot(true4mom.Vect()) * trueFliDir;
  TVector3 truePvisNor = true4mom.Vect() - truePvisPar;
  streamlog_out(DEBUG4) << "	TRUE VISIBLE FOUR-MOMENTUM(par) (Px,Py,Pz):	" << truePvisPar.Px()
...
TLorentzVector SLDCorrection::getNeutrinoFourMomentum(const TVector3& flightDirection,
                                                      const TLorentzVector& visibleFourMomentum,
  ...
  streamlog_out(DEBUG4) << "		flightDirection:			( " << flightDirection.Px()
                        << "	, " << flightDirection.Py() << "	, " << flightDirection.Pz() << "	)"
                        << std::endl;
  ...
  streamlog_out(DEBUG4)
      << "		Visible Energy:									" << visible_E
      << std::endl;

  TVector3 visible_p = TVector3(visibleFourMomentum.Px(), visibleFourMomentum.Py(), visibleFourMomentum.Pz());
  streamlog_out(DEBUG4) << "		Visible Momentum:			( " << visible_p.Px() << "	, "
                        << visible_p.Py() << "	, " << visible_p.Pz() << "	)" << std::endl;
  streamlog_out(DEBUG4)
      << "		(Visible Momentum).(FlightDirection):							"
      << visible_p.Dot(flightDirection) << std::endl;

  TVector3 visible_p_par = visible_p.Dot(flightDirection) * flightDirection;
  m_P_vis_par.push_back(visible_p_par.Mag());
  streamlog_out(DEBUG4) << "		Visible Momentum (par):			( " << visible_p_par.Px() << "	, "
                        << visible_p_par.Py() << "	, " << visible_p_par.Pz() << "	)" << std::endl;
  ...
  streamlog_out(DEBUG4) << "		Visible Momentum (nor):			( " << visible_p_nor.Px() << "	, "
                        << visible_p_nor.Py() << "	, " << visible_p_nor.Pz() << "	)" << std::endl;
  ...
  streamlog_out(DEBUG4)
      << "		Visible Energy (prime):								"
      << visible_E_prime << std::endl;

  TVector3 visible_p_par_prime =
      solutionSign *
      sqrt(pow((pow(ParentHadronMass, 2) - pow(visible_mass, 2)) / (2 * ParentHadronMass), 2) - visible_p_nor.Mag2()) *
      flightDirection;
  streamlog_out(DEBUG4) << "		Visible Momentum (par-prime):		( " << visible_p_par_prime.Px()
                        << "	, " << visible_p_par_prime.Py() << "	, " << visible_p_par_prime.Pz() << "	)"
                        << std::endl;
  ...
  double sigma_parent_hadron_E2 =
      pow(ParentHadronMass, 2) *
      (pow(visible_E_prime / (pow(visible_mass, 2) + pow(visible_p_nor.Mag(), 2)), 2) * pow(sigma_E_vis, 2) +
       ...
       pow(visible_p_par_prime.Mag() / (pow(visible_mass, 2) + pow(visible_p_nor.Mag(), 2)), 2) *
           pow(sigma_p_vis_par, 2) +
       pow(visible_p_par.Mag() / (pow(visible_mass, 2) + pow(visible_p_nor.Mag(), 2)), 2) *
           pow(sigma_p_vis_par_prime, 2) +
       pow(2 * visible_p_nor.Mag() * (visible_E_prime * visible_E - visible_p_par_prime.Mag() * visible_p_par.Mag()) /
               pow((pow(visible_mass, 2) + pow(visible_p_nor.Mag(), 2)), 2),
           2) *
           pow(sigma_p_vis_nor, 2));
  streamlog_out(DEBUG4)
      << "		Parent Hadron Sigma_E =									"
      << sigma_parent_hadron_E2 << std::endl;
  ...
  streamlog_out(DEBUG4) << "		Neutrino Momentum (nor):		( " << Neutrino_p_nor.Px() << "	, "
                        << Neutrino_p_nor.Py() << "	, " << Neutrino_p_nor.Pz() << "	)" << std::endl;

  TVector3 Neutrino_p_par = sqrt(pow(Neutrino_E, 2) - Neutrino_p_nor.Mag2()) * flightDirection;
  streamlog_out(DEBUG4) << "		Neutrino Momentum (par):		( " << Neutrino_p_par.Px() << "	, "
                        << Neutrino_p_par.Py() << "	, " << Neutrino_p_par.Pz() << "	)" << std::endl;

  TVector3 Neutrino_p = Neutrino_p_nor + Neutrino_p_par;
  streamlog_out(DEBUG4) << "		Neutrino Momentum:			( " << Neutrino_p_par.Px() << "	, "
                        << Neutrino_p_par.Py() << "	, " << Neutrino_p_par.Pz() << "	)" << std::endl;
  ...
  streamlog_out(DEBUG2) << "	Reconstructed Neutrino 4-Momentum:		( " << Neutrino_tlv.Px() << "	, "
                        << Neutrino_tlv.Py() << "	, " << Neutrino_tlv.Pz() << "	, " << Neutrino_tlv.E() << " )"
                        << std::endl;
  return Neutrino_tlv;
}

TLorentzVector SLDCorrection::getNeutrinoFourMomentumModified(TVector3& flightDirection,
                                                              const TLorentzVector& visibleFourMomentum,
                                                              const double& ParentHadronMass,
                                                              const float& solutionSign) {
  int sign = (solutionSign != 0 ? solutionSign / abs(solutionSign) : 1);
  m_solutionSign.push_back(sign);
  const char* solSign = (sign >= 0 ? "+" : "-");
  // clang-format off
  ...
  flightDirection.SetMag(1.0);
  streamlog_out(DEBUG9) << "		flightDirection:			( " << flightDirection.Px()
                        << "	, " << flightDirection.Py() << "	, " << flightDirection.Pz() << "	)"
                        << std::endl;
  ...
  TVector3 visible_p = TVector3(visibleFourMomentum.Px(), visibleFourMomentum.Py(), visibleFourMomentum.Pz());
  streamlog_out(DEBUG9) << "		Visible Momentum:			( " << visible_p.Px() << "	, "
                        << visible_p.Py() << "	, " << visible_p.Pz() << "	)" << std::endl;
  streamlog_out(DEBUG9)
      << "		(Visible Momentum).(FlightDirection):							"
      << visible_p.Dot(flightDirection) << std::endl;
  ...
  streamlog_out(DEBUG9) << "		Visible Momentum (par):			( " << visible_p_par.Px() << "	, "
                        << visible_p_par.Py() << "	, " << visible_p_par.Pz() << "	)" << std::endl;
  ...
  streamlog_out(DEBUG9) << "		Visible Momentum (nor):			( " << visible_p_nor.Px() << "	, "
                        << visible_p_nor.Py() << "	, " << visible_p_nor.Pz() << "	)" << std::endl;
  ...
  double visible_p_par_prime_squared =
      pow((pow(ParentHadronMass, 2) - pow(visible_mass, 2)) / (2 * ParentHadronMass), 2) - visible_p_nor.Mag2();
  streamlog_out(DEBUG9) << "		Visible Momentum (par-prime)2:		= " << visible_p_par_prime_squared
                        << std::endl;
  m_P_vis_par_prime_squared.push_back(visible_p_par_prime_squared);
  if (visible_p_par_prime_squared < 0.0) {
    TVector3 visible_p_nor_direction = visible_p_nor;
    visible_p_nor_direction.SetMag(1.0);
    double visible_p_nor_corrected = (pow(ParentHadronMass, 2) - pow(visible_mass, 2)) / (2 * ParentHadronMass);
    streamlog_out(DEBUG9) << "		|corrected visible_p_nor|=		" << visible_p_nor_corrected
                          << std::endl;
    double visible_p_par_corrected = sqrt(visible_p.Mag2() - pow(visible_p_nor_corrected, 2));
    streamlog_out(DEBUG9) << "		|corrected visible_p_par|=		" << visible_p_par_corrected
                          << std::endl;
    TVector3 visible_p_direction = visible_p;
    visible_p_direction.SetMag(1.0);
    ...
    streamlog_out(DEBUG9) << "		new flightDirection:		( " << flightDirection.Px() << "	, "
                          << flightDirection.Py() << "	, " << flightDirection.Pz() << "	)" << std::endl;
  ...
  m_P_vis_par_prime.push_back(visible_p_par_prime.Mag());
  streamlog_out(DEBUG9) << "		Visible Momentum (par-prime):		( " << visible_p_par_prime.Px()
                        << "	, " << visible_p_par_prime.Py() << "	, " << visible_p_par_prime.Pz() << "	)"
                        << std::endl;

  streamlog_out(DEBUG9) << "		|Visible Momentum (par-prime)|:	" << visible_p_par_prime.Mag() << std::endl;

  double parent_hadron_E = ((visibleFourMomentum.E() * visible_E_prime) - visible_p_par.Dot(visible_p_par_prime)) *
                           ParentHadronMass / (pow(visible_mass, 2) + visible_p_nor.Mag2());
  streamlog_out(DEBUG9)
      << "		Parent Hadron Energy =									"
      << parent_hadron_E << std::endl;
  ...
  streamlog_out(DEBUG9) << "		Neutrino Momentum (nor):		( " << Neutrino_p_nor.Px() << "	, "
                        << Neutrino_p_nor.Py() << "	, " << Neutrino_p_nor.Pz() << "	) = " << Neutrino_p_nor.Mag()
                        << std::endl;

  TVector3 Neutrino_p_par = parent_hadron_p - visible_p_par;
  streamlog_out(DEBUG9) << "		Neutrino Momentum (par):		( " << Neutrino_p_par.Px() << "	, "
                        << Neutrino_p_par.Py() << "	, " << Neutrino_p_par.Pz()
                        << "	) = " << parent_hadron_p.Mag() - visible_p_par.Mag() << std::endl;

  TVector3 Neutrino_p = Neutrino_p_nor + Neutrino_p_par;
  streamlog_out(DEBUG9) << "		Neutrino Momentum:			( " << Neutrino_p.Px() << "	, "
                        << Neutrino_p.Py() << "	, " << Neutrino_p.Pz() << "	)" << std::endl;
  ...
  streamlog_out(DEBUG9) << "	Reconstructed Neutrino 4-Momentum:		( " << Neutrino_tlv.Px() << "	, "
                        << Neutrino_tlv.Py() << "	, " << Neutrino_tlv.Pz() << "	, " << Neutrino_tlv.E() << " )"
                        << std::endl;
  return Neutrino_tlv;
}

TLorentzVector SLDCorrection::getNeutrinoFourMomentumStandardMethod(const TVector3& flightDirection,
                                                                    const TLorentzVector& visibleFourMomentum,
                                                                    const double& parentHadronMass,
                                                                    const float& solutionSign) {
  int sign = (solutionSign != 0 ? solutionSign / abs(solutionSign) : 1);
  ...
  streamlog_out(DEBUG8) << "		(A) ( M_B^2 - M_vis^2 - 2 * P_vis(nor)^2 ) * P_vis(par) =	" << A <<
  std::endl; double B = 4.0 * pow( P_vis_nor , 2 ) * pow( visible_E , 2 ) - pow( pow( parentHadronMass , 2 ) - pow(
  visible_mass , 2 ) - 2.0 * pow( P_vis_nor , 2 ) , 2 ); streamlog_out(DEBUG8) << "		(B) 4 * E_vis^2 *
  P_vis(par)^2 - ( M_B^2 - M_vis^2 - 2 * P_vis(nor)^2 )^2 =	" << B << std::endl;
  */
  ...
  streamlog_out(DEBUG8) << "		(B) ( M_B^2 - M_vis^2 - 2 * P_vis(nor)^2 ) * P_vis(par) =	" << B
                        << std::endl;
  double C = pow(P_vis_nor, 2) * pow(visible_E, 2) - pow(Q, 2) / 4.0;
  streamlog_out(DEBUG8)
      << "		(C) E_vis^2 * P_vis(nor)^2 - ( M_B^2 - M_vis^2 - 2 * P_vis(nor)^2 )^2 / 4 =	" << C
      << std::endl;

  double P_nu_par =
      (pow(B, 2) >= 4.0 * A * C ? (B + solutionSign * std::sqrt(pow(B, 2) - 4.0 * A * C)) / (2.0 * A) : B / (2.0 * A));
  streamlog_out(DEBUG8) << "		|Neutrino Momentum (par)|=		" << P_nu_par << std::endl;
  TVector3 Neutrino_p_par = P_nu_par * flightDirection;
  streamlog_out(DEBUG8) << "		Neutrino Momentum (par):		( " << Neutrino_p_par.Px() << "	, "
                        << Neutrino_p_par.Py() << "	, " << Neutrino_p_par.Pz() << "	)" << std::endl;

  TVector3 Neutrino_p_nor = -1.0 * visible_p_nor;
  streamlog_out(DEBUG8) << "		Neutrino Momentum (nor):		( " << Neutrino_p_nor.Px() << "	, "
                        << Neutrino_p_nor.Py() << "	, " << Neutrino_p_nor.Pz() << "	)" << std::endl;
  TVector3 Neutrino_p = Neutrino_p_nor + Neutrino_p_par;
  streamlog_out(DEBUG8) << "		Neutrino Momentum:			( " << Neutrino_p.Px() << "	, "
                        << Neutrino_p.Py() << "	, " << Neutrino_p.Pz() << "	)" << std::endl;
  double Neutrino_E = Neutrino_p.Mag();
  streamlog_out(DEBUG8)
      << "		Neutrino Energy:									"
  ...
  streamlog_out(DEBUG4) << "	Reconstructed Neutrino 4-Momentum:		( " << Neutrino_tlv.Px() << "	, "
                        << Neutrino_tlv.Py() << "	, " << Neutrino_tlv.Pz() << "	, " << Neutrino_tlv.E() << " )"
                        << std::endl;
  return Neutrino_tlv;
}
...
void SLDCorrection::fillTrueRecoFourMomentum(
    const TLorentzVector& trueVisibleFourMomentumAtSLDVertex, const TLorentzVector& truePVATrueFourMomentum,
    const TLorentzVector& truePVARecoFourMomentum, const TLorentzVector& recoPVARecoFourMomentum,
    const TLorentzVector& visibleFourMomentum, const TLorentzVector& trueLeptonFourMomentum,
    const TLorentzVector& recoLeptonFourMomentum, const TLorentzVector& leptonFourMomentum,
    const TLorentzVector& truePVATrueChargedFourMomentum, const TLorentzVector& truePVARecoChargedFourMomentum,
    const TLorentzVector& recoSLDVertexChargedFourMomentum, const TLorentzVector& recoPVARecoChargedFourMomentum,
    const TLorentzVector& chargedFourMomentum, const TLorentzVector& truePVATrueNeutralFourMomentum,
    const TLorentzVector& truePVARecoNeutralFourMomentum, const TLorentzVector& recoPVARecoNeutralFourMomentum,
    const TLorentzVector& neutralFourMomentum, const TLorentzVector& trueNeutrinoFourMomentum,
    const TLorentzVector& recoNeutrinoFourMomentumClose, const TLorentzVector& trueHadronFourMomentum,
    const TLorentzVector& recoHadronFourMomentum, const TVector3& trueFlightDirection,
    const TVector3& recoFlightDirection, const TVector3& flightDirection) {
  ...
  m_allChargedE_to_parentE.push_back((trueLeptonFourMomentum + truePVATrueChargedFourMomentum).E() /
                                     trueHadronFourMomentum.E());
  ...
  TVector3 recoPVARecoDirection = recoPVARecoFourMomentum.Vect();
  recoPVARecoDirection.SetMag(1.0);
  TVector3 truePVARecoDirection = truePVARecoFourMomentum.Vect();
  truePVARecoDirection.SetMag(1.0);
  ...
  TVector3 recoPVARecoChargedDirection = (recoSLDVertexChargedFourMomentum + recoPVARecoChargedFourMomentum).Vect();
  recoPVARecoChargedDirection.SetMag(1.0);
  TVector3 truePVARecoChargedDirection = truePVARecoChargedFourMomentum.Vect();
  truePVARecoChargedDirection.SetMag(1.0);
  if (truePVARecoChargedFourMomentum.E() > 0.0 &&
      (recoSLDVertexChargedFourMomentum + recoPVARecoChargedFourMomentum).E() > 0.0) {
  ...
  TLorentzVector recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum =
      recoPVARecoFourMomentum - recoLeptonFourMomentum - truePVARecoNeutralFourMomentum;
  TLorentzVector recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum =
      recoPVARecoFourMomentum - recoLeptonFourMomentum - truePVARecoChargedFourMomentum;
  m_recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum_Px.push_back(
      recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum.Px());
  m_recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum_Py.push_back(
      recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum.Py());
  m_recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum_Pz.push_back(
      recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum.Pz());
  m_recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum_E.push_back(
      recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum.E());
  m_recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum_M.push_back(
      recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum.M());
  m_recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum_Px.push_back(
      recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum.Px());
  m_recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum_Py.push_back(
      recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum.Py());
  m_recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum_Pz.push_back(
      recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum.Pz());
  m_recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum_E.push_back(
      recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum.E());
  m_recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum_M.push_back(
      recoPVARecoFourMomentum_minus_truePVARecoChargedFourMomentum.M());

  TVector3 recoPVARecoNeutralDirection = recoPVARecoNeutralFourMomentum.Vect();
  recoPVARecoNeutralDirection.SetMag(1.0);
  TVector3 truePVARecoNeutralDirection = truePVARecoNeutralFourMomentum.Vect();
  truePVARecoNeutralDirection.SetMag(1.0);
  ...
  TVector3 recoPVARecoMomentum_minus_truePVARecoNeutralMomentum =
      recoPVARecoFourMomentum_minus_truePVARecoNeutralFourMomentum.Vect();
  ...
  m_SinAlpha_RecoPVARecoAll_minus_TruePVARecoNeutral_vs_recoPVArecoCharged.push_back(
      sin(acos(recoPVARecoMomentum_minus_truePVARecoNeutralMomentum_Direction.Dot(recoPVARecoChargedDirection))));
...
void SLDCorrection::investigateJetEnergyContent(const RecoParticle& assignedJet) {
...
void SLDCorrection::plotHistograms(const TLorentzVector& trueFourMomentumNeutrino,
                                   const TLorentzVector& FourMomentumNuClose, const FloatVector& NeutrinoCovMat) {
  double NuPxResidual = FourMomentumNuClose.Px() - trueFourMomentumNeutrino.Px();
  m_NuPxResidual.push_back(NuPxResidual);
  double NuPyResidual = FourMomentumNuClose.Py() - trueFourMomentumNeutrino.Py();
  m_NuPyResidual.push_back(NuPyResidual);
  double NuPzResidual = FourMomentumNuClose.Pz() - trueFourMomentumNeutrino.Pz();
  m_NuPzResidual.push_back(NuPzResidual);
  double NuEResidual = FourMomentumNuClose.E() - trueFourMomentumNeutrino.E();
  m_NuEResidual.push_back(NuEResidual);
  double NuPxNormalizedResidual = NuPxResidual / sqrt(NeutrinoCovMat[0]);
  m_NuPxNormalizedResidual.push_back(NuPxNormalizedResidual);
  double NuPyNormalizedResidual = NuPyResidual / sqrt(NeutrinoCovMat[2]);
  m_NuPyNormalizedResidual.push_back(NuPyNormalizedResidual);
  double NuPzNormalizedResidual = NuPzResidual / sqrt(NeutrinoCovMat[5]);
  m_NuPzNormalizedResidual.push_back(NuPzNormalizedResidual);
  double NuENormalizedResidual = NuEResidual / sqrt(NeutrinoCovMat[9]);
  m_NuENormalizedResidual.push_back(NuENormalizedResidual);
  h_NuPxResidual->Fill(NuPxResidual);
  h_NuPyResidual->Fill(NuPyResidual);
  h_NuPzResidual->Fill(NuPzResidual);
  h_NuEResidual->Fill(NuEResidual);
  h_NuPxNormalizedResidual->Fill(NuPxNormalizedResidual);
  h_NuPyNormalizedResidual->Fill(NuPyNormalizedResidual);
  h_NuPzNormalizedResidual->Fill(NuPzNormalizedResidual);
  h_NuENormalizedResidual->Fill(NuENormalizedResidual);
...
void SLDCorrection::InitializeHistogram(TH1F* histogram, int scale, int color, int lineWidth, int markerSize,
                                        int markerStyle) {
...
void SLDCorrection::check(EVENT::LCEvent* pLCEvent) {
  try {
    streamlog_out(DEBUG4) << "	CHECK : processed events: " << m_nEvtSum << " , Number of semi-leptonic vertex: "
                          << pLCEvent->getCollection(m_SLDVertex)->getNumberOfElements() << std::endl;
    streamlog_out(DEBUG4) << "	CHECK : processed events: " << m_nEvtSum << " , Number of semi-leptonic vertexRP: "
                          << pLCEvent->getCollection(m_SLDVertexRP)->getNumberOfElements() << std::endl;
    streamlog_out(DEBUG4) << "	CHECK : processed events: " << m_nEvtSum << " , Number of Reconstrcuted Neutrinos: "
                          << pLCEvent->getCollection(m_reconstructedNeutrino)->getNumberOfElements() << std::endl;
  } catch (DataNotAvailableException& e) {
...
void SLDCorrection::end() {
  if (m_fillRootTree) {
    ...
    h_BHadronType->Write();
    h_CHadronType->Write();
    InitializeHistogram(h_NuPxResidual, h_NuPxResidual->GetEntries(), 4, 1, 1.0, 1);
    InitializeHistogram(h_NuPyResidual, h_NuPyResidual->GetEntries(), 4, 1, 1.0, 1);
    InitializeHistogram(h_NuPzResidual, h_NuPzResidual->GetEntries(), 4, 1, 1.0, 1);
    InitializeHistogram(h_NuEResidual, h_NuEResidual->GetEntries(), 4, 1, 1.0, 1);
    InitializeHistogram(h_NuPxNormalizedResidual, h_NuPxNormalizedResidual->GetEntries(), 4, 1, 1.0, 1);
    InitializeHistogram(h_NuPyNormalizedResidual, h_NuPyNormalizedResidual->GetEntries(), 4, 1, 1.0, 1);
    InitializeHistogram(h_NuPzNormalizedResidual, h_NuPzNormalizedResidual->GetEntries(), 4, 1, 1.0, 1);
    InitializeHistogram(h_NuENormalizedResidual, h_NuENormalizedResidual->GetEntries(), 4, 1, 1.0, 1);
