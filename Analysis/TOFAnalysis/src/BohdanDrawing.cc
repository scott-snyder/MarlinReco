...
void plotECALTimes(EVENT::Cluster* cluster, Vector3D posAtEcal, Vector3D momAtEcal, MCParticle* mc) {
  ...
  TF1 f("f", Form("%f + (x-%f)/%f", y_all_smeared.front(), x_all.front(), CLHEP::c_light), 0.,
        *std::max_element(x_all.begin(), x_all.end()));
  ...
  float average =
      std::reduce(y_frank_smeared_corr.begin(), y_frank_smeared_corr.end()) / float(y_frank_smeared_corr.size());
...
void plotTrackParams(const std::vector<HitState>& trackHitStates, EVENT::MCParticle* mc, float bField) {
  ...
  myStyle->cd();
  gROOT->ForceStyle();
  TCanvas c = TCanvas("c", "Track parameters evolution through the track", 600 / (1. - margin.left - margin.right),
                      600 / (1. - margin.top - margin.bottom));
  c.SetGridx(true);
  ...
  auto axis = TGaxis(c.GetUxmax(), c.GetUymax(), c.GetUxmax(), c.GetUymin(), omega2pt(c.GetUymax()),
                     omega2pt(c.GetUymin()), 510, "L-");
  ...
  // COPYPASTE for TanL
  TCanvas c2 = TCanvas("c2", "Track parameters evolution through the track", 600 / (1. - margin.left - margin.right),
                       600 / (1. - margin.top - margin.bottom));
  c2.SetGridx(true);
  ...
  // draw an axis on the right side
  auto axis2 = TGaxis(c2.GetUxmax(), c2.GetUymax(), c2.GetUxmax(), c2.GetUymin(), tanL2theta(c2.GetUymax()),
                      tanL2theta(c2.GetUymin()), 510, "L-");
