...
TauFinder::TauFinder() : Processor("TauFinder") {
  ...
  _description = "TauFinder writes tau candidates as ReconstructedParticles into collection. It runs on a collection "
                 "of ReconstructedParticels, if you want  to run on MCParticles you have to convert them before hand "
                 "(use e.g. PrepareRECParticles processor)";

  // register steering parameters: name, description, class-variable, default value

  // the link between the reconstructed tau and the input particles used for it

  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "PFOCollection", "Collection of PFOs", _incol,
                          std::string("PandoraPFANewPFOs"));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "TauRecCollection", "Collection of Tau Candidates", _outcol,
                           std::string("TauRec_PFO"));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "TauRecRestCollection",
                           "Collection of Particles in Rest Group not in Tau Candidates", _outcolRest,
                           std::string("TauRecRest_PFO"));

  registerOutputCollection(LCIO::LCRELATION, "TauRecLinkCollectionName",
                           "Name of the Tau link to ReconstructedParticle collection", _colNameTauRecLink,
                           std::string("TauRecLink_PFO"));

  registerProcessorParameter("FileName_Signal", "Name of the Signal output file ", _OutputFile_Signal,
                             std::string("Signal.root"));

  registerProcessorParameter("pt_cut", "Cut on pt to suppress background", _ptcut, (float)0.2);
  registerProcessorParameter("cosT_cut", "Cut on cosT to suppress background", _cosTcut, (float)0.99);

  registerProcessorParameter("searchConeAngle", "Opening angle of the search cone for tau jet in rad", _coneAngle,
                             (float)0.05);
  ...
  registerProcessorParameter("isolationEnergy", "Energy allowed within isolation cone region", _isoE, (float)5.0);

  registerProcessorParameter("ptseed", "Minimum tranverse momentum of tau seed", _ptseed, (float)5.0);

  registerProcessorParameter("invariant_mass", "Upper limit on invariant mass of tau candidate", _minv, (float)2.0);
}
...
void TauFinder::processEvent(LCEvent* evt) {
  ...
  if (colRECO != 0) {
    ...
    for (int i = 0; i < nRCP; i++) {
      ...
      double Cos_T = fabs(particle->getMomentum()[2]) /
                     sqrt(pow(particle->getMomentum()[0], 2) + pow(particle->getMomentum()[1], 2) +
  ...
  for (unsigned int p = 0; p < tauvec.size(); p++) {
    ...
    if (mass_inv > _minv || mass_inv < -0.001 || chargedtracks > 4 || chargedtracks == 0) {
      ...
      if (_nEvt < coutUpToEv || _nEvt == coutEv) {
        ...
        streamlog_out(DEBUG) << "Tau candidate failed: minv=" << mass_inv << "   pt=" << pt_tau << " " << E
                             << " Q trks:" << chargedtracks << " N trks:" << neutraltracks << " " << phi << " " << theta
                             << endl;
      }
      delete taurec;
      continue;
    ...
    if (_nEvt < coutUpToEv || _nEvt == coutEv) {
      double phi = 180. / TMath::Pi() * atan(mom[1] / mom[0]);
      double theta = 180. / TMath::Pi() * atan(pt_tau / fabs(mom[2]));
      streamlog_out(DEBUG) << "Tau candidate " << p << ": " << E << " Q trks:" << chargedtracks
                           << " N trks:" << neutraltracks << " " << phi << " " << theta << endl;
    }
      ...
      for (unsigned int t2 = t + 1; t2 < tauRecvec.size(); t2++) {
        ...
        if (angle < _coneAngle) {
          ...
          if (_nEvt < coutUpToEv || _nEvt == coutEv) {
            ...
            streamlog_out(DEBUG) << t2 << " " << taun->getEnergy() << " " << angle << " -> " << En << " "
  ...
  for (unsigned int t = 0; t < tauRecvec.size(); t++) {
    ...
    if (QTvec[t + erasecount] + NTvec[t + erasecount] > 10 || QTvec[t + erasecount] > 4) {
      _fail_Qtr++;
      if (_nEvt < coutUpToEv || _nEvt == coutEv)
        streamlog_out(DEBUG) << "Tau " << tau->getEnergy() << ": too many particles: " << QTvec[t + erasecount] << " "
    ...
    if (E_iso > _isoE) {
      _fail_isoE++;
      if (_nEvt < coutUpToEv || _nEvt == coutEv)
        streamlog_out(DEBUG) << "Tau " << tau->getEnergy() << ": Isolation Energy: " << E_iso << " in " << nparticles
                             << " particles" << endl;
      // put those particles into rest group
      for (unsigned int tp = 0; tp < (*iter)->getParticles().size(); tp++)
        restcol->addElement((*iter)->getParticles()[tp]);
      ...
      streamlog_out(DEBUG) << "Tau " << tau->getEnergy() << " " << QTvec[t + erasecount] << " " << NTvec[t + erasecount]
                           << endl;
      iter++;
    }
  }
...
bool TauFinder::FindTau(std::vector<ReconstructedParticle*>& Qvec, std::vector<ReconstructedParticle*>& Nvec,
  ...
  // just for printing out info
  {
    ...
    if (_nEvt < coutUpToEv || _nEvt == coutEv)
      streamlog_out(DEBUG) << "seeding: " << tauseed->getType() << "\t" << tauseed->getEnergy() << "\t" << p << "\t"
                           << theta << "\t" << phi << endl;
  }
  ...
  for (unsigned int s = 0; s < Qvec.size(); s++) {
    ...
    if (angle < _coneAngle) {
      ...
      if (_nEvt < coutUpToEv || _nEvt == coutEv)
        streamlog_out(DEBUG) << "Adding Q: " << track->getType() << "\t" << track->getEnergy() << "\t" << p << "\t"
                             << theta << "\t" << phi << std::endl;
  ...
  for (unsigned int s = 0; s < Nvec.size(); s++) {
    ...
    if (angle < _coneAngle) {
      ...
      if (_nEvt < coutUpToEv || _nEvt == coutEv)
        streamlog_out(DEBUG) << "Adding N: " << track->getType() << "\t" << track->getEnergy() << "\t" << p << "\t"
                             << theta << "\t" << phi << std::endl;
...
void TauFinder::end() {
  ...
  streamlog_out(MESSAGE) << "TauFinder::end()  " << name() << " processed " << _nEvt << " events in " << _nRun
                         << " runs " << std::endl;
