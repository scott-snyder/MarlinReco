...
TrueJet::TrueJet() : Processor("TrueJet") {
  ...
  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "TrueJets", "Name of the TrueJetCollection output collection",
                           _trueJetCollectionName, std::string("TrueJets"));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "FinalColourNeutrals",
  ...
  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "InitialColourNeutrals",
  ...
  registerOutputCollection(LCIO::LCRELATION, "TrueJetPFOLink", "Name of the TrueJetPFOLink output collection",
                           _trueJetPFOLink, std::string("TrueJetPFOLink"));

  registerOutputCollection(LCIO::LCRELATION, "TrueJetMCParticleLink",
                           "Name of the TrueJetMCParticleLink output collection", _trueJetMCParticleLink,
                           std::string("TrueJetMCParticleLink"));

  registerOutputCollection(LCIO::LCRELATION, "FinalElementonLink", "Name of the  FinalElementonLink output collection",
                           _finalElementonLink, std::string("FinalElementonLink"));

  registerOutputCollection(LCIO::LCRELATION, "InitialElementonLink",
                           "Name of the  InitialElementonLink output collection", _initialElementonLink,
                           std::string("InitialElementonLink"));

  registerOutputCollection(LCIO::LCRELATION, "FinalColourNeutralLink",
                           "Name of the  FinalColourNeutralLink output collection", _finalColourNeutralLink,
                           std::string("FinalColourNeutralLink"));

  registerOutputCollection(LCIO::LCRELATION, "InitialColourNeutralLink",
                           "Name of the  InitialColourNeutralLink output collection", _initialColourNeutralLink,
                           std::string("InitialColourNeutralLink"));
  // steering

  registerProcessorParameter("Whizard1", "true: Input has MCParticles in Whizard1 convention ", _whiz1, bool(false));
}
...
void TrueJet::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }
...
void TrueJet::processEvent(LCEvent* event) {
  ...
  if (mcpcol != NULL) {
    ...
    // At this point, the job is done. All the rest is printouts, putting things into collections and setting up
    // navigators. Non-debuggung code is beween ******:s and =========:s
    ...
    if (!((n_mixed == 0) && njet == 2 * nstr + n_hard_lepton + 2 * nclu + nisr + n_beam_jet + nphot &&
      ...
      if (!_higgs_to_glue_glue) {
        ...
        streamlog_out(ERROR) << n_mixed << " / " << njet << " / "
                             << 2 * nstr + n_hard_lepton + 2 * nclu + nisr + n_beam_jet << " / " << n_jetless << " / "
                             << n_hard_lepton << std::endl;
    ...
    for (int k_py = 1; k_py <= nlund; k_py++) {
      streamlog_out(DEBUG3) << std::setw(7) << k_py << std::setw(7) << k[k_py][1] << std::setw(12) << k[k_py][2]
                            << std::setw(7) << k[k_py][3] << std::setw(7) << k[k_py][4] << std::setw(7) << k[k_py][5]
                            << std::setw(12) << p[k_py][1] << std::setw(12) << p[k_py][2] << std::setw(12) << p[k_py][3]
                            << std::setw(12) << p[k_py][4] << std::setw(12) << p[k_py][5] <<

          std::setw(7) << jet[k_py] << std::setw(7) << companion[abs(jet[k_py])] << std::setw(7) << type[abs(jet[k_py])]
                            << std::endl;
    }
    streamlog_out(DEBUG3)
        << "       jet     fafp-beg  qrk/lept  fafp-end fafp-boson  type   dijet-beg  dijet-end    boson" << std::endl;
    for (int k_py = 1; k_py <= njet; k_py++) {
      streamlog_out(DEBUG3) << std::setw(10) << k_py << std::setw(10) << fafp[k_py] << std::setw(10) << elementon[k_py]
                            << std::setw(10) << fafp_last[k_py] << std::setw(10) << fafp_boson[k_py] << std::setw(10)
                            << type[k_py] << std::setw(10) << dijet_begining[k_py] << std::setw(10) << dijet_end[k_py]
                            << std::setw(10) << boson[k_py] << std::endl;
    }
    ...
    for (int i_jet = 1; i_jet <= njet; i_jet++) { // jet-loop
      ...
      streamlog_out(DEBUG1)
          << "       line    mcp          status    pdg       parent    first     last       jet     pfo(s)"
          << std::endl;
      streamlog_out(DEBUG1) << "                                                        daughter   daugther "
                            << std::endl;
    }
    ...
    for (int k_py = 1; k_py <= nlund; k_py++) { // pyjets loop
      ...
      if (i_jet > 0) {
        ...
        streamlog_out(DEBUG1) << std::setw(10) << k_py << " (" << mcp_pyjets[k_py] << ")" << std::setw(10)
                              << k[k_py][1] % 30 << std::setw(10) << k[k_py][2] << std::setw(10) << k[k_py][3]
                              << std::setw(10) << k[k_py][4] << std::setw(10) << k[k_py][5] << std::setw(10)
                              << jet[k_py];
        ...
        //              if ( abs(k[k_py][2]) != 12 &&  abs(k[k_py][2]) != 14 &&  abs(k[k_py][2]) != 16 ) { // not
        //              neutrino
        ...
        if (recovec.size() > 0) { // if reconstructed
          ...
          if (!seen[k[k_py][3]]) {
            ...
            streamlog_out(DEBUG3) << " Ancestor of true particle " << k_py
            ...
            if (truejet_pfo_Nav.getRelatedFromObjects(reco_part).size() == 0) { // only if not yet used
              ...
              if (split_between_jets) {
                ...
                for (int j_jet = 1; j_jet <= njet; j_jet++) {
                  streamlog_out(DEBUG3) << "     jetweights for " << j_jet << " " << wgt_trk[j_jet] << " "
                                        << wgt_clu[j_jet];
                  if (wgt_trk[j_jet] > wgt_trk_max) {
                    imax_trk = j_jet;
                    wgt_trk_max = wgt_trk[j_jet];
                  }
                  if (wgt_clu[j_jet] > wgt_clu_max) {
                    imax_clu = j_jet;
                    wgt_clu_max = wgt_clu[j_jet];
                  }
                }
                streamlog_out(DEBUG3) << "    " << imax_clu << " " << imax_trk << " " << wgt_clu_max << " "
                                      << wgt_trk_max;
                if (imax_clu == imax_trk || wgt_trk_max >= 750) {
                  ...
                  streamlog_out(DEBUG3) << " was ? " << imax_clu << " " << imax_trk << " " << wgt_clu_max << " "
                                        << wgt_trk_max << std::endl;
                  for (int j_jet = 1; j_jet <= njet; j_jet++) {
                    streamlog_out(DEBUG3) << " was    jetweights for " << j_jet << " " << wgt_trk[j_jet] << " "
                                          << wgt_clu[j_jet] << std::endl;
                  }
                  // probably the cluster is somewhat more reliable in this case. Anyways, this happens very rarely (for
                  // 3 PFOs in 6400 4f_had...)
                  if (imax_clu > 0) {
                    winner = imax_clu;
                  } else {
                    // ... as a last resort ...
              ...
              if (winner > 0 && winner != last_winner)
                true_jet = dynamic_cast<ReconstructedParticleImpl*>(jet_vec->getElementAt(winner - 1));
              ...
              if (winner > 0) {
                ...
                // FIXME: a follow on error for h->glueglue: If a particle from a non-higgs jet gets reconstructed into
                // a PFO where
                //   the majority contribution is from a higgs jet (which are not found in  h->glueglue, and hence has
                //   jet# 0 ).
                if (!_higgs_to_glue_glue) {
                  streamlog_out(WARNING) << " reco particle " << i_reco << " of pythia-particle " << k_py
                                         << " has weights = 0 to ALL MCPs ??? " << std::endl;
                  streamlog_out(WARNING) << " Event: " << evt->getEventNumber() << ",   run:  " << evt->getRunNumber()
                                         << std::endl;
                }
              }
            } // end if not yet used
    ...
    for (int i_jet = 1; i_jet <= njet; i_jet++) { // jet-loop
      ...
      streamlog_out(DEBUG5) << "  jet       " << std::setw(4) << i_jet << " (seen)         p : " << mom[0] << " "
                            << mom[1] << " " << mom[2] << " " << " E " << true_jet->getEnergy() << std::endl;
      streamlog_out(DEBUG5) << "               " << "  (true)         p : " << tmom[i_jet][0] << " " << tmom[i_jet][1]
                            << " " << tmom[i_jet][2] << " " << " E " << tE[i_jet] << std::endl;
      streamlog_out(DEBUG5) << "               " << "  (true of seen) p : " << tmomS[i_jet][0] << " " << tmomS[i_jet][1]
                            << " " << tmomS[i_jet][2] << " " << " E " << tES[i_jet] << std::endl;
      streamlog_out(DEBUG5) << "  ancestor 1" << std::setw(4) << elementon[i_jet]
                            << "                p : " << p[elementon[i_jet]][1] << " " << p[elementon[i_jet]][2] << " "
                            << p[elementon[i_jet]][3] << " " << " E " << p[elementon[i_jet]][4] << std::endl;
      streamlog_out(DEBUG5) << "  ancestor 2" << std::setw(4) << fafp_last[i_jet]
                            << "                p : " << p[fafp_last[i_jet]][1] << " " << p[fafp_last[i_jet]][2] << " "
                            << p[fafp_last[i_jet]][3] << " " << " E " << p[fafp_last[i_jet]][4] << std::endl;
      streamlog_out(DEBUG5) << "  ancestor 3" << std::setw(4) << fafp[i_jet]
                            << "                p : " << p[fafp[i_jet]][1] << " " << p[fafp[i_jet]][2] << " "
                            << p[fafp[i_jet]][3] << " " << " E " << p[fafp[i_jet]][4] << std::endl;
      streamlog_out(DEBUG5) << std::endl;

      streamlog_out(DEBUG5) << "   Character of jet " << i_jet << " : " << type[i_jet] % 100
                            << " (1=string, 2=lepton , "
      ...
      int odd_even = 0;
      if (type[i_jet] % 100 == 3 && n_hard_lepton % 2 == 1)
        odd_even = 1;
      ...
      if (i_jet % 2 == odd_even && type[i_jet] % 100 <= 3) {
        ...
        double masssq_t =
            str_tE * str_tE - str_tmom[0] * str_tmom[0] - str_tmom[1] * str_tmom[1] - str_tmom[2] * str_tmom[2];
        if (masssq_t > 0.) {
          streamlog_out(DEBUG6) << "         true         " << sqrt(masssq_t) << std::endl;
        }
        masssq =
            str_tES * str_tES - str_tmomS[0] * str_tmomS[0] - str_tmomS[1] * str_tmomS[1] - str_tmomS[2] * str_tmomS[2];
        if (masssq > 0.) {
        ...
        if (type[i_jet] % 100 == 3) {
          ...
          dilept_mass = sqrt(dilept_E * dilept_E - dilept_P[1] * dilept_P[1] - dilept_P[2] * dilept_P[2] -
                             dilept_P[3] * dilept_P[3]);
        ...
        if (masssq_t > 0.) {
          if (abs(sqrt(masssq_t) - p[k[elementon[i_jet]][4]][5]) / p[k[elementon[i_jet]][4]][5] > 0.007) {
            if (type[i_jet] % 100 == 1 && type[i_jet - 1] % 100 == 1 && nfsr[i_jet] + nfsr[i_jet - 1] == 0) {
              if (abs(tE[i_jet] + tE[i_jet - 1] - p[k[elementon[i_jet]][4]][4]) / p[k[elementon[i_jet]][4]][4] >
                  0.001) {
                ...
                for (int i_py = 1; i_py <= nlund; i_py++) {
                  streamlog_out(DEBUG9) << "list " << std::setw(7) << i_py << std::setw(7) << k[i_py][1]
                                        << std::setw(12) << k[i_py][2] << std::setw(7) << k[i_py][3] << std::setw(7)
                                        << k[i_py][4] << std::setw(7) << k[i_py][5] << std::setw(12) << p[i_py][1]
                                        << std::setw(12) << p[i_py][2] << std::setw(12) << p[i_py][3] << std::setw(12)
                                        << p[i_py][4] << std::setw(12) << p[i_py][5] <<

                      std::setw(7) << jet[i_py] << std::setw(7) << companion[abs(jet[i_py])] << std::setw(7)
                                        << type[abs(jet[i_py])] << std::endl;
                }
                  ...
                  if (jet[i_py] == i_jet || jet[i_py] == i_jet - 1) {
                    if (k[i_py][1] == 11) {
                      ...
                      if ((abs(e_kid - p[i_py][4]) / p[i_py][4]) > 0.001) {
                        streamlog_out(DEBUG9) << i_py << " " << k[i_py][4] << " " << k[i_py][5] << " " << e_kid << " "
                                              << p[i_py][4] << std::endl;
                      }
                    }
                  }
                ...
                for (int j_jet = i_jet - 1; j_jet <= i_jet; j_jet++) {
                  streamlog_out(WARNING) << std::setw(10) << i_jet << std::setw(10) << fafp[j_jet] << std::setw(10)
                                         << elementon[j_jet] << std::setw(10) << fafp_last[j_jet] << std::setw(10)
                                         << fafp_boson[4] << std::setw(10) << type[j_jet] << std::setw(10)
                                         << dijet_begining[j_jet] << std::setw(10) << dijet_end[j_jet] << std::setw(10)
                ...
                streamlog_out(DEBUG8) << "           E (sum/initial) " << " " << tE[i_jet] + tE[i_jet - 1] << " / "
                                      << p[k[elementon[i_jet]][4]][4] << std::endl;
                streamlog_out(DEBUG8) << " (As the energy matches well, this is probably due to the B-field) "
                                      << std::endl;
                streamlog_out(DEBUG8) << " Event: " << evt->getEventNumber() << ",   run:  " << evt->getRunNumber()
                                      << std::endl;
    ...
    for (int k_dj_end = 1; k_dj_end <= n_dje; k_dj_end++) {
      ...
      if (type[jets_end[1][k_dj_end]] % 100 == 5) {
        ...
        for (int j_jet_end = 1; j_jet_end <= jets_end[0][k_dj_end]; j_jet_end++) {
          ...
          if (type[jets_end[1][k_dj_end]] % 100 == 2 || type[jets_end[1][k_dj_end]] % 100 == 4 ||
              type[jets_end[1][k_dj_end]] % 100 == 6) {
          ...
          for (int j_jet_end = 1; j_jet_end <= jets_end[0][k_dj_end]; j_jet_end++) {
            for (int k_py = first; k_py <= last; k_py++) {
              ...
              if (abs(jet[k_py]) == jets_end[j_jet_end][k_dj_end]) {
                ...
                if (type[jets_end[j_jet_end][k_dj_end]] % 100 != 3) {
                  break;
                }
              }
            }
      ...
      for (int j_jet_end = 1; j_jet_end <= jets_end[0][k_dj_end]; j_jet_end++) {
        fafpf->addParticle(
            dynamic_cast<ReconstructedParticle*>(jet_vec->getElementAt(jets_end[j_jet_end][k_dj_end] - 1)));
      }

      fafpfPidHandler.setParticleID(
          fafpf,
          type[jets_end[1][k_dj_end]] %
              100, // maybe flag from boson? could be 0,1, or 2 jets in the fafp that's from boson ..
          pdg[0], 0, fafpf_mainPidId, {});

      for (int j_jet_end = 1; j_jet_end <= jets_end[0][k_dj_end]; j_jet_end++) {
        fafpfPidHandler.setParticleID(fafpf, type[jets_end[j_jet_end][k_dj_end]], pdg[j_jet_end], 0,
                                      fafpf_pidIds[j_jet_end - 1], {});
    ...
    for (int k_dj_begin = 1; k_dj_begin <= n_djb; k_dj_begin++) {
      ...
      if (k[last][2] == 92) {
        first = last;
      } //  IS THIS RIGHT ? If there is a gluon radiation ?!
      ...
      for (int j_jet_begin = 1; j_jet_begin <= jets_begin[0][k_dj_begin]; j_jet_begin++) {
        fafpiPidHandler.setParticleID(fafpi, type[jets_begin[j_jet_begin][k_dj_begin]], pdg[j_jet_begin], 0,
                                      fafpi_pidIds[j_jet_begin - 1], {});
        fafpi->addParticle(
            dynamic_cast<ReconstructedParticle*>(jet_vec->getElementAt(jets_begin[j_jet_begin][k_dj_begin] - 1)));
      }
    ...
    streamlog_out(DEBUG4)
        << " HEPEVT relation table up with jet assignment, extended status codes, and PFOs w. energy (if any) "
        << std::endl;
    streamlog_out(DEBUG4)
    ...
    for (int i_py = 1; i_py <= std::min(nlund, 200); i_py++) {
      ...
      if (k[i_py][1] > 0 && k[i_py][1] < 30) {
        ...
        streamlog_out(DEBUG4) << std::setw(4) << i_py << std::setw(7) << k[i_py][1] << std::setw(5) << istat
                              ...
                              << std::setw(7) << k[i_py][4] << std::setw(5) << k[i_py][5] << std::setw(7) << k[i_py][7]
                              << std::setw(7) << k[i_py][8] << std::setw(12) << p[i_py][1] << std::setw(12)
                              << p[i_py][2] << std::setw(12) << p[i_py][3] << std::setw(12) << p[i_py][4]
                              << std::setw(12) << p[i_py][5] <<
        ...
        if (recovec.size() > 0) {                                        // if reconstructed
          for (unsigned i_reco = 0; i_reco < recovec.size(); i_reco++) { //  reco-of-this-true loop
            if (k[i_py][1] > 0 && k[i_py][1] < 30) {
              ...
              ReconstructedParticle* reco_part = dynamic_cast<ReconstructedParticle*>(recovec[i_reco]);
              streamlog_out(DEBUG3) << " [" << std::setw(10) << reco_part << " /" << std::setw(8)
                                    << reco_part->getEnergy() << "] ";
            }
          }
        } else if (istat == 1) {
    ...
    for (int i_jet = 1; i_jet <= njet; i_jet++) {
      streamlog_out(DEBUG6) << std::setw(10) << i_jet << std::setw(10) << fafp[i_jet] << std::setw(10)
                            << elementon[i_jet] << std::setw(10) << fafp_last[i_jet] << std::setw(10)
                            << fafp_boson[i_jet] << std::setw(10) << type[i_jet] << std::setw(10)
                            << dijet_begining[i_jet] << std::setw(10) << dijet_end[i_jet] << std::setw(10)
    ...
    for (unsigned i_cn_b = 0; i_cn_b < fafpi_vec->size(); i_cn_b++) {
      ...
      streamlog_out(DEBUG6) << std::setw(7) << i_cn_b + 1 << std::setw(12) << mom[0] << std::setw(12) << mom[1]
                            << std::setw(12) << mom[2] << std::setw(12) << fafpi->getEnergy() << std::setw(12)
                            << fafpi->getMass() << std::setw(7) << fafpi->getParticleIDs()[0]->getType() << "   ";
      for (unsigned i_pid = 0; i_pid < fafpi->getParticleIDs().size(); i_pid++) {
        if (fafpi->getParticleIDs()[i_pid]->getType() >= 100)
          glurad = 1;
    ...
    for (unsigned i_cn_e = 0; i_cn_e < fafpf_vec->size(); i_cn_e++) {
      ...
      streamlog_out(DEBUG6) << std::setw(7) << i_cn_e + 1 << std::setw(12) << mom[0] << std::setw(12) << mom[1]
                            << std::setw(12) << mom[2] << std::setw(12) << fafpf->getEnergy() << std::setw(12)
                            << fafpf->getMass() << std::setw(7) << fafpf->getParticleIDs()[0]->getType() << "   ";
      if (fafpf->getParticleIDs().size() < 3)
        streamlog_out(DEBUG6) << std::setw(3) << " ";
...
void TrueJet::getPyjets(LCCollection* mcpcol) {
  ...
  if (nMCP > 4000) {
    streamlog_out(WARNING) << " More than 4000 MCParticles in event " << evt->getEventNumber() << ",   run  "
  ...
  for (int j_mcp = 0; j_mcp < std::min(nMCP, 4000); j_mcp++) {
    ...
    if (j_mcp == 0 && mcp->getSimulatorStatus() == 0) {
      _generator_input_format = true;
    }
    k_py++;
    mcp_pyjets[k_py] = mcp;
  ...
  for (int j_mcp = 0; j_mcp < nMCP; j_mcp++) {
    ...
    if (mcp->ext<MCPpyjet>() != 0) {
      ...
      if (mcp->getParents().size() != 0) {
        ...
        if (i_py > first_line + 1 && !mcp->isOverlay() && k[i_py][3] == 0 && abs(mcp->getPDG()) != 6 &&
            abs(mcp->getPDG()) != 24) {
          streamlog_out(MESSAGE4) << " Motherless generator particle " << i_py << ". pdg: " << mcp->getPDG()
                                  << std::endl;
          streamlog_out(MESSAGE4) << " Event: " << evt->getEventNumber() << ",   run:  " << evt->getRunNumber()
                                  << std::endl;
          k[i_py][3] = -1;
        }
      }
      ...
      if (mcp->getDaughters().size() != 0) {
        ...
        if (!_generator_input_format) { // none of the below can happen if we are looking at the generator input
          ...
          if (k[i_py][1] != 0) {
            ...
            for (unsigned i_dau = 0; i_dau < mcp->getDaughters().size(); i_dau++) {
              ...
              if (mcp->getDaughters()[i_dau]->getGeneratorStatus() == 0 ||
                  (mcp->getDaughters()[i_dau]->getSimulatorStatus() == 0 && k[j_dau][1] == 1)) {
  ...
  streamlog_out(DEBUG4) << "                                             daughter daugther    parent   colour   colour "
                        << std::endl;
  for (int j_py = 1; j_py <= nlund; j_py++) {
    if (k[j_py][1] != 0 && k[j_py][1] < 30) {
      streamlog_out(DEBUG4) << std::setw(10) << j_py << std::setw(10) << k[j_py][1] << std::setw(10) << k[j_py][2]
                            << std::setw(10) << k[j_py][3] << std::setw(10) << k[j_py][4] << std::setw(10) << k[j_py][5]
                            << std::setw(10) << k[j_py][6] << std::setw(10) << k[j_py][7] << std::setw(10) << k[j_py][8]
                            << std::endl;
    } else {
      streamlog_out(DEBUG3) << std::setw(10) << j_py << std::setw(10) << k[j_py][1] << std::setw(10) << k[j_py][2]
                            << std::setw(10) << k[j_py][3] << std::setw(10) << k[j_py][4] << std::setw(10) << k[j_py][5]
                            << std::setw(10) << k[j_py][6] << std::setw(10) << k[j_py][7] << std::setw(10) << k[j_py][8]
                            << std::endl;
    }
  ...
  for (int i_py = 1; i_py <= nlund; i_py++) {
    ...
    if (k[i_py][2] == 94 && k[i_py][1] < 30) {
      ...
      int line94 = i_py;
      if (_whiz1) {
        stdhep_reader_bug_workaround(line94);
      }
      ...
      for (int mother = first_94_mother; mother <= last_94_mother; mother++) {
        if ((abs(k[mother][2]) == 24 && abs(k[k[mother][3]][2]) != 6) || k[mother][2] == 23 || k[mother][2] == 25)
          continue;
        if (abs(k[mother][4]) != line94)
          continue; // intruder, go to next line
        ...
        } else { // mother has several daughters, find the daughter that has the same pdg as the mother (actually an
                 // intruder)
...
void TrueJet::stdhep_reader_bug_workaround(int line94) {
  ...
  bool inconsitent_pid =
      ((k[first_94_daughter][2] != k[first_94_mother][2] &&
        k[first_94_daughter][2] != k[last_94_mother][2]) || // inconsistent pdgs
  ...
  if ((inconsitent_pid || inconsitent_E) && (!gluon_daughters)) {
    ...
    while (1) {
      ...
      if (first) {
        if (inconsitent_pid) {
          if (k[first_94_daughter][2] * k[last_94_daughter][2] > 0) {
            ...
            streamlog_out(ERROR) << " 94 on line " << line94
      ...
      for (int jjj = 1; jjj <= 4; jjj++) {
        target[jjj] = p[line94][jjj] - p[right_mother][jjj];
      }
    ...
    if (new_mother == 0) {
      ...
      if (new_mother < right_mother) {
        first_94_mother = new_mother;
        last_94_mother = right_mother;
      } else {
        first_94_mother = right_mother;
        last_94_mother = new_mother;
      }
    }

    k[line94][3] = first_94_mother;
    k[line94][6] = last_94_mother;
    k[first_94_mother][4] = line94;
    k[first_94_mother][5] = line94;
    k[last_94_mother][4] = line94;
...
void TrueJet::true_lepton() {
  ...
  if (_top_event) { // start looking for leptons after the last top - there are un-connected leptons before that to
                    // avoid!
  ...
  for (int k_py = start_loop; k_py <= nlund; k_py++) {
    if ((abs(k[k_py][2]) >= 11 && abs(k[k_py][2]) <= 16)) {
      streamlog_out(DEBUG1) << " found lepton, k_py = " << k_py << ", k[k_py][3] = " << k[k_py][3]
                            << ", k[3][3] = " << k[3][3] << std::endl;
      if (((!_whiz1) && ((k[k[k_py][3]][1] == 21) || ((abs(k[k[k_py][3]][2]) == 23 || abs(k[k[k_py][3]][2]) == 24) &&
                                                      k[k[k[k_py][3]][3]][1] == 21))) ||
  ...
  for (int j_lept = 1; j_lept <= n_hard_lepton; j_lept++) {
    if (ihard_lepton[j_lept] == 0) {
      ...
      for (int l_lept = 1; l_lept <= n_hard_lepton; l_lept++) {
        ...
        if (flavour(k[l_py][2]) == flavour(k[k_py][2]) &&
            k[l_py][2] * k[k_py][2] < 0) { // consider only
                                           // lepton-flavour/anti-lepton flavour groupings
          if ((k[l_py][2] > 0 && k[l_py][7] == k[k_py][8] && k[l_py][7] != 0) ||
              (k[l_py][2] < 0 && k[l_py][8] == k[k_py][7] &&
               k[l_py][8] != 0)) { // a hint on grouping (leptons from the same 94)
            streamlog_out(DEBUG4) << " hint match for lepton on lines " << l_py << " and " << k_py << std::endl;
            hint_companion = l_lept;
            n_hint++;
          }
          if (k[l_py][3] == k[k_py][3]) {
            if (k[k[l_py][3]][2] == 23 || k[k[l_py][3]][2] == 24 ||
                k[k[l_py][3]][2] == 25) { // leptons from the same boson
              streamlog_out(DEBUG4) << " boson match for lepton on lines " << l_py << " and " << k_py << std::endl;
              boson_companion = l_lept;
              n_boson++;
            }
            if (k[l_py][7] == 0 && k[l_py][8] == 0 && k[k_py][7] == 0 &&
                k[k_py][8] == 0) { // if no hint nor boson parent found,
  ...
  // outcome of this loop is the -(jet number) for each hard lepton in jet[ (each hard-lepton pyjets-line) ] and the
  // elementon of the corresponding jet elementon[jet] = hard-lepton pyjets-line. lept on exit will be the first post-CM
  // shower incarnation of the lepton.
  ...
  for (int j_lept = 1; j_lept <= n_hard_lepton; j_lept++) {
    ...
    if (_top_event) { // we want to back-track the lepton to the top, so that jet grouping can group all top daughters
                      // into on "c.n."
      int oma = lept;
      int fafp_top = 0;
      while (1) {
      ...
      if (fafp_top != 0) {
        fafp_last[current_jet + j_lept] = fafp_top;
      }
    }
  ...
  // ! set jet for all further descendants of hard leptons . Don't change already set assignments. Never assign a jet to
  // a 94.
...
void TrueJet::cluster() {
  ...
  nclu = 0;
  for (int k_py = 1; k_py <= nlund; k_py++) {
    streamlog_out(DEBUG0) << " Checking cluster at " << k_py << " k[k_py][2], and k[k_py][1] : " << " " << k[k_py][2]
                          << " , " << k[k_py][1] << std::endl;
  ...
  for (int i_clu = 1; i_clu <= nclu; i_clu++) {
    ...
    jet1 = current_jet + 2 * (i_clu - 1) + 1;
    jet2 = current_jet + 2 * (i_clu - 1) + 2;
    elementon[jet1] = k[clu][3];
    elementon[jet2] = elementon[jet1];
    while (k[elementon[jet2] + 1][4] == clu) {
      elementon[jet2] = elementon[jet2] + 1;
    }
    streamlog_out(DEBUG4) << " Assigning jets " << jet1 << " or " << jet2 << " to CLUSTER at " << clu << std::endl;
...
void TrueJet::photon() {
  ...
  nphot = 0;
  for (int k_py = 1; k_py <= nlund; k_py++) {
    streamlog_out(DEBUG0) << " Checking photon at " << k_py << " k[k_py][2], and k[k_py][1] : " << " " << k[k_py][2]
                          << " , " << k[k_py][1] << std::endl;
...
void TrueJet::string() {
  int n_left = 0, sstr = 0, lquark = 0, lstr = 0, istr = 0, str_nb = 0, jet1 = 0, jet2 = 0, istr_previous = 0,
      iback = 0;
  ...
  for (int k_py = 1; k_py <= nlund; k_py++) {
    streamlog_out(DEBUG0) << " particle " << k_py << " with PDG " << k[k_py][2] << " is assigned to jet " << jet[k_py]
                          << std::endl;
  ...
  if (k[k[lquark][4]][2] !=
      92) { // there might be an intruder between the lquark and the first string. This is taken care of here
    if (!_higgs_to_glue_glue) {
      int lll = lquark;
  ...
  istr = lstr;
  str_nb = nstr;
  streamlog_out(DEBUG3) << "  sstr, lstr, lquark, current_jet, str_nb " << sstr << " " << lstr << " " << lquark << " "
                        << current_jet << " " << str_nb << std::endl;
  ...
  while (istr >= str_index[sstr]) {
    ...
    streamlog_out(DEBUG1) << " calculating jet1/2 from current_jet = " << current_jet << ", str_nb = " << str_nb
                          << std::endl;
    streamlog_out(DEBUG1) << " istr = " << istr << ", k[istr][3] = " << k[istr][3] << ", lquark = " << lquark
                          << std::endl;

    jet1 = current_jet + 2 * (str_nb - 1) + 1;
    jet2 = current_jet + 2 * (str_nb - 1) + 2;

    elementon[jet1] = k[istr][3];
    elementon[jet2] = lquark; // elementons: the last quarks before hadronisation, the ones
    ...
    istr_previous = istr;
    iback = str_index[rev_index[istr] - 1];
    istr = k[iback][3];
    if (istr < str_index[sstr])
      break; // before the py-line of the first string -> done
...
void TrueJet::assign_jet(int jet1, int jet2, int this_string) {
  ...
  streamlog_out(DEBUG4) << "   assign_jet: elementon[" << jet1 << "] = " << elementon[jet1] << ", elementon[" << jet2
  ...
  absp_2 = sqrt(absp_2);
  for (int kk = 1; kk <= 3; kk++) {
    dir_diff[kk] = p[elementon[jet2]][kk] / absp_2 - p[elementon[jet1]][kk] / absp_1;
  }

  for (int k_py_had = k[this_string][4]; k_py_had <= k[this_string][5];
       k_py_had++) { // this loops over the first generation hadrons
...
void TrueJet::first_parton(int this_partic, int this_jet, int& first_partic, int& last_94_parent, int& nfsr_here,
                           int& info, int& info2) {
  ...
  streamlog_out(DEBUG4) << "   first_parton: this_quark = " << this_quark << ",  quark_pdg = " << quark_pdg
                        << std::endl;
  ...
  while (k[this_quark][3] > 0 && abs(k[k[this_quark][3]][2]) != 11 && abs(k[k[this_quark][3]][2]) != 21 &&
         abs(k[k[this_quark][3]][2]) != 22 && abs(k[k[this_quark][3]][2]) != 23 && abs(k[k[this_quark][3]][2]) != 24 &&
         abs(k[k[this_quark][3]][2]) != 25) {
    // (this end-condition works both for whiz1 and whiz2)

    streamlog_out(DEBUG3) << "     start of while loop:  this_quark = " << this_quark
                          << ", its PDG = " << k[this_quark][2] << ", its parent = " << k[this_quark][3]
                          << ", parent's PDG  = " << abs(k[k[this_quark][3]][2]) << ", quark_pdg  = " << quark_pdg
                          << std::endl;
    ...
    streamlog_out(DEBUG3) << "     end of while loop: this_quark = " << this_quark << ", its PDG = " << k[this_quark][2]
  ...
  if (resonance_insertion == 0 && (abs(k[k[this_quark][3]][2]) == 21 || abs(k[k[this_quark][3]][2]) == 24)) {
    ...
    streamlog_out(DEBUG3) << "     in while loop calling first_parton with: k[this_quark][3] = " << k[this_quark][3]
                          << std::endl;
...
void TrueJet::grouping() {
    ...
    if (jet[i_py] > 0) {
      if (k[i_py][1] == 11) {
        ...
        if (abs(Ekid - p[i_py][4]) / p[i_py][4] > 0.001) {
          ...
          streamlog_out(DEBUG3) << " Event: " << evt->getEventNumber() << ",   run:  " << evt->getRunNumber()
                                << std::endl;
          streamlog_out(DEBUG3) << std::endl;
  ...
  for (int k_jet = 1; k_jet <= njet; k_jet++) {
    ...
    if (type[k_jet] % 100 <= 3) { // only group strings, clusters and hard leptons
      ...
      for (int j_jet = 1; j_jet <= njet; j_jet++) {
        if (type[j_jet] % 100 <= 3) {
          ...
          bool boson_connected = (k[fafp[j_jet]][3] == k[fafp[k_jet]][3] &&
                                  (abs(k[k[fafp[k_jet]][3]][2]) == 6 || k[k[fafp[k_jet]][3]][2] == 23 ||
                                   abs(k[k[fafp[k_jet]][3]][2]) == 24 || k[k[fafp[k_jet]][3]][2] == 25));
          ...
          if (k_lept == j_lept ||
              _top_event) { // only group quarks with quarks, leptons with leptons, except in top events,
            ...
            if ((fafp[j_jet] ==
                 fafp[k_jet]) || // the fafp:s are the same or have the same parent. NB will group k_jet with itself!
                colour_connected ||
                boson_connected || (no_colour && (same_parent && same_family)) ||
              ...
              if (k_jet != j_jet) {
                ...
                if (k[fafp[k_jet]][3] != 0) {
                  streamlog_out(DEBUG4) << " parents of the fafp of the jets are " << k[fafp[k_jet]][3] << " and  "
                                        << k[fafp[j_jet]][3] << " .";
                }
      ...
      for (int i_grp = 1; i_grp <= group[0][k_jet]; i_grp++) {
        ...
        if (group[i_grp][k_jet] == companion[k_jet] || companion[k_jet] == 0) {
          compthere = true;
        }
      }
      if (!compthere && not _top_event) {
        streamlog_out(ERROR) << " jet " << k_jet << " : companion " << companion[k_jet] << " not in group "
                             << std::endl;
      }
      streamlog_out(DEBUG4) << std::endl;
  ...
  for (int k_jet = 1; k_jet <= njet; k_jet++) {
    ...
    if ((type[k_jet] % 100 == 3) && (tE[k_jet] > 0.0000001) && (tE[companion[k_jet]] > 0.0000001)) {
      n_2jet_clu++;
    }
  }
  ...
  for (int k_jet = 1; k_jet <= njet - 1; k_jet += 2) {
    if ((type[k_jet] % 100 == 1) && (k[fafp[k_jet]][4] != k[fafp[k_jet + 1]][4]) &&
  ...
  //   jsum=jets_summary_t(njet,n_djb,n_dje,2*nstr, n_hard_lepton, nboson,  2*nclu,nisr , n_mixed,  n_jetless,
  //   n_2jet_clu, n_0_E_jets,jt(1:njet),dijb(1:n_djb),dije(1:n_dje))
}
