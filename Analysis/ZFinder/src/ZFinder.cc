...
ZFinder::ZFinder() : marlin::Processor("ZFinder") {
  ...
  registerProcessorParameter("ZDecay", "Z decay mode", _zdecay, zdecay);

  std::string inputParticleCollectionName = "PandoraPFOs";
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "InputParticleCollectionName", "Input Particle Collection Name ",
                          _inputParticleCollectionName, inputParticleCollectionName);
  ...
  registerProcessorParameter("AddPhotons", "Include photons with reconstructed Z", _addPhotons, (int)1);

  registerProcessorParameter("CanUseClusterEnergyForElectrons",
                             "Allow possibilit of cluster not track used for electrons",
                             _canUseClusterEnergyForElectrons, (int)1);

  registerProcessorParameter("FermionMomentumCut", "Momentum Cut for fermion from Z decay", _momentumCut, (float)10.);

  registerProcessorParameter("CosTrackGammaCut", "Minimum cosine of track-photon angle", _cosTrackGammaCut,
                             (float)0.999);

  registerProcessorParameter("MaxDeltaMz", "Maximum difference between candidate and Z mass", _dmzcut, (float)50.);

  registerProcessorParameter("MuonEcalEnergyCut", "Cut on muon ECAL energy", _muonEcalEnergyCut, (float)2.5);

  registerProcessorParameter("MuonHcalEnergyCut", "Cut on Muon HCAL energy", _muonHcalEnergyCut, (float)10.);

  registerProcessorParameter("MuonHcalEnergyCut1", "Cut on Muon HCAL energy (alt)", _muonHcalEnergyCut1, (float)5.);

  registerProcessorParameter("ElectronEcalEnergyCut", "Cut on electron ECAL energy", _electronEcalEnergyCut,
                             (float)10.);

  registerProcessorParameter("ElectronHcalEnergyCut", "Cut on Electron HCAL energy", _electronHcalEnergyCut,
                             (float)10.);

  registerProcessorParameter("ElectronEoverPCutLow", "Cut on Electron E/p cut", _electronEoPCutLow, (float)0.5);

  registerProcessorParameter("ElectronEoverPCutHigh", "Cut on Electron E/p cut", _electronEoPCutHigh, (float)1.3);
...
void ZFinder::init() { return; }

//===================================================================================

void ZFinder::processRunHeader(LCRunHeader* /*run*/) { return; }
...
void ZFinder::processEvent(LCEvent* evt) {
  ...
  if (found) {
    if (_printing > 1)
      std::cout << "Analysis : " << _zdecay << std::endl;
    if (_zdecay.find("ee") != std::string::npos)
      this->FindZee(recparcol);
    if (_zdecay.find("mm") != std::string::npos)
      this->FindZmumu(recparcol);
    if (_zdecay.find("mumu") != std::string::npos)
      this->FindZmumu(recparcol);
    if (_zdecay.find("tautau") != std::string::npos)
      streamlog_out(ERROR) << "ZFinder tautau decay mode not implemented" << std::endl;
    if (_zdecay.find("qq") != std::string::npos)
      streamlog_out(ERROR) << "ZFinder qq decay mode not implemented" << std::endl;
  }
...
void ZFinder::FindZmumu(LCCollectionVec* recparcol) {
  ...
  if (_printing > 1)
    std::cout << "FindZmumu : " << _pfovec.size() << std::endl;
  ...
  for (unsigned int i = 0; i < _pfovec.size(); i++) {
    if (_pfovec[i]->getCharge() != 0) {
      ...
      bool muon = true;
      if (_pfovec[i]->getEnergy() < _momentumCut)
        muon = false;
      if (ecal1 > _muonEcalEnergyCut)
        muon = false;
      if (hcal1 > _muonHcalEnergyCut)
        muon = false;
      if (hcal1 > _muonHcalEnergyCut1 && muon1 < 0.01)
        muon = false;
      if (muon) {
        LorentzVector pmu(_pfovec[i]->getMomentum()[0], _pfovec[i]->getMomentum()[1], _pfovec[i]->getMomentum()[2],
                          _pfovec[i]->getEnergy());
  ...
  if (bestdmz < _dmzcut && pMplus != NULL && pMminus != NULL) {
    ...
    if (_addPhotons > 0) {
      for (unsigned int i = 0; i < _pfovec.size(); i++) {
        if (_pfovec[i]->getType() == 22) {
          ...
          float cosm = 0.;
          if (eg > 0 && eem > 0)
            cosm = (pxg * pxem + pyg * pyem + pzg * pzem) / eg / eem;
          float cosp = 0.;
          if (eg > 0 && eep > 0)
            cosp = (pxg * pxep + pyg * pyep + pzg * pzep) / eg / eep;
    ...
    recoPart->setType(94);
    if (_printing > 1)
      std::cout << "Found mmX : " << Mass << std::endl;
...
void ZFinder::FindZee(LCCollectionVec* recparcol) {
  ...
  if (_printing > 1)
    std::cout << "FindZee : " << _pfovec.size() << std::endl;
  ...
  for (unsigned int i = 0; i < _pfovec.size(); i++) {
    if (_pfovec[i]->getCharge() != 0 && _pfovec[i]->getEnergy() > 0) {
      ...
      if (_pfovec[i]->getEnergy() < _momentumCut)
        electron = false;
      if (ecal1 < _electronEcalEnergyCut)
        electron = false;
      if (hcal1 > _electronHcalEnergyCut)
        electron = false;
      float eop = ecal1 / _pfovec[i]->getEnergy();
      if (eop < _electronEoPCutLow)
        electron = false;
      if (eop > _electronEoPCutHigh)
        electron = false;
      if (muon1 > 0.01)
        electron = false;
      if (abs(_pfovec[i]->getType()) == 11 && _pfovec[i]->getEnergy() > _momentumCut / 2.0)
        electron = true;
      if (electron) {
        LorentzVector pe(_pfovec[i]->getMomentum()[0], _pfovec[i]->getMomentum()[1], _pfovec[i]->getMomentum()[2],
                         _pfovec[i]->getEnergy());
        if (_pfovec[i]->getCharge() > 0.5) {
          peplus.push_back(pe);
  ...
  if (bestdmz < _dmzcut && pEplus != NULL && pEminus != NULL) {
    ...
    if (ti.size() == 1)
      sigpPlus = trackEPlus * sqrt(ti[0]->getCovMatrix()[5]) / fabs(ti[0]->getOmega());
    if (ci.size() > 0) {
    ...
    if (tj.size() == 1)
      sigpMinus = trackEMinus * sqrt(tj[0]->getCovMatrix()[5]) / fabs(tj[0]->getOmega());
    if (cj.size() > 0) {
        ...
        if (_pfovec[i]->getType() == 22) {
          ...
          if (cosp > _cosTrackGammaCut || cosm > _cosTrackGammaCut) {
            ...
            if (cosp > cosm) {
              ...
              if (drp < 30.0 && chiEPlus > 4.0 && fabs(chiNew) < chiEPlus)
                merge = true;
              if (drp < 40.0 && chiEPlus > 5.0 && fabs(chiNew) < chiEPlus)
                merge = true;
              if (drp < 50.0 && chiEPlus > 7.0 && fabs(chiNew) < chiEPlus)
                merge = true;
              // sanity check
              if (fabs(chiEPlus) < 2.0 && chiNew * chiNew > chiEPlus * chiEPlus + 5.0)
                merge = false;
              ...
              if (drm < 20.0)
                merge = true;
              // if fairly close merge if chi2 for matching improves greatly
              if (drm < 30.0 && chiEMinus > 4.0 && fabs(chiNew) < chiEMinus)
                merge = true;
              if (drm < 40.0 && chiEMinus > 5.0 && fabs(chiNew) < chiEMinus)
                merge = true;
              if (drm < 50.0 && chiEMinus > 7.0 && fabs(chiNew) < chiEMinus)
                merge = true;
              // sanity check
              if (fabs(chiEMinus) < 2.0 && chiNew * chiNew > chiEMinus * chiEMinus + 5.0)
                merge = false;
              // always merge if very close - can't expect reconstruction to work
              if (drm < 10.0)
                merge = true;
    ...
    if (_canUseClusterEnergyForElectrons && (chiEPlus < -3.5 || chiEMinus < -3.5)) {
      ...
      float scaleP = 1.0;
      if (chiEPlus < -3.5)
        scaleP = clusterEPlus / trackEPlus;
      float scaleM = 1.0;
      if (chiEMinus < -3.5)
        scaleM = clusterEMinus / trackEMinus;
    ...
    recoPart->setType(94);
    if (_printing > 1)
      std::cout << "Found eeX : " << Mass << std::endl;
