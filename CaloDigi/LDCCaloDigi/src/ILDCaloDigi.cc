...
struct XToLower {
  int operator()(int ch) { return std::tolower(ch); }
};
...
ILDCaloDigi::ILDCaloDigi() : Processor("ILDCaloDigi") {
  ...
  registerInputCollections(LCIO::SIMCALORIMETERHIT, "ECALCollections", "ECAL Collection Names", _ecalCollections,
                           ecalCollections);
  ...
  registerInputCollections(LCIO::SIMCALORIMETERHIT, "HCALCollections", "HCAL Collection Names", _hcalCollections,
                           hcalCollections);
  ...
  registerOutputCollection(LCIO::CALORIMETERHIT, "ECALOutputCollection0", "ECAL Collection of real Hits",
                           _outputEcalCollections[0], std::string("ECALBarrel"));

  registerOutputCollection(LCIO::CALORIMETERHIT, "ECALOutputCollection1", "ECAL Collection of real Hits",
                           _outputEcalCollections[1], std::string("ECALEndcap"));

  registerOutputCollection(LCIO::CALORIMETERHIT, "ECALOutputCollection2", "ECAL Collection of real Hits",
                           _outputEcalCollections[2], std::string("ECALOther"));

  // hcal

  registerOutputCollection(LCIO::CALORIMETERHIT, "HCALOutputCollection0", "HCAL Collection of real Hits",
                           _outputHcalCollections[0], std::string("HCALBarrel"));

  registerOutputCollection(LCIO::CALORIMETERHIT, "HCALOutputCollection1", "HCAL Collection of real Hits",
                           _outputHcalCollections[1], std::string("HCALEndcap"));

  registerOutputCollection(LCIO::CALORIMETERHIT, "HCALOutputCollection2", "HCAL Collection of real Hits",
                           _outputHcalCollections[2], std::string("HCALOther"));

  registerOutputCollection(LCIO::LCRELATION, "RelationOutputCollection", "CaloHit Relation Collection",
                           _outputRelCollection, std::string("RelationCaloHit"));

  registerProcessorParameter("ECALThreshold", "Threshold for ECAL Hits in GeV", _thresholdEcal, (float)5.0e-5);

  registerProcessorParameter(
      "ECALThresholdUnit",
      "Unit for ECAL Threshold. Can be \"GeV\", \"MIP\" or \"px\". MIP and px need properly set calibration constants",
      _unitThresholdEcal, std::string("GeV"));

  std::vector<float> hcalThresholds;
  hcalThresholds.push_back(0.00004);
  registerProcessorParameter("HCALThreshold", "Threshold for HCAL Hits in GeV", _thresholdHcal, hcalThresholds);

  registerProcessorParameter(
      "HCALThresholdUnit",
      "Unit for HCAL Threshold. Can be \"GeV\", \"MIP\" or \"px\". MIP and px need properly set calibration constants",
      _unitThresholdHcal, std::string("GeV"));

  std::vector<int> ecalLayers;
  ecalLayers.push_back(20);
  ecalLayers.push_back(100);
  registerProcessorParameter("ECALLayers", "Index of ECal Layers", _ecalLayers, ecalLayers);

  std::vector<int> hcalLayers;
  hcalLayers.push_back(100);
  registerProcessorParameter("HCALLayers", "Index of HCal Layers", _hcalLayers, hcalLayers);

  std::vector<float> calibrEcal;
  calibrEcal.push_back(40.91);
  calibrEcal.push_back(81.81);
  registerProcessorParameter("CalibrECAL", "Calibration coefficients for ECAL", _calibrCoeffEcal, calibrEcal);

  std::vector<float> calibrHcalBarrel;
  calibrHcalBarrel.push_back(0.);
  registerProcessorParameter("CalibrHCALBarrel", "Calibration coefficients for Barrel HCAL", _calibrCoeffHcalBarrel,
                             calibrHcalBarrel);

  std::vector<float> calibrHcalEndCap;
  calibrHcalEndCap.push_back(0.);
  registerProcessorParameter("CalibrHCALEndcap", "Calibration coefficients for EndCap HCAL", _calibrCoeffHcalEndCap,
                             calibrHcalEndCap);

  std::vector<float> calibrHcalOther;
  calibrHcalOther.push_back(0.);
  registerProcessorParameter("CalibrHCALOther", "Calibration coefficients for Other (Ring) HCAL", _calibrCoeffHcalOther,
                             calibrHcalOther);

  registerProcessorParameter("IfDigitalEcal", "Digital Ecal", _digitalEcal, 0);

  registerProcessorParameter("MapsEcalCorrection", "Ecal correction for theta dependency of calibration for MAPS",
                             _mapsEcalCorrection, 0);

  registerProcessorParameter("IfDigitalHcal", "Digital Hcal", _digitalHcal, 0);

  registerProcessorParameter("ECALGapCorrection", "Correct for ECAL gaps", _ecalGapCorrection, (int)1);

  registerProcessorParameter("HCALGapCorrection", "Correct for ECAL gaps", _hcalGapCorrection, (int)1);

  registerProcessorParameter("ECALEndcapCorrectionFactor", "Energy correction for ECAL endcap",
                             _ecalEndcapCorrectionFactor, (float)1.025);

  registerProcessorParameter("HCALEndcapCorrectionFactor", "Energy correction for HCAL endcap",
                             _hcalEndcapCorrectionFactor, (float)1.025);

  registerProcessorParameter("ECALGapCorrectionFactor", "Factor applied to gap correction", _ecalGapCorrectionFactor,
                             (float)1.0);

  registerProcessorParameter("ECALModuleGapCorrectionFactor", "Factor applied to module gap correction",
                             _ecalModuleGapCorrectionFactor, (float)0.5);

  registerProcessorParameter("HCALModuleGapCorrectionFactor", "Factor applied to module gap correction",
                             _hcalModuleGapCorrectionFactor, (float)0.5);

  registerProcessorParameter("Histograms", "Hit times histograms", _histograms, (int)0);

  registerProcessorParameter("UseEcalTiming", "Use ECAL hit times", _useEcalTiming, (int)0);

  registerProcessorParameter("ECALCorrectTimesForPropagation",
                             "Correct ECAL hit times for propagation: radial distance/c",
                             _ecalCorrectTimesForPropagation, (int)0);

  registerProcessorParameter("ECALTimeWindowMin", "ECAL Time Window minimum time in ns", _ecalTimeWindowMin,
                             (float)-10.0);

  registerProcessorParameter("ECALEndcapTimeWindowMax", "ECAL Endcap Time Window maximum time in ns",
                             _ecalEndcapTimeWindowMax, (float)+100.0);

  registerProcessorParameter("ECALBarrelTimeWindowMax", "ECAL BarrelTime Window maximum time in ns",
                             _ecalBarrelTimeWindowMax, (float)+100.0);

  registerProcessorParameter("ECALDeltaTimeHitResolution", "ECAL Minimum Delta Time in ns for resolving two hits",
                             _ecalDeltaTimeHitResolution, (float)+10.0);

  registerProcessorParameter("ECALTimeResolution", "ECAL Time Resolution used to smear hit times", _ecalTimeResolution,
                             (float)10.);
  ...
  registerProcessorParameter(
      ...
      _ecalSimpleTimingCut, (bool)true);

  registerProcessorParameter("UseHcalTiming", "Use HCAL hit times", _useHcalTiming, (int)1);

  registerProcessorParameter("HCALCorrectTimesForPropagation",
                             "Correct HCAL hit times for propagation: radial distance/c",
                             _hcalCorrectTimesForPropagation, (int)0);

  registerProcessorParameter("HCALTimeWindowMin", "HCAL Time Window minimum time in ns", _hcalTimeWindowMin,
                             (float)-10.0);

  registerProcessorParameter("HCALBarrelTimeWindowMax", "HCAL Time Window maximum time in ns", _hcalBarrelTimeWindowMax,
                             (float)+100.0);

  registerProcessorParameter("HCALEndcapTimeWindowMax", "HCAL Time Window maximum time in ns", _hcalEndcapTimeWindowMax,
                             (float)+100.0);

  registerProcessorParameter("HCALDeltaTimeHitResolution", "HCAL Minimum Delta Time in ns for resolving two hits",
                             _hcalDeltaTimeHitResolution, (float)+10.0);

  registerProcessorParameter("HCALTimeResolution", "HCAL Time Resolution used to smear hit times", _hcalTimeResolution,
                             (float)10.);
  ...
  registerProcessorParameter(
      ...
      _hcalSimpleTimingCut, (bool)true);

  // additional digi effects (Daniel Jeans)

  registerProcessorParameter("CalibECALMIP", "calibration to convert ECAL deposited energy to MIPs", _calibEcalMip,
                             (float)1.0e-4);
  ...
  registerProcessorParameter("ECAL_miscalibration_uncorrel",
                             ...
                             _misCalibEcal_uncorrel, (float)0.0);
  ...
  registerProcessorParameter(
      ...
      _misCalibEcal_uncorrel_keep, (bool)false);
  ...
  registerProcessorParameter("ECAL_miscalibration_correl",
                             ...
                             _misCalibEcal_correl, (float)0.0);

  registerProcessorParameter("ECAL_deadCellRate", "ECAL random dead cell fraction (as a fraction: 0->1) ",
                             _deadCellFractionEcal, (float)0.);
  ...
  registerProcessorParameter("ECAL_deadCell_memorise",
                             ...
                             _deadCellEcal_keep, (bool)false);

  registerProcessorParameter("ECAL_strip_absorbtionLength", "length scale for absorbtion along scintillator strip (mm)",
                             _strip_abs_length, (float)1000000.);

  registerProcessorParameter("ECAL_pixel_spread",
                             "variation of mppc/sipm pixels capacitance in ECAL (as a fraction: 0.01=1%)",
                             _ecal_pixSpread, float(0.05));

  registerProcessorParameter("ECAL_elec_noise_mips", "typical electronics noise (ECAL, in MIP units)", _ecal_elec_noise,
                             float(0));

  registerProcessorParameter("energyPerEHpair", "energy required to create e-h pair in silicon (in eV)", _ehEnergy,
                             (float)3.6);

  registerProcessorParameter("ECAL_maxDynamicRange_MIP", "maximum of dynamic range for ECAL (in MIPs)", _ecalMaxDynMip,
                             float(2500));

  registerProcessorParameter("StripEcal_default_nVirtualCells",
                             "default number of virtual cells (used if not found in gear file)",
                             _ecalStrip_default_nVirt, int(9));

  registerProcessorParameter("ECAL_default_layerConfig",
                             "default ECAL layer configuration (used if not found in gear file)",
                             _ecal_deafult_layer_config, std::string("000000000000000"));

  // HCAL realistic digi parameters
  registerProcessorParameter("CalibHCALMIP", "calibration to convert HCAL deposited energy to MIPs", _calibHcalMip,
                             (float)1.0e-4);

  registerProcessorParameter("HCAL_apply_realistic_digi",
                             "apply realistic digitisation to HCAL hits? (0=none, 1=scintillator/SiPM)", _applyHcalDigi,
                             int(0));

  registerProcessorParameter("HCAL_PPD_PE_per_MIP",
                             "# Photo-electrons per MIP (for AHCAL): used to poisson smear #PEs if >0",
                             _hcal_PPD_pe_per_mip, (float)10.);
  ...
  registerProcessorParameter("HCAL_miscalibration_uncorrel",
                             ...
                             _misCalibHcal_uncorrel, (float)0.0);
  ...
  registerProcessorParameter(
      ...
      _misCalibHcal_uncorrel_keep, (bool)false);
  ...
  registerProcessorParameter("HCAL_miscalibration_correl",
                             ...
                             _misCalibHcal_correl, (float)0.0);

  registerProcessorParameter("HCAL_deadCellRate", "HCAL random dead cell fraction (as a fraction: 0->1) ",
                             _deadCellFractionHcal, (float)0.);
  ...
  registerProcessorParameter("HCAL_deadCell_memorise",
                             ...
                             _deadCellHcal_keep, (bool)false);

  registerProcessorParameter("HCAL_pixel_spread",
                             "variation of mppc/sipm pixels capacitance in HCAL (as a fraction: 0.01=1%)",
                             _hcal_pixSpread, float(0.));

  registerProcessorParameter("HCAL_elec_noise_mips", "typical electronics noise (HCAL, in MIP units)", _hcal_elec_noise,
                             float(0.));

  registerProcessorParameter("HCAL_maxDynamicRange_MIP", "maximum of dynamic range for HCAL (in MIPs)", _hcalMaxDynMip,
                             float(200.));

  // end daniel

  registerProcessorParameter("CellIDLayerString", "name of the part of the cellID that holds the layer",
                             _cellIDLayerString, std::string("K-1"));

  registerProcessorParameter("CellIDModuleString", "name of the part of the cellID that holds the module",
                             _cellIDModuleString, std::string("M"));
  registerProcessorParameter("CellIDStaveString", "name of the part of the cellID that holds the stave",
                             _cellIDStaveString, std::string("S-1"));

  registerProcessorParameter("CellIDIndexIString", "name of the part of the cellID that holds the index I",
                             _cellIDIndexIString, std::string("I"));

  registerProcessorParameter("CellIDIndexJString", "name of the part of the cellID that holds the index J",
                             _cellIDIndexJString, std::string("J"));
}
  ...
  try {
    ...
    try {
      ...
      streamlog_out(WARNING)
          << "You seem to be using an older version of the Mokka ECAL driver, affected by a bug in the gear file output"
          << endl;
      streamlog_out(WARNING)
          << "If you have a mix of silicon/scintillator layers in the ECAL, this may cause some confusion" << endl;
      streamlog_out(WARNING) << "You are recommended to use a later Mokka version, at least for hybrid (si+sc) ECALs"
                             << endl;
    }
    ...
    try { // first try to get from ECAL barrel section of gear file (it's here for "latest" Mokka versions)
      ...
      streamlog_out(MESSAGE) << "taking number of virtual cells from calo section of gear file: " << _strip_virt_cells
                             << endl;
      ...
      try { // otherwise look in the mokka parameters section
        ...
        if (!(convert >> _strip_virt_cells)) {
          streamlog_out(ERROR) << "could not decipher number of virtual cells! " << nVirtualMokkaS << " "
                               << _strip_virt_cells << endl;
          assert(0);
        }

        streamlog_out(MESSAGE) << "taking number of virtual cells from Mokka section of gear file: " << nVirtualMokkaS
                               << " " << _strip_virt_cells << endl;
        ...
        streamlog_out(WARNING) << "taking number of virtual cells from steering file (not found in gear file): "
                               << _strip_virt_cells << endl;
      }
    }
    ...
    try {
      ...
      try {
        ...
        streamlog_out(WARNING) << "taking layer layout from steering file (not found in gear file): " << _ecalLayout
                               << endl;
      }
    }
    ...
    try {
      ...
      for (int hitIdx(0); hitIdx < numElements; ++hitIdx) {
        ...
        if (energy > _thresholdEcal) {
          ...
          // if you want to understand the timing cut code, please refer to the hcal timing cut further below. it is
          // functionally identical, but has comments, explanations and excuses.
          if (_useEcalTiming) {
          ...
          //    std::cout << hit->getTimeCont(0) << " count = " << count <<  " E ECAL = " << energyCal << " - " <<
          //    eCellInTime << " - " << eCellOutput << std::endl;
        } // energy threshold
      }
  ...
  for (unsigned int colIdx(0); colIdx < _hcalCollections.size(); ++colIdx) {
    ...
    try {
      ...
      for (int hitIdx(0); hitIdx < numElements; ++hitIdx) { // loop over all SimCalorimeterHits in this collection
        ...
        if (energy > _thresholdHcal[0] / 2) { // preselect for SimHits with energySum>threshold. Doubtful at least, as
                                              // lower energy hit might fluctuate up and still be counted
          int cellid = hit->getCellID0();
          int cellid1 = hit->getCellID1();
          float calibr_coeff(1.);
          int layer = idDecoder(hit)[_cellIDLayerString];
          ...
          if (_useHcalTiming) {
            ...
            float r = sqrt(x * x + y * y + z * z); // this is a crude approximation. assumes initial particle originated
                                                   // at the very center of the detector.
            ...
            for (unsigned int i = 0; i < n; i++) {   // loop over all subhits
              ...
              if (!used[i]) { // if current subhit has not been merged with previous hits already, take current hit as
                ...
                for (unsigned int j = i; j < n; j++) { // loop through all hits after current hit
                  ...
                  if (!used[j]) {
                    ...
                    if (_hcalSimpleTimingCut) {
                      ...
                      if (deltatij < _hcalDeltaTimeHitResolution) { // if this subhit is close to current subhit, add
                                                                    // this hit's energy to timecluster
                        if (energyj > energyi)
                          timei = timej; // this is probably not what was intended. i guess this should find the largest
                                         // hit of one timecluster and use its hittime for the cluster, but instead it
                                         // compares the current hit energy to the sum of already found hit energies
                ...
                // if SimpleTimingCut == false
                // energyi carries the sum of subhit energies within +- one hcal time resolution - the timecluster
                // energy. timei carries something vaguely similar to the central hit time of the merged subhits
                ...
                if (energyi > _thresholdHcal[0]) { // now would be the correct time to do threshold comparison
                  ...
                  if (timei > _hcalTimeWindowMin &&
                    ...
                    //              std::cout << "Drop HCAL hit : " << timei << " " << calibr_coeff*energyi <<
                    //              std::endl;
                  }
                }
              }
          ...
          // std::cout << hit->getTimeCont(0) << " count = " << count <<  " EHCAL = " << energyCal << " - " <<
          // eCellInTime << " - " << eCellOutput << std::endl;
        }
      }
          ...
          for (unsigned int j = i + 1; j < _calHitsByStaveLayer[is][il].size(); ++j) {
            ...
            if (fabs(zi) < _zOfEcalEndcap && fabs(zj) < _zOfEcalEndcap) {
              ...
              if (xgap || ygap || xygap) {
                ...
                // cout <<"NewLDCCaloDigi found endcap gap, adjusting energy! " << xgap << " " << ygap << " " << xygap
                // << " , " << il << endl; cout << "stave " << is <<  " layer " << il << endl; cout << "  dx, dy " <<
                // dx<< " " << dy << " , sizes = " << pixsizex << " " << pixsizey << endl; cout << " xmin... " << xmin
                // << " " << xminm << " " << xmax << " ymin... " << ymin << " " << yminm << " " << ymax << endl;
...
float ILDCaloDigi::digitalHcalCalibCoeff(CHT::Layout caloLayout, float energy) {
  ...
  case CHT::barrel:
    if (ilevel > _calibrCoeffHcalBarrel.size() - 1) {
      streamlog_out(ERROR) << " Semi-digital level " << ilevel << " greater than number of HCAL Calibration Constants ("
                           << _calibrCoeffHcalBarrel.size() << ")" << std::endl;
    } else {
  ...
  case CHT::endcap:
    if (ilevel > _calibrCoeffHcalEndCap.size() - 1) {
      streamlog_out(ERROR) << " Semi-digital level " << ilevel << " greater than number of HCAL Calibration Constants ("
                           << _calibrCoeffHcalEndCap.size() << ")" << std::endl;
    } else {
  ...
  case CHT::plug:
    if (ilevel > _calibrCoeffHcalOther.size() - 1) {
      streamlog_out(ERROR) << " Semi-digital level " << ilevel << " greater than number of HCAL Calibration Constants ("
                           << _calibrCoeffHcalOther.size() << ")" << std::endl;
    } else {
...
LCCollection* ILDCaloDigi::combineVirtualStripCells(LCCollection* col, bool isBarrel, int stripOrientation) {
  ...
  if (stripOrientation == SQUARE) {
    streamlog_out(ERROR) << "ILDCaloDigi::combineVirtualStripCells trying to deal with silicon strips??? I do not know "
  ...
  for (int j(0); j < numElements; ++j) { // loop over virtual cells
    ...
    //   gearlayer = km1+1;
    // } else {
    //   streamlog_out ( ERROR ) << "ERROR unknown value for ecal driver version, _ecal_driver_version=" <<
    //   _ecal_driver_version << endl;
    // }
    // assert (gearlayer>=0);
  ...
  for (std::map<std::pair<int, int>, SimCalorimeterHitImpl*>::iterator itt = newhits.begin(); itt != newhits.end();
       itt++) {
...
int ILDCaloDigi::getNumberOfVirtualCells() {
  ...
  //     streamlog_out (MESSAGE) << "INFO: taking number of virtual cells from steering file (not found in gear file): "
  //     << _ecalStrip_default_nVirt << endl; _strip_virt_cells = _ecalStrip_default_nVirt;
  //   }
  // }
