...
MokkaCaloDigi::MokkaCaloDigi() : Processor("MokkaCaloDigi") {
  ...
  registerOutputCollection(LCIO::CALORIMETERHIT, "NewECALCollName", "name for the new collection of ECAL hits",
                           _newCollNameECAL, std::string("ECAL"));

  registerOutputCollection(LCIO::CALORIMETERHIT, "NewHCALCollName", "name for the new collection of HCAL hits",
                           _newCollNameHCAL, std::string("HCAL"));

  registerOutputCollection(LCIO::LCRELATION, "RelationCollName",
                           "name for collection of relations between CalorimeterHits and SimCalorimeterHits",
                           _relationCollName, std::string("RelationCaloHit"));

  registerProcessorParameter("ECALThreshold", "Threshold for ECAL Hits in GeV", _thresholdEcal, (float)1.0e-4);

  registerProcessorParameter("HCALThreshold", "Threshold for HCAL Hits in GeV", _thresholdHcal, (float)4.0e-4);
  ...
  registerProcessorParameter("CalibrHCAL", "Calibration coefficients for HCAL", _calibrCoeffHcal, calibrHcal);
...
void MokkaCaloDigi::init() {
  ...
  if (end_module_type == 1) {
    ...
    float total_z_size = 140 + 2 * tpc_ecal_hcalbarrel_halfz;
    _regularBarrelModuleLength = total_z_size / 5. * 1080. / 1120.;
    top_end_dim_z = (total_z_size - 3 * _regularBarrelModuleLength) / 2;
  }
  ...
  for (int i = 0; i < _numberOfHcalLayers; i++) {
    ...
    if (z < y_dim1_for_x) {
      ...
      float x = middle_dim_x -
                2. * (id * _hcalAbsorberThickness + (id - 1) * _hcalSensitiveThickness - y_dim1_for_x) / tan(Pi / 8.) -
    ...
    if (z >= y_dim1_for_z && z < (y_dim1_for_z + y_dim2_for_z)) {
      float zTmp = _regularBarrelModuleLength - 2 * _lateralPlateThickness +
                   (id * _hcalAbsorberThickness + (id - 1) * _hcalSensitiveThickness - y_dim1_for_z) *
                       (top_end_dim_z - _regularBarrelModuleLength) / y_dim2_for_z;
...
void MokkaCaloDigi::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }
...
MyHit* MokkaCaloDigi::ProcessHitInBarrel(SimCalorimeterHit* hit) {
  ...
  if (Module == 1) {
    zBegin =
        -1.5 * _regularBarrelModuleLength - 2 * _modulesGap - _lateralPlateThickness - _endBarrelChamberLength[Layer];
  }
  if (Module == 2) {
  ...
  int cellidNew = ((Module << SHIFT_M) & MASK_M) | ((Stave << SHIFT_S) & MASK_S) | ((Jnew << SHIFT_J) & MASK_J) |
                  ((Layer << SHIFT_K) & MASK_K) | ((Inew << SHIFT_I) & MASK_I);
...
void MokkaCaloDigi::end() {
  ...
  std::cout << "MokkaCaloDigi::end()  " << name() << " processed " << _nEvt << " events in " << _nRun << " runs "
            << std::endl;
}
