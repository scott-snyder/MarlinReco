...
ClusterCheater5_3::ClusterCheater5_3() : Processor("ClusterCheater5_3") {
  ...
  registerOutputCollection(LCIO::CLUSTER, "TrueClusterCollection", "Collection of True Clusters", _trueClustCollection,
                           std::string("TrueClusters"));

  registerOutputCollection(LCIO::LCRELATION, "TrueClusterToMCPCollection", "Relation Collection Cluster to MCP",
                           _trueClustToMCP, std::string("TrueClusterToMCP"));
  ...
  registerInputCollections(LCIO::CALORIMETERHIT, "CaloCollections", "Calorimeter Collection Names", _caloCollections,
                           caloCollections);

  registerInputCollection(LCIO::LCRELATION, "RelCollection", "SimCaloHit to CaloHit Relations Collection Name",
                          _relCollection, std::string("RelationCaloHit"));

  registerInputCollection(LCIO::MCPARTICLE, "MCParticleCollection", "Calorimeter Collection Names", _MCcollection,
                          std::string("MCParticle"));

  registerProcessorParameter("CutBackscatter", "Not to connect hist from  backscatter", _backcut, (int)1);

  registerProcessorParameter("MinHitsInCluster", "Minimal number of hits in cluster", _Nmin, (int)0);
}
      ...
      if (objectVec.size() > 0) {
        ...
        second_jump:
          ...
          if (p2clust[par] == NULL) {
            ...
            if (((radius < gearRMax && fabs(z) < zmax) && (radius1 > gearRMax || fabs(z1) > zmax)) ||
...
void ClusterCheater5_3::end() { cout << "cluster cheater  n hits lost = " << _nlost << endl; }
