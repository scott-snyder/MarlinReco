...
hybridRecoProcessor::hybridRecoProcessor() : Processor("hybridRecoProcessor") {
  ...
  registerInputCollections(LCIO::CALORIMETERHIT, "ECALcollections_cells", "Name of the ECAL cell collections",
                           _ecalCollectionsCells, ecalCollectionsCells);

  std::vector<std::string> ecalCollectionsTranStrips;
  ecalCollectionsTranStrips.push_back(std::string("ECALScTransverseBarrel"));
  registerInputCollections(LCIO::CALORIMETERHIT, "ECALcollections_tranStrips",
                           "Name of the ECAL transverse strip collections", _ecalCollectionsTranStrips,
  ...
  registerInputCollections(LCIO::CALORIMETERHIT, "ECALcollections_longStrips",
                           "Name of the ECAL longitudinal strip collections", _ecalCollectionsLongStrips,
  ...
  registerProcessorParameter("saveCheckRootHistograms", "save root file with debugging histograms?", _makePlots, false);
}
...
void hybridRecoProcessor::init() {
  ...
  if (_makePlots) { // define output root file and some histograms to put in it
    ...
    h_phiThetaMC = new TH2F("MCphiTheta", "MCphiTheta", 100, -2 * TMath::Pi(), 2 * TMath::Pi(), 100, -2 * TMath::Pi(),
                            2 * TMath::Pi());
    ...
    for (int ieb = 0; ieb < 2; ieb++) {
      for (int is = 0; is < 10; is++) {
        for (int im = 0; im < 10; im++) {
          ...
          name += im;
          h_cth_phi[ieb][is][im] =
              new TH2F(name + "_angle", name + "_angle", 100, -1, 1, 100, -TMath::Pi(), TMath::Pi());
...
void hybridRecoProcessor::processRunHeader(LCRunHeader* /*run*/) { return; }
...
void hybridRecoProcessor::setupGeometry() {
  ...
  try {
    ...
    streamlog_out(MESSAGE) << "taking number of Mokka virtual cells from calo section of gear file: "
    ...
    try {
      ...
      streamlog_out(MESSAGE) << "taking number of Mokka virtual cells from Mokka section of gear file: "
                             << nVirtualMokkaS << " " << nMokkaVirtualCells << endl;
      ...
      streamlog_out(WARNING) << "taking number of Mokka virtual cells from steering file (not found in gear file): "
                             << nMokkaVirtualCells << endl;
    }
  }
  ...
  for (int i = 0; i < ecalBarrelLayout.getNLayers(); i++) {
    ...
    if (i % 2 == 0)
      size1 *= nMokkaVirtualCells;
    else
      size0 *= nMokkaVirtualCells;

    streamlog_out(DEBUG) << "layer " << i << " " << size0 << " " << size1 << " "
  ...
  streamlog_out(DEBUG) << "strip length = " << _stripLength << " width " << _stripWidth << " aspect ratio "
                       << _stripAspectRatio << " cell size " << _cellSize << " ; symmetry = " << _symmetry << endl;
...
std::vector<CalorimeterHit*> hybridRecoProcessor::getVirtualHits(LCEvent* evt, CalorimeterHit* hit, int orientation,
                                                                 bool barrel) {
  ...
  if (_saveIntersections) {
    ...
    if (orientation == TRANSVERSE)
      stripEndsTransCol->addElement(interhit);
    else if (orientation == LONGITUDINAL)
      stripEndsLongCol->addElement(interhit);
    ...
    for (uint i = 0; i < splitter->size(); i++) {
      try {
        ...
        for (int j = 0; j < nelem; ++j) {
          ...
          if (_makePlots) {
            if (intercept.Mag() > 0)
              h_stripDist_intercept->Fill(dist / _stripLength);
            else
          ...
          if (intercept.Mag() > 0) { // intercept found, calculate in which virtual cell
            ...
            if (frac >= 0.0 && frac <= 1.0) {
              ...
              if (segment >= 0 && segment < _nVirtual) {
                ...
                streamlog_out(WARNING) << "strange segment " << segment << " frac = " << frac
...
TVector3 hybridRecoProcessor::stripIntersect(CalorimeterHit* hit0, TVector3 axis0, CalorimeterHit* hit1,
                                             TVector3 axis1) {
  ...
  if (!isStrip[0])
    streamlog_out(ERROR) << "ERROR from hybridRecoProcessor::stripIntersect, first hit should be a strip" << endl;
