...
class SortByPosX {
public:
  bool operator()(const T1& object1, const T1& object2) { return object1.first->x() < object2.first->x(); }
};
} // namespace
...
FPCCDDigitizer::FPCCDDigitizer() : Processor("FPCCDDigitizer") {
  ...
  registerProcessorParameter("CutOption_mode", "true make the Cut option active.", _cutMode, bool(false));

  registerProcessorParameter("CutOption_theta_From", "from", _cutThetaFrom, double(0));

  registerProcessorParameter("CutOption_theta_To", "To", _cutThetaTo, double(0));

  registerProcessorParameter("CutOption_phi_From", "phi From", _cutPhiFrom, double(0));

  registerProcessorParameter("CutOption_phi_To", "phi To", _cutPhiTo, double(0));
  ...
  registerProcessorParameter("isSignalEvent_isBGEvent_forLinkerInClustering",
                             ...
                             _signalProperty, int(1));

  registerProcessorParameter("Debug", "Debugging option", _debug, int(0));

  registerProcessorParameter("modifySimTrackerHit", "Modify SimTrackerHit into layer", _modifySimTHit, bool(true));

  registerProcessorParameter("OccupancyStudyMode",
                             "OccupancyStudyMode==1: We discreminate direct and backscatter.  0:NormalMode for "
                             "tracking. Make sure HitQuality Option for discreminating signal and BG ",
                             _OccupancyStudy, int(0));

  registerProcessorParameter("FPCCD_pixelSize(mm)", "Pixel size of FPCCD (unit:mm) (default: 0.005)", _pixelSize,
                             float(0.005));
  ...
  registerProcessorParameter("PixelHeight(mm)", "Pixel Height(mm).", _pixelheight, float(0.015));

  registerProcessorParameter("sigmaConstant", "ration of sigma of Landau distribution and path length", _sigmaConst,
                             double(1600 / 0.05));

  registerProcessorParameter(
      "HitQuality",
      "If OccupancyStydyMode == 0, this becomes active. Then, true: digitize evt as signal. false: digitize as bkg. "
      "Then TrackerHit output from FPCCDClustering has a quality identifying signal hit or bkg hit. If you use "
      "FPCCDOverlayBX, I recommend to differentiate inputfile by this option.",
      _isSignal, bool(true));

  registerProcessorParameter("HelixTMomentumCriteria(MeV)",
                             "Transverse momentum criteria for helix approximation (MeV)", _momCut,
                             float(100.0)); // old:1.0

  registerProcessorParameter("Ladder_Number_encoded_in_cellID", "Mokka has encoded the ladder number in the cellID",
                             _ladder_Number_encoded_in_cellID,
                             bool(true)); // old:false

  registerProcessorParameter("EnergyLoss_almostOFF", "false : normal mode, true : for computing Spatial Resolution.",
                             _EL_almostOFF, bool(false));

  // Input collections
  registerInputCollection(LCIO::SIMTRACKERHIT, "VTXCollectionName", "Name of the VTX SimTrackerHit collection",
                          _colNameVTX, std::string("VXDCollection"));

  // Output collections
  registerOutputCollection(LCIO::LCGENERICOBJECT, "VTXPixelHitCollection", "Name of the VXD PixelHit output collection",
                           _outColNameVTX, std::string("VTXPixelHits"));
}
...
// =====================================================================
void FPCCDDigitizer::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }
...
void FPCCDDigitizer::modifyEvent(LCEvent* evt) {
  ...
  if (col != 0) {
    ...
    // mori: _maxLadder is number of ladders which belong to the n-th(in FPCCD case, 5 or 6th.00) layer with most number
    // of ladders.
    //--> In case of default setteing, _nLayer = 6 and _maxLadder = 17.
    //  Constructor does only resize matrix whose element is _pxHits derived from std::vector<
    //  std::vector<std::map<unsigned int, FPCCDPixelHit*>> >
    ...
    if (_debug == 1) {
      std::cout << "Now Range shows " << std::endl
    ...
    for (int i = 0; i < nSimHits; i++) {
      ...
      double pos_x = SimTHitImpl->getPosition()[0];
      ...
      if (_debug == 2) {
        std::cout << "3-pos = " << pos_x << " , " << pos_y << " , " << pos_z << std::endl;
      ...
      if (_cutMode == false ||
          ((_cutThetaFrom < theta && theta < _cutThetaTo) && (_cutPhiFrom < phi && phi < _cutPhiTo))) {
        ...
        if (_signalProperty == 1) {
          ...
          std::cout << "something wrong at FPCCDDigitizer at line 246 to 256" << std::endl;
        if (_debug == 2)
    ...
    theData.packPixelHits(*fpccdDataVec); // Save _pxHits info in *LCCollectionVec ( in this case the elements of
                                          // LCCollectionVec have LCGenericObject)
...
// =====================================================================
void FPCCDDigitizer::makePixelHits(
    SimTrackerHitImpl* SimTHit, FPCCDData& hitVec,
    int nth_simthit) { // The type of FPCCDData is not Vector. So the word "&hitVec" is confusing. Pay attention to it.
                       // nth_simthit is the number of the element of STHcol. Namely, it is the nth_simthit of
                       // STHcol->getElementAt(nth_simthit). This makes data remember which simthit a pixel hit is
                       // originated from.
  gear::Vector3D* HitPosInMokka =
      new gear::Vector3D(SimTHit->getPosition()[0], SimTHit->getPosition()[1], SimTHit->getPosition()[2]);
  /************get basic info.**************/
  ...
  if (_ladder_Number_encoded_in_cellID) {
    streamlog_out(DEBUG3) << "Get Layer Number using StandardILD Encoding from ILDConf.h : cellId = " << cellId
                          << std::endl;
    UTIL::BitField64 encoder(lcio::LCTrackerCellID::encoding_string());
    encoder.setValue(cellId);
    layer = encoder[lcio::LCTrackerCellID::layer()];
    ladderID = encoder[lcio::LCTrackerCellID::module()];
  ...
  gear::Vector3D* MomAtHitPos =
      new gear::Vector3D(SimTHit->getMomentum()[0], SimTHit->getMomentum()[1], SimTHit->getMomentum()[2]);
  ...
  gear::Vector3D* MomAtLocalHitPos = new gear::Vector3D(
      MomAtHitPos->x() * _geodata[layer].sinphi[ladderID] - MomAtHitPos->y() * _geodata[layer].cosphi[ladderID],
      MomAtHitPos->z(),
      MomAtHitPos->x() * _geodata[layer].cosphi[ladderID] + MomAtHitPos->y() * _geodata[layer].sinphi[ladderID]);
  gear::Vector3D* LocalBField = new gear::Vector3D(
      _geodata[layer].sinphi[ladderID] * BField->x() - _geodata[layer].cosphi[ladderID] * BField->y(), BField->z(),
  ...
  if (sqrt(MomAtLocalHitPos->x() * MomAtLocalHitPos->x() + MomAtLocalHitPos->z() * MomAtLocalHitPos->z()) <
      _momCut * 1e-03) { // transvers momentum criteria for helix approximation
    if (_debug == 1)
      cout << "========== Track of this hit is trated as a helix ==========" << endl;
    getInOutPosOfHelixOnLadder(layer, PosOutFromLadder, PosInToLadder, LocalHitPos, MomAtLocalHitPos, LocalBField,
                               charge);
  } else {
    getInOutPosOnLadder(layer, PosOutFromLadder, PosInToLadder, LocalHitPos, MomAtLocalHitPos);
    if (_debug == 1) {
      std::cout << " layer  = " << layer << " : PosIntoLadder = " << PosInToLadder->x() << "," << PosInToLadder->y()
                << "," << PosInToLadder->z() << std::endl;
      std::cout << " layer  = " << layer << " : PosOutFromLadder = " << PosOutFromLadder->x() << ","
                << PosOutFromLadder->y() << "," << PosOutFromLadder->z() << std::endl;
    }
  }

  if (inSensitiveRegion(PosOutFromLadder, layer) &&
      inSensitiveRegion(PosInToLadder, layer)) { // check if the particle through the sensitive region.
    ...
    sort(EdgeOfPixel.begin(), EdgeOfPixel.end(),
         SortByPosX<std::pair<const gear::Vector3D*, int>, const gear::Vector3D*, int>());
    ...
    while (i_pixel_local != pixel_local.end()) {
      ...
      if (_OccupancyStudy == 0) {
        if (_isSignal && mcp->getParents().size() == 0) {
          quality = FPCCDPixelHit::kSingle;
        } else {
          quality = FPCCDPixelHit::kBKG;
        }
      } else {
        if (SimTHit->getTime() > 10) {
          quality = FPCCDPixelHit::kSingle;
        } else {
          quality = FPCCDPixelHit::kBKG;
        }
      ...
      if (nth_simthit > -1) {
        aHit.setOrderID(nth_simthit);
      } else if (nth_simthit == -1) {
        aHit.setOrderID(-1);
      } // From 2013_01_25, -1 is regarded as Background data.
      else {
        std::cout << "3rd argument in makepixelhit is something wrong. Please set nth_simthit value. wrong number is "
                  << nth_simthit << std::endl;
      }
    ...
    if (_debug == 1)
      cout << "This particle didn't through the sensitive region." << endl;
  }
...
int FPCCDDigitizer::getLadderID(const gear::Vector3D* pos, const int layer) {
  ...
  if (ladderID > 100)
    cout << "LADDERID>100!: " << ladderID << endl;
  ...
  if (ladderID == 50) {
    for (int j = 0; j < nladder; j++) {
      local_phi = posphi - dphi * j - offset_phi;
      if ((posR * cos(local_phi) - radius >= -1. * layerthickness) &&
          (posR * cos(local_phi) - radius <= 1. * layerthickness)) {
        ladderID = j;
        break;
      }
    }
...
gear::Vector3D* FPCCDDigitizer::getLocalPos(const gear::Vector3D* pos, const int layer, const int ladder) {
  ...
  gear::Vector3D* LocalPos = new gear::Vector3D(
      posR * (cos(posphi) * _geodata[layer].sinphi[ladder] - sin(posphi) * _geodata[layer].cosphi[ladder]), pos->z(),
      posR * (cos(posphi) * _geodata[layer].cosphi[ladder] + sin(posphi) * _geodata[layer].sinphi[ladder]) - radius);
  return LocalPos;
}

// =====================================================================
void FPCCDDigitizer::getInOutPosOnLadder(int layer, gear::Vector3D* outpos, gear::Vector3D* inpos, gear::Vector3D* pos,
                                         gear::Vector3D* mom) {
  ...
  *pos = gear::Vector3D((outpos->x() + inpos->x()) / 2, (outpos->y() + inpos->y()) / 2, (outpos->z() + inpos->z()) / 2);

  return;
}

// =====================================================================
void FPCCDDigitizer::ModifyIntoLadder(gear::Vector3D* bemodifiedpos, const int f_layer, gear::Vector3D* pos,
                                      gear::Vector3D* mom) {
...
// =====================================================================
void FPCCDDigitizer::getInOutPosOfHelixOnLadder(int layer, gear::Vector3D* outpos, gear::Vector3D* inpos,
                                                gear::Vector3D* pos, gear::Vector3D* mom, gear::Vector3D* BField,
                                                float Charge) {
  double out[3];
  double in[3];
  ...
  double init_dir[3] = {mom->x() / tmom.r(), mom->y() / tmom.r(), mom->z() / tmom.r()};
  ...
  if (fabs((outerZ - offset_pos[2]) / radius) <= 1.)
    outerZ_phi = 1 / Charge * asin((outerZ - offset_pos[2]) / radius) - asin_offset;
  if (fabs((innerZ - offset_pos[2]) / radius) <= 1.)
    innerZ_phi = 1 / Charge * asin((innerZ - offset_pos[2]) / radius) - asin_offset;
  ...
  if (radius * cos(Charge * (outphi + offset_phi)) + offset_pos[0] <= sximin ||
      ...
      v_y * (outphi + offset_phi) + offset_pos[1] >= szetamax)
    flag = flag + 1; // outerZ_phi will be modified.

  if (radius * cos(Charge * (inphi + offset_phi)) + offset_pos[0] <= sximin ||
      radius * cos(Charge * (inphi + offset_phi)) + offset_pos[0] >= sximax ||
      v_y * (inphi + offset_phi) + offset_pos[1] <= szetamin || v_y * (inphi + offset_phi) + offset_pos[1] >= szetamax)
    flag = flag + 2; // innerZ_phi will be modified.
  ...
  if (flag != 0) {
    ...
    if (fabs((sximin - offset_pos[0]) / radius) <= 1.)
      sximin_phi = sign * (1 / Charge * acos((sximin - offset_pos[0]) / radius) - acos_offset);
    if (fabs((sximax - offset_pos[0]) / radius) <= 1.)
      sximax_phi = sign * (1 / Charge * acos((sximax - offset_pos[0]) / radius) - acos_offset);
    double szetamin_phi = (szetamin - offset_pos[1]) / v_y - offset_phi;
    double szetamax_phi = (szetamax - offset_pos[1]) / v_y - offset_phi;

    if (fabs(sximin_phi) <= fabs(sximax_phi) && fabs(sximin_phi) <= fabs(szetamin_phi) &&
        fabs(sximin_phi) <= fabs(szetamax_phi)) {
      tmpphi = sximin_phi;
      sximin_phi = 1e+8;
  ...
  *mom = gear::Vector3D(-radius * sin(Charge * (center_phi + offset_phi)), v_y,
...
// =====================================================================
std::vector<std::pair<const gear::Vector3D*, int>>
FPCCDDigitizer::getIntersectionOfTrkAndPix(const int f_layer, gear::Vector3D* top, gear::Vector3D* bottom) {
  std::vector<std::pair<const gear::Vector3D*, int>> EdgeOfPixel;
  double sximin = _geodata[f_layer].sximin;
  double szetamin = -_geodata[f_layer].hlength;
  double small_localx;
  ...
  if (top->x() >= bottom->x()) {
    large_localx = top->x();
    small_localx = bottom->x();
  } else {
    large_localx = bottom->x();
    small_localx = top->x();
  }
  if (top->y() >= bottom->y()) {
    large_localy = top->y();
    small_localy = bottom->y();
  } else {
    large_localy = bottom->y();
    small_localy = top->y();
  }
  ...
  for (int j = 0; j < nloopx; j++) {
    double XEdgeOfPixelZ = ((top->z() - bottom->z()) / (top->x() - bottom->x())) *
                               (sximin + (sloopx + j) * _pixelSize - (top->x() + bottom->x()) / 2.) +
                           (top->z() + bottom->z()) / 2.;
    double XEdgeOfPixelX = sximin + (sloopx + j) * _pixelSize;
    double XEdgeOfPixelY = ((top->y() - bottom->y()) / (top->x() - bottom->x())) *
                               (sximin + (sloopx + j) * _pixelSize - (top->x() + bottom->x()) / 2.) +
                           (top->y() + bottom->y()) / 2.;
  ...
  for (int j = 0; j < nloopy; j++) {
    double YEdgeOfPixelZ = ((top->z() - bottom->z()) / (top->y() - bottom->y())) *
                               (szetamin + (sloopy + j) * _pixelSize - (top->y() + bottom->y()) / 2.) +
                           (top->z() + bottom->z()) / 2.;
    double YEdgeOfPixelX = ((top->x() - bottom->x()) / (top->y() - bottom->y())) *
                               (szetamin + (sloopy + j) * _pixelSize - (top->y() + bottom->y()) / 2.) +
                           (top->x() + bottom->x()) / 2.;
...
FPCCDDigitizer::getLocalPixel(SimTrackerHitImpl* simthit, vector<std::pair<const gear::Vector3D*, int>> edgeofpixel) {
...
// =====================================================================
std::pair<int, int>* FPCCDDigitizer::FindPixel(std::pair<const gear::Vector3D*, int> fst,
                                               std::pair<const gear::Vector3D*, int> nxt, int layer) {
...
// =====================================================================
void FPCCDDigitizer::makeCandidates(std::pair<const gear::Vector3D*, int> edge, std::pair<int, int>* cand_array,
                                    int layer) {
  ...
  default:
    if (_debug == 1)
      std::cout << "pixel candidates cannot found. : " << edge.second << std::endl;
    break;
  }
  *cand_array = candID1;
...
// =====================================================================
void FPCCDDigitizer::makeNewSimTHit(SimTrackerHitImpl* simthit, gear::Vector3D* newpos, gear::Vector3D* newmom,
                                    int layer, int ladder, double newPathLength) {
  ...
  double pos[3] = {newpos->x() * _geodata[layer].sinphi[ladder] + eta0 * _geodata[layer].cosphi[ladder],
                   -newpos->x() * _geodata[layer].cosphi[ladder] + eta0 * _geodata[layer].sinphi[ladder], newpos->y()};
...
bool FPCCDDigitizer::inSensitiveRegion(gear::Vector3D* pos, int layer) {
  if (fabs(pos->z()) > 0.5 * _pixelheight + 1e-5 || fabs(pos->y()) > _geodata[layer].hlength + 1e-5 ||
      pos->x() > _geodata[layer].sximax + 1e-5 || pos->x() < _geodata[layer].sximin - 1e-5) {
    return false;
  } else
    return true;
}
