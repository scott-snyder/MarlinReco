...
VTXNoiseClusters::VTXNoiseClusters() : Processor("VTXNoiseClusters") {
  ...
  registerProcessorParameter("HitDensityPerLayer_VTX", "hit densities (hits/cm^2) per VXD layer", _densities,
                             densityDefault);
  ...
  registerProcessorParameter("RootHistograms", "root file name and histogram names (one per layer)", _rootNames,
                             rootDefault);
  ...
  registerProcessorParameter("RandomSeed", "random seed - default 42", _ranSeed, int(42));
}
...
void VTXNoiseClusters::init() {
  ...
  if ((unsigned)layerVXD.getNLayers() != _rootNames.size() - 1) {
    ...
    streamlog_out(ERROR) << " *************************************************** " << std::endl
                         << " wrong number of histograms: " << _rootNames.size() << " for " << layerVXD.getNLayers()
                         << " VXD layers "
                         << "  - do nothing ! " << std::endl
                         << " *************************************************** " << std::endl;
  }
  ...
  for (int i = 0; i < layerVXD.getNLayers(); ++i) {
    ...
    streamlog_out(DEBUG) << " 2D histo - bins (x,y): " << _hist[i]->GetNbinsX() << ", "
                         ...
                         << " , " << _hist[i]->GetNbinsY() << ", " << _hist[i]->GetYaxis()->GetBinLowEdge(1) << ", "
...
void VTXNoiseClusters::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }
...
void VTXNoiseClusters::modifyEvent(LCEvent* evt) {
  ...
  try {
    ...
    streamlog_out(WARNING) << " VTX collection " << _colNameVTX << " not found - do nothing ! " << std::endl;
  ...
  if ((unsigned)layerVXD.getNLayers() != _densities.size()) {
    ...
    streamlog_out(ERROR) << " *************************************************** " << std::endl
                         << " wrong number of hit densities: " << _densities.size() << " for " << layerVXD.getNLayers()
                         << " VXD layers "
                         << "  - do nothing ! " << std::endl
                         << " *************************************************** " << std::endl;
  ...
  for (int i = 0; i < layerVXD.getNLayers(); i++) {
    ...
    for (int j = 0; j < nLad; j++) {
      ...
      streamlog_out(DEBUG) << " layer: " << i << " - ladder: " << j << " density: " << _densities[i]
                           << " area: " << area << " #hits: " << nHit << std::endl;
...
void VTXNoiseClusters::check(LCEvent* evt) {
...
#ifdef MARLIN_USE_AIDA
  struct H1D {
    enum { hitsLayer1, hitsLayer2, hitsLayer3, hitsLayer4, hitsLayer5, hitsLayer6, size };
  };
  ...
  if (isFirstEvent()) {
    ...
    float hitMax = 100000.;
    _hist1DVec[H1D::hitsLayer1] =
        AIDAProcessor::histogramFactory(this)->createHistogram1D("hitsLayer1", "hits Layer 1", 100, 0., hitMax);
    _hist1DVec[H1D::hitsLayer2] =
        AIDAProcessor::histogramFactory(this)->createHistogram1D("hitsLayer2", "hits Layer 2", 100, 0., hitMax);
    _hist1DVec[H1D::hitsLayer3] =
        AIDAProcessor::histogramFactory(this)->createHistogram1D("hitsLayer3", "hits Layer 3", 100, 0., hitMax);
    _hist1DVec[H1D::hitsLayer4] =
        AIDAProcessor::histogramFactory(this)->createHistogram1D("hitsLayer4", "hits Layer 4", 100, 0., hitMax);
    _hist1DVec[H1D::hitsLayer5] =
        AIDAProcessor::histogramFactory(this)->createHistogram1D("hitsLayer5", "hits Layer 5", 100, 0., hitMax);
    _hist1DVec[H1D::hitsLayer6] =
        AIDAProcessor::histogramFactory(this)->createHistogram1D("hitsLayer6", "hits Layer 6", 100, 0., hitMax);
    ...
    for (int i = 0; i < H1D::size; ++i) {
      ...
      _hist2DVec[i] = AIDAProcessor::histogramFactory(this)->createHistogram2D(name.str(), comment.str(), nx, xMin,
                                                                               xMax, ny, yMin, yMax);
    }
  }
#endif
...
void VTXNoiseClusters::end() {
  ...
  std::cout << "VTXNoiseClusters::end()  " << name() << " processed " << _nEvt << " events in " << _nRun << " runs "
            << std::endl;
}
