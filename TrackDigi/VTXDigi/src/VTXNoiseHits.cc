...
VTXNoiseHits::VTXNoiseHits() : Processor("VTXNoiseHits") {
  ...
  // Input collection
  registerInputCollection(LCIO::TRACKERHIT, "VTXCollectionName", "Name of the VTX TrackerHit collection", _colNameVTX,
                          std::string("VTXTrackerHits"));

  registerProcessorParameter("PointResolutionRPhi_VTX", "R-Phi Resolution in VTX", _pointResoRPhiVTX, float(0.0027));

  registerProcessorParameter("PointResolutionZ_VTX", "Z Resolution in VTX", _pointResoZVTX, float(0.0027));
}
...
void VTXNoiseHits::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }
...
void VTXNoiseHits::end() {
  ...
  std::cout << "VTXNoiseHits::end()  " << name() << " processed " << _nEvt << " events in " << _nRun << " runs "
            << std::endl;
}
