...
bool isEqual(gear::Vector3D v0, gear::Vector3D v1) {
  ...
  return isEqual(v0[0], v1[0]) && isEqual(v0[1], v1[1]) && isEqual(v0[2], v1[2]);
}

VXDGeometry::VXDGeometry(gear::GearMgr* gearMgr) : _gearMgr(gearMgr) { init(); }
...
void VXDGeometry::init() {
  ...
  streamlog_out(DEBUG0) << " initializing VXD ladder geometry from gear ... " << std::endl;
  ...
  for (int i = 0; i < layerVXD.getNLayers(); i++) {
    ...
    streamlog_out(DEBUG4) << " layer: " << i << " phi0 : " << phi0 << " offs : " << offs << " width : " << width
                          << " length : " << _vxdLayers[i].length << " ladderArea : " << _vxdLayers[i].ladderArea
                          << " rMin : " << _vxdLayers[i].rMin << " rMax : " << _vxdLayers[i].rMax << std::endl;
    ...
    for (int j = 0; j < nLad; j++) {
      ...
      gear::Vector3D t1(std::abs(offs), phi + sign * M_PI / 2.,
                        0, // keep z coordinate
...
gear::Vector3D VXDGeometry::lab2LadderPos(gear::Vector3D labPos, int layerID, int ladderID) {
  ...
  return gear::Vector3D(u.rho(), u.phi() - _vxdLadders[layerID][ladderID].phi, u.z(), gear::Vector3D::cylindrical);
}

gear::Vector3D VXDGeometry::ladder2LabPos(gear::Vector3D ladderPos, int layerID, int ladderID) {

  gear::Vector3D r(ladderPos.rho(), ladderPos.phi() + _vxdLadders[layerID][ladderID].phi, ladderPos.z(),
                   gear::Vector3D::cylindrical);
...
gear::Vector3D VXDGeometry::lab2LadderDir(gear::Vector3D labDir, int layerID, int ladderID) {
  ...
  return gear::Vector3D(labDir.rho(), labDir.phi() - _vxdLadders[layerID][ladderID].phi, labDir.z(),
                        gear::Vector3D::cylindrical);
}

gear::Vector3D VXDGeometry::ladder2LabDir(gear::Vector3D ladderDir, int layerID, int ladderID) {

  return gear::Vector3D(ladderDir.rho(), ladderDir.phi() + _vxdLadders[layerID][ladderID].phi, ladderDir.z(),
                        gear::Vector3D::cylindrical);
}
...
std::pair<int, int> VXDGeometry::getLadderID(gear::Vector3D labPos, int layerID) {
    ...
    for (unsigned i = 0; i < n; ++i) { // loop over all layers
      ...
      if (_vxdLayers[i].rMin <= r && r <= _vxdLayers[i].rMax) { // candidate layer
        ...
        for (unsigned j = 0; j < m; ++j) { // check
          ...
          if ((0 < z_abs && z_abs < _vxdLayers[i].length) && (std::abs(l.y()) < _vxdLayers[i].width / 2.) &&
  ...
  for (unsigned j = 0; j < m; ++j) { // check
    ...
    if ((0 < z_abs && z_abs < _vxdLayers[layerID].length) && (std::abs(l.y()) < _vxdLayers[layerID].width / 2.) &&
...
void VXDGeometry::test() {
  ...
  for (unsigned i = 0; i < _vxdLayers.size(); ++i) {
    ...
    for (unsigned j = 0; j < _vxdLadders[i].size(); ++j) {
      ...
      streamlog_out(MESSAGE) << " testing  " << nHit << " random points for ladder " << j << " of layer " << i
                             << std::endl;
      ...
      for (int k = 0; k < nHit; k++) {
        ...
        if (!isEqual(ladRnd, ladder)) {
          ...
          streamlog_out(WARNING) << " l1,l2: " << l1 << ", " << l2 << std::endl
                                 << " ladRnd: " << ladRnd << " lab   : " << lab << " ladder: " << ladder << std::endl;

          streamlog_out(DEBUG4) << "isEqual(ladRnd[0],ladder[0]): " << isEqual(ladRnd[0], ladder[0]) << std::endl
                                << "isEqual(ladRnd[1],ladder[1]): " << isEqual(ladRnd[1], ladder[1]) << std::endl
                                << "isEqual(ladRnd[2],ladder[2]): " << isEqual(ladRnd[2], ladder[2]) << std::endl;
        }
        ...
        if ((unsigned)id.first != i) {
          ...
          streamlog_out(WARNING) << "  getLadderID layer wrong : " << id.first << " instead of " << i << std::endl;
        }
        if ((unsigned)id.second != j) {

          isOK = false;

          streamlog_out(WARNING) << "  getLadderID ladder wrong : " << id.second << " instead of " << j << " layer "
                                 << i << std::endl;
        }
        ...
        if (id != getLadderID(lab, i)) {
          ...
          streamlog_out(WARNING) << "  getLadderID with ID=" << i << " - ladder wrong : " << id.second << std::endl;
        }

        streamlog_out(DEBUG) << " ladRnd: " << ladRnd << " lab   : " << lab << " ladder: " << ladder << std::endl;
      }
      if (isOK)
        streamlog_out(MESSAGE) << " tested  " << nHit << " random points for ladder " << j << " of layer " << i
                               << " OK ! " << std::endl;
    }
  } // ------ loop over layers
}
