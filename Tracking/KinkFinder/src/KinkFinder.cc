...
KinkFinder::KinkFinder() : Processor("KinkFinder") {
  ...
  registerInputCollection(LCIO::TRACK, "TrackCollection", "Name of input collection of reconstructed particles",
                          _trackColName, std::string("LDCTracks"));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "KinkRecoParticleCollection",
                           "Name of output collection of kink reconstructed particles", _kinkRecoPartColName,
                           std::string("KinkRecoParticles"));

  registerOutputCollection(LCIO::VERTEX, "KinkVertexCollection", "Name of output collection of kink vertices",
                           _kinkVertexColName, std::string("KinkVertices"));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "ProngRecoParticleCollection",
                           "Name of output collection of prong reconstructed particles", _prongRecoPartColName,
                           std::string("ProngRecoParticles"));

  registerOutputCollection(LCIO::VERTEX, "ProngVertexCollection", "Name of output collection of prong vertices",
                           _prongVertexColName, std::string("ProngVertices"));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "SplitRecoParticleCollection",
                           "Name of output collection of split reconstructed particles", _splitRecoPartColName,
                           std::string("SplitRecoParticles"));

  registerOutputCollection(LCIO::VERTEX, "SplitVertexCollection", "Name of output collection of split vertices",
                           _splitVertexColName, std::string("SplitVertices"));

  registerProcessorParameter("SplitTrackMaxFracDeltaP", "Maximum fractional difference in deltaP for split tracks",
                             _maxSplitTrackFracDeltaP, float(0.02));

  registerProcessorParameter("SplitTrackMaxDeltaP", "Maximum difference in deltaP for split tracks",
                             _maxSplitTrackDeltaP, float(0.2));

  registerProcessorParameter("CutOnRadius", "Cuts on Kink radius", _rKinkCut, float(100.0));

  registerProcessorParameter("MinimumTrackHits", "cuts on number of track hits", _minTrackHits, int(5));

  registerProcessorParameter("MaxDeltaTpcLayers", "Cuts on Kink DeltaRxy for tracks in TPC", _maxDeltaTpcLayers,
                             int(10));

  registerProcessorParameter("PionDecayMassCut", "Cuts on kink mass for pion decay", _pionDecayMassCut, float(0.03));

  registerProcessorParameter("KaonDecayMassCut", "Cuts on kink mass for kaon decay", _kaonDecayMassCut, float(0.075));

  registerProcessorParameter("SigmaDecayMassCut", "Cuts on kink mass for sigma decay", _sigmaDecayMassCut, float(0.15));

  registerProcessorParameter("SigmaTimeCut", "Cuts on lifetime for sigma decay", _sigmaTimeCut, float(6.));

  registerProcessorParameter("HyperonDecayMassCut", "Cuts on kink mass for hyperon decay", _hyperonDecayMassCut,
                             float(0.15));

  registerProcessorParameter("HyperonTimeCut", "Cuts on lifetime for hyperon decay", _hyperonTimeCut, float(6.));

  registerProcessorParameter("KinkProjectionCutTPC", "Cuts on kink distance of closest approach in TPC", _drCutTPC,
                             float(20));

  registerProcessorParameter("TightKinkProjectionCutTPC", "Tight Cut on kink distance of closest approach in TPC",
                             _tightDrCutTPC, float(5));

  registerProcessorParameter("VeryTightKinkProjectionCutTPC",
                             "Very Tight Cut on kink distance of closest approach in TPC", _veryTightDrCutTPC,
                             float(1));

  registerProcessorParameter("KinkProjectionCutSIT", "Cuts on kink distance of closest approach in SIT", _drCutSIT,
                             float(10));

  registerProcessorParameter("LooseProjectionCutSIT", "Cuts on kink distance of closest approach in SIT",
                             _looseDrCutSIT, float(10));

  registerProcessorParameter("MinELambda", "Minimum Lambda Energy", _minELambda, float(2.));

  registerProcessorParameter("DebugPrinting", "Debug level", _debugPrinting, int(0));
}
...
void KinkFinder::processEvent(LCEvent* evt) {
  ...
  if (_debugPrinting > 0)
    std::cout << " Searching for kinks/prongs/split tracks " << std::endl;
  ...
  for (unsigned int itrack = 0; itrack < tracks.size(); ++itrack) {
    ...
    if (trackNavigator != NULL) {
      ...
      if (objectVec.size() > 0)
        mcpart = dynamic_cast<MCParticle*>(objectVec[0]);
      if (objectVec.size() > 1) {
    ...
    helixStart[itrack]->Initialize_BZ(x0, y0, r0, bz, phiH, _bField, signPz, zBegin);
    ...
    helixEnd[itrack]->Initialize_BZ(x0, y0, r0, bz, phiH, _bField, signPz, zEnd);
        ...
        if (i != j && hits[j] > _minTrackHits && momentum[j] < (1.0 + _maxSplitTrackFracDeltaP) * momentum[i]) {
          ...
          if (rInner[i] > _rKinkCut || rInner[j] > _rKinkCut) {
              ...
              if (parents.size() > 0 && mcParticle[i] != NULL) {
                for (unsigned im = 0; im < parents.size(); im++) {
                  if (parents[im] == mcParticle[i] && momentum[i] > 2.0) {
                    ...
                    if (abs(mcParticle[i]->getPDG()) == 321 && abs(mcParticle[j]->getPDG()) == 13)
                      mcKink = true;
                    if (abs(mcParticle[i]->getPDG()) == 321 && abs(mcParticle[j]->getPDG()) == 211)
                      mcKink = true;
                    if (abs(mcParticle[i]->getPDG()) == 3222 && abs(mcParticle[j]->getPDG()) == 211)
                      mcKink = true;
                    if (abs(mcParticle[i]->getPDG()) == 3212 && abs(mcParticle[j]->getPDG()) == 211)
                      mcKink = true;
                    if (abs(mcParticle[i]->getPDG()) == 3112 && abs(mcParticle[j]->getPDG()) == 211)
                      mcKink = true;
                  }
                }
              }
            ...
            if (rkink > _rVTX[_nLayersVTX - 1]) {
              ...
              if (dr < _tightDrCutTPC)
                deltaRxyCut = deltaRxyCut * 1.5;
              if (dr < _veryTightDrCutTPC)
                deltaRxyCut = deltaRxyCut / 1.5 * 2.0;
              ...
              if (rkink < _tpcInnerR + deltaRxyCut) {
                ...
                if (iSitLayer == 0)
                  deltaRxyCut = _rSIT[0] - _rVTX[_nLayersVTX - 1];
                // kink between SIT and TPC
                if (iSitLayer == _nLayersSIT) {
                  deltaRxyCut = _tpcInnerR - _rSIT[_nLayersSIT - 1];
                  drCut = _drCutTPC;
                }
                if (iSitLayer > 0 && iSitLayer < _nLayersSIT)
                  deltaRxyCut = _rSIT[iSitLayer] - _rSIT[iSitLayer - 1];
                // add some protection for SIT layers - large gap
                int ili = -999;
                int ilo = -999;
                for (int il = 0; il < _nLayersSIT; il++) {
                  if (fabs(rOuter[i] - _rSIT[il]) < 10.)
                    ili = il;
                  if (fabs(rInner[j] - _rSIT[il]) < 10.)
                    ilo = il;
                }
                if (ili >= 0 && ilo >= 0) {
            ...
            //	    std::cout << i << " : " << j << " dr = " << dr << " ( " << drCut << " )    deltaRxy = " << deltarxy
            ...
            if ((dr < drCut && deltarxy < deltaRxyCut * 2) || mcKink) {
              ...
              if ((rkink > _rKinkCut && !flipped) || mcKink) {
                ...
                if (_pionDecayMassCut > 0) {
                  ...
                  if (deltaPion < 1 && tPion > 0.001)
                    probPion = 3.125 * deltaPion * deltaPion + tPion;
                }
                if (_kaonDecayMassCut > 0) {
                  ...
                  if (deltaKaonPiPi < deltaKaon)
                    deltaKaon = deltaKaonPiPi;
                  if (deltaKaon < 1 && tKaon > 0.005)
                    probKaon = 3.125 * deltaKaon * deltaKaon + tKaon;
                }
                if (_sigmaDecayMassCut > 0 && momentum[i] > _minELambda) {
                  ...
                  if (deltaSigmaPPi0 < deltaSigma)
                    deltaSigma = deltaSigmaPPi0;
                  if (deltaSigma < 1 && tSigma < _sigmaTimeCut)
                    probSigma = 3.125 * deltaSigma * deltaSigma + tSigma;
                }
                if (_hyperonDecayMassCut > 0 && momentum[i] > _minELambda) {
                  float deltaHyperon = fabs(massPiL - mHyperon) / _hyperonDecayMassCut;
                  if (deltaHyperon < 1 && tHyperon < _hyperonTimeCut)
                    probHyperon = 3.125 * deltaHyperon * deltaHyperon + tHyperon;
                }
                bool goodKinkMass = false;
                if (probPion > 0.0001 || probKaon > 0.0001 || probSigma > 0.0001 || probHyperon > 0.0001)
                  goodKinkMass = true;
                ...
                if ((deltarxy < deltaRxyCut && dr < drCut) || possibleSplit) {
                  ...
                  if (deltarxy < 2 * deltaRxyCut && dr < drCut * 2) {
                    if (possibleSplit && charge[i] * charge[j] > 0) {
                      ...
                      if (_debugPrinting > 0) {
                        std::cout << " CAND SPLIT I : " << nhitsi << " ntpc " << ntpci << " nclose " << nclosei
                                  << " max " << maxdisti << " fclose : " << fclosei << std::endl;
                        std::cout << " CAND SPLIT J : " << nhitsj << " ntpc " << ntpcj << " nclose " << nclosej
                                  << " max " << maxdistj << " fclose : " << fclosej << std::endl;
                      }
                  ...
                  if (deltarxy < deltaRxyCut && dr < drCut) {
                    if (charge[i] * charge[j] > 0 && goodKinkMass) {
                      if (_debugPrinting > 0)
                        std::cout << "Found  kink candidate " << i << " " << j << std::endl;
                      if (probPion > 0 || probKaon > 0 || probSigma > 0 || probHyperon > 0)
                        goodKinkMass = true;
                      if (probPion > probKaon && probPion > probSigma && probPion > probHyperon) {
                        kinkij.pdgCode = 211;
                        if (charge[i] < 0)
                          kinkij.pdgCode = -211;
                        kinkij.mass = mPion;
                      }
                      if (probKaon > probPion && probKaon > probSigma && probKaon > probHyperon) {
                        kinkij.mass = mKaon;
                        kinkij.pdgCode = 321;
                        if (charge[i] < 0)
                          kinkij.pdgCode = -321;
                      }
                      if (probSigma > probPion && probSigma > probKaon && probSigma > probHyperon) {
                        kinkij.mass = mSigma;
                        kinkij.pdgCode = 3222;
                        if (charge[i] < 0)
                          kinkij.pdgCode = -3222;
                      }
                      if (probHyperon > probPion && probHyperon > probKaon && probHyperon > probSigma) {
                  ...
                  if (deltarxy < deltaRxyCut && dr < drCut) {
                    ...
                    if (_debugPrinting > 0)
                      std::cout << "Found prong candidate " << i << " " << j << std::endl;
                ...
                if (_debugPrinting > 0) {
                  ...
                  std::cout << "   Candidate kink tracks : " << i << "," << j << " p[i] " << momentum[i] << " p[j] "
                            << momentum[j] << std::endl;
                  if (flipped)
                    std::cout << "   Flipped Track " << std::endl;
                  ;
                  std::cout << "   MC  : ";
                  if (mcParticle[i] != NULL)
                    std::cout << mcParticle[i]->getPDG();
                  std::cout << " - ";
                  if (mcParticle[j] != NULL)
                    std::cout << mcParticle[j]->getPDG();
                  std::cout << std::endl;
                  std::cout << "   Mass                  : " << massENu << " " << massMuNu << " " << massPiPi << " "
                            << massPiN << " " << massPPi0 << " " << massPiL << std::endl;
                  if (flipped)
                    std::cout << "   MassF                 : " << FmassENu << " " << FmassMuNu << " " << FmassPiPi
                              << " " << FmassPiN << " " << FmassPPi0 << FmassPiL << std::endl;

                  std::cout << "     ProbPion            : " << probPion << " TimePion    : " << tPion
                            << " Dm = " << massMuNu - mPion << std::endl;
                  std::cout << "     ProbKaon            : " << probKaon << " TimeKaon    : " << tKaon
                            << " Dm = " << massMuNu - mKaon << " Dm = " << massPiPi - mKaon << std::endl;
                  std::cout << "     ProbSigma           : " << probSigma << " TimeSigma   : " << tSigma
                            << " Dm = " << massPiN - mSigma << " Dm = " << massPPi0 - mSigma << std::endl;
                  std::cout << "     ProbHyperon         : " << probHyperon << " TimeHyperon : " << tHyperon
                            << " Dm = " << massPiL - mHyperon << std::endl;
                  std::cout << "   Charge                : " << i << "," << j << " q[i] " << charge[i] << " q[j] "
                            << charge[j] << std::endl;
                  std::cout << "   Rendi                 : " << i << "," << j << "  rend " << rendi << "  dr = " << dr
                            << " (deltaR = " << deltar << ")" << std::endl;
                  std::cout << "   zkink                 : " << zkink << " rzykink " << rkink << std::endl;
                  if (deltarxy < deltaRxyCut)
                    std::cout << "   DeltaRxy vs cut       : " << deltarxy << " < " << deltaRxyCut << std::endl;
                  if (deltarxy > deltaRxyCut)
                    std::cout << "   DeltaRxy vs cut       : " << deltarxy << " > " << deltaRxyCut << std::endl;
                  if (dr < drCut)
                    std::cout << "   dr vs cut             : " << dr << " < " << drCut << std::endl;
                  if (dr > drCut)
                    std::cout << "   dr vs cut             : " << dr << " > " << drCut << std::endl;
                  std::cout << "   zi/zj                 : " << zAtStart[i] << " - " << zAtEnd[i] << "     "
                            << zAtStart[j] << " - " << zAtEnd[j] << std::endl;
                  std::cout << "   rinner/router         : " << rInner[i] << " - " << rOuter[i] << "     " << rInner[j]
                            << " - " << rOuter[j] << std::endl;
                }
              }
            }
    ...
    if (kinkDaughters[i].size() > 0 || prongDaughters[i].size() > 0 || splitDaughters[i].size() > 0) {
      if (_debugPrinting > 0) {
        if (kinkDaughters[i].size() > 0) {
          ...
          for (unsigned j = 0; j < kinkDaughters[i].size(); j++)
            std::cout << kinkDaughters[i][j].trackj << " ";
          std::cout << std::endl;
        }
        if (prongDaughters[i].size() > 0) {
          std::cout << " Track " << i << " has " << prongDaughters[i].size() << " prong daughters : ";
          for (unsigned j = 0; j < prongDaughters[i].size(); j++)
            std::cout << prongDaughters[i][j].trackj << " ";
          std::cout << std::endl;
        }
        if (splitDaughters[i].size() > 0) {
          std::cout << " Track " << i << " has " << splitDaughters[i].size() << " split daughters : ";
          for (unsigned j = 0; j < splitDaughters[i].size(); j++)
            std::cout << splitDaughters[i][j].trackj << " ";
          std::cout << std::endl;
        }
      }
      ...
      if (splitDaughters[i].size() == 1) {
        if (_debugPrinting > 0)
          std::cout << " Reconstructed SPLIT " << std::endl;
        ...
        if (_debugPrinting > 0) {
          ...
          std::cout << "   Momentum = (" << mom[0] << "," << mom[1] << "," << mom[2] << ")" << std::endl;
        }
      ...
      if (splitDaughters[i].size() != 1) {
        if (kinkDaughters[i].size() == 1 && prongDaughters[i].size() < 2) {
          if (_debugPrinting > 0)
            std::cout << " Reconstructed KINKS " << std::endl;
          ReconstructedParticleImpl* part = new ReconstructedParticleImpl();
          VertexImpl* vtx = new VertexImpl();
          float vertex[3];
          float mom[3];
          ...
          if (_debugPrinting > 0)
            std::cout << " Reconstructed PRONG " << std::endl;
          ReconstructedParticleImpl* part = new ReconstructedParticleImpl();
          VertexImpl* vtx = new VertexImpl();
          float vertex[3];
          float mom[3];
          ...
          for (unsigned id = 0; id < prongDaughters[i].size(); id++)
            part->addTrack(tracks[prongDaughters[i][id].trackj]);
...
float KinkFinder::kinkMass(HelixClass* parent, HelixClass* daughter, float daughterMass, float neutralMass) {
  ...
  float mx = sqrt((daughterE + Enu) * (daughterE + Enu) - parentMom[0] * parentMom[0] - parentMom[1] * parentMom[1] -
