...
TrackLengthProcessor::TrackLengthProcessor() : marlin::Processor("TrackLengthProcessor") {
  _description =
      "TrackLengthProcessor computes track length and harmonic mean of the momentum square up to the last tracker hit and Ecal surface.\
...
void TrackLengthProcessor::processEvent(EVENT::LCEvent* evt) {
  ...
  for (int i = 0; i < pfos->getNumberOfElements(); ++i) {
    ...
    if (pfo->getTracks().empty()) {
      ...
      // Note: If the pfo has more than one track attached (e.g. kink, v0), only the first track will be used. The
      // results may be not accurate in these cases.
    ...
    vector<float> results{float(trackLengthToSET), float(trackLengthToEcal), float(harmonicMomToSET),
    ...
    streamlog_out(DEBUG9) << "Harmonic mean momentum to the last tracker hit: " << float(harmonicMomToSET) << " GeV"
                          << std::endl;
